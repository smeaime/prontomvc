...
Begin VB.Form frmOCsPendientes 
   ...
   End
End
Attribute VB_Name = "frmOCsPendientes"
...
Private Sub cmd_Click(Index As Integer)
   ...
   'Set oRs = Aplicacion.OrdenesCompra.TraerFiltrado("_ItemsPendientesDeProducirModuloProduccion")
...
Private Sub Form_Load()
  ...
  'Filtrar:  que el botón OCs muestre las OCs que corresponden para el Material o Terminado a evaluar, idem para las OPs.
   ...
   'oRs.Filter = "idArticulo" = FiltroArticulo
   Set Lista.DataSource = oRs
...
Public Sub Lista_DblClick()
   ...
   End If
        ...
        glbIdDetOC = Lista.SelectedItem.Tag
        glbIdOC = Lista.SelectedItem.SubItems(18)
        glbNumeroOC = Lista.SelectedItem.SubItems(1)
         glbIdArticuloRet = Lista.SelectedItem.SubItems(7)
         glbCantRet = Lista.SelectedItem.SubItems(9)
         glbUniRet = Lista.SelectedItem.SubItems(10)
         glbClienteRet = Lista.SelectedItem.SubItems(5)
         glbFechaEntrega = Lista.SelectedItem.SubItems(2)
         glbIdColorRet = Lista.SelectedItem.SubItems(19)
         
         Me.Hide



'no Se puede elegir una lista  items?
'a medida que el tipo elije, habria que modificar el renglon de item
'de articulo de la OP, y en un idDetalleOrdenCompra1,2,3, ir metiendo
'los elegidos


'el coso de consultas solo te permite elegir un item... el popup de gmail para
'contactos tiene un check, no?  cómo puede ser que llegue a esta altura sin esas
'herramientas listas a mano???? -Fijate que el popup de contactos de gmail tiene
'un filtro + 2 grillas (la segunda tiene los que fuiste eligiendo, con la posibilidad
'de quitarlos desde ahi)
