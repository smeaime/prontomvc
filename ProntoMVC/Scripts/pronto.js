...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*
// para traerse los parametros de la url. Modo de uso: 
...
function QuitarRenglonDragDrop(idazar, getdata) {
    ...
    ///////////////
    // paso 1: borrar el renglon vac�o de yapa que agrega el D&D (pero no el dblClick) -pero c�mo sab�s que est�s en modo D&D?
    ///////////////
    ...
    if (segundorenglon != undefined) {
        if (segundorenglon.indexOf("dnd") != -1) {
            // tir� el renglon en modo dragdrop, no hizo dobleclic
    ...
    //ac� hay un problemilla... si el tipo est� usando el DnD, se crea un renglon libre arriba de todo...
...
function TraerPosicionLibre() {
    //ac� hay un problemilla... si el tipo est� usando el DnD, se crea un renglon libre arriba de todo... Y usando solo click, no.
...
function AgregarRenglonesEnBlanco(renglonVacio, nombregrilla) {
    ...
    //pasa q tengo q ver cu�ntos de los renglones existentes ya est�n vac�os!!!
    //pasa q tengo q ver cu�ntos de los renglones existentes ya est�n vac�os!!!
    //pasa q tengo q ver cu�ntos de los renglones existentes ya est�n vac�os!!!
    //pasa q tengo q ver cu�ntos de los renglones existentes ya est�n vac�os!!!
    //pasa q tengo q ver cu�ntos de los renglones existentes ya est�n vac�os!!!
    ...
    if (emptyRows > 0 && (rowsLlenas == rows || rows < 3)) {
        ...
        for (var i = 1; i <= emptyRows; i++) {
            ...
            gridceil = Math.ceil(Math.random() * 1000000); // ojo con esto, si usas el mismo id, la edicion de un renglon se va a pasar a otro al instante!, y no vas a entender q est� pasando 
...
$(function () {
    ...
    window.onerror = function (message, file, line, column, errorObj) {
        ...
        //qu� hacer con los casos de �Script Error.�???? http://danlimerick.wordpress.com/2014/01/18/how-to-catch-javascript-errors-with-window-onerror-even-on-chrome-and-firefox/
        //usar crossorigin
    ...
    // esta funcion es grosa, la unica que encontr� que arrancase

    $.fn.serializeObject = function () {

        // si no anda, verific� que el form est� declarado con:
        //  @using (Html.BeginForm(null, null, FormMethod.Post, new { id = "formid" }))
        // y no con un tag <form>

        //no funciona con lo oculto (por ejemplo, lo que no est� en el tab seleccionado!)
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data != "") {
                    ...
                    // alert("exito, se ejecutar� el callback");

                    if (typeof fCallbackEnExito == "function") fCallbackEnExito(); else alert('No se pas� el callback!');
                    return true; // tiene sentido devolver algo en una funcion as�ncrona?
                    // ademas, todo lo que hiciese esa funcion debiera ser revalidado: por ejemplo, si llama al m�todo Anular, le debe pasar como parametros la contrase�a 
                    // -pero entonces por qu� no hac�s la validacion de la contrase�a solo en la llamada del m�todo? 
...
$("#grabarNormal").click(function () {
    ...
    // cabecera.DetalleFacturas = [];  // el array no existe en el objeto cabecera, lo anexo a lo bestia (ojo que, en verdad, s� hay uno, porque lo cre� con HiddenFor)
...
function PedirFirma(o) {
    ...
    if (false) {
        //  mientras no sepa c�mo traerme los posibles firmantes sin pasar el Orden como parametro, usar el listado completo
