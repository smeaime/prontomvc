...
function calculateTotal() {
    ...
    try {
        ...
        for (var i = 0; i < dataIds.length; i++) {
            ...
            pr = parseFloat(data['PrecioU'].replace(",", "."));
            if (isNaN(pr)) { pr = 0; }
            ...
            ///////////////////////////////////////
            // bonificacion
            ib = parseFloat(data['ImporteBonificacion'].replace(",", ".")); // esta es la bonificacion del item (el item tiene un %bonif individual. El importe lo calculó "calculateItem", no? 
            if (isNaN(ib)) { ib = 0; }
        ...
        ibgbrut1 = Math.round((mvarNetoGravado - mvarIVANoDiscriminado) * mvarPorcentajeIBrutos1 / 100) || 0;
        ibgbrut2 = Math.round((mvarNetoGravado - mvarIVANoDiscriminado) * mvarPorcentajeIBrutos2 / 100) || 0;
        ibgbrut3 = Math.round((mvarNetoGravado - mvarIVANoDiscriminado) * mvarPorcentajeIBrutos3 / 100) || 0;
        ...
        mvarBaseMinimaParaPercepcionIVA = parseFloat($("#Cliente_BaseMinimaParaPercepcionIVA").val().replace(",", "."));
        mvarPorcentajePercepcionIVA = parseFloat($("#Cliente_PorcentajePercepcionIVA").val().replace(",", "."));
        if (mvarEsAgenteRetencionIVA && mvarNetoGravado >= mvarBaseMinimaParaPercepcionIVA) {
            mvarPercepcionIVA = Math.round(mvarNetoGravado * mvarPorcentajePercepcionIVA / 100)
        ...
        $("#ImporteBonificacion").val(ib2.toFixed(4));
        $("#Subtotal2").val(st3.toFixed(4));
        $("#ImporteIva1").val(ii1.toFixed(4));
        $("#ImporteTotal").val(tg.toFixed(4));

        grid.jqGrid('footerData', 'set', { NumeroObra: 'TOTALES', Cantidad: totalCantidad.toFixed(2), ImporteBonificacion: ib1.toFixed(4), ImporteIva: ii1.toFixed(4), ImporteTotalItem: tg.toFixed(4) });
...
function copiarListaDrag(rowid) {
    ...
    try {
        ...
        tmpdata['PrecioU'] = getdata['Precio'];
...
function copiarListaDrag2(rowid) {
    ...
    try {
        ...
        tmpdata['PrecioU'] = 0; // getdata['Precio'];
        tmpdata['Cantidad'] = getdata['Cantidad'];
        tmpdata['Costo'] = 0;
        tmpdata['Bonificacion'] = 0; // getdata['Bonificacion'];
...
$(document).ready(function () {
    ...
    $('#ListaDocumentos').jqGrid({
        url: ROOT + 'Articulo/DetDocumentos/', // no puedo usar el modelo para cargar la grilla?????
        postData: { 'Id': function () { return $("#IdArticulo").val(); } }, //esto es el parametro con que se llama a la url
        editurl: ROOT + 'Articulo/EditGridData/', // "clientArray",
        /////////
        datatype: 'json',
        mtype: 'POST',
        //////////////////////////////////////////////
        ...
        colNames: ['Acciones', 'IdDetalleArticuloDocumento', 'Documento'],
        colModel: [
                        ...
                        { name: 'PathDocumento', index: 'PathDocumento', label: 'TB', align: 'left', width: 300, editable: true,
                            hidden: false,
                            editoptions: {
                                //    disabled: 'disabled'
                            },
    ...
    $("#addDataDocumentos").click(function () {
        gridLocal = $("#ListaDocumentos");

        jQuery("#ListaDocumentos").jqGrid('editGridRow', "new", { addCaption: "Agregar registro", bSubmit: "Aceptar", bCancel: "Cancelar", width: 800, reloadAfterSubmit: false, closeOnEscape: true, closeAfterAdd: true,
            recreateForm: true,
            beforeShowForm: function (form) {
                var dlgDiv = $("#editmod" + gridLocal[0].id);
    ...
    $("#edtDataDocumentos").click(function () {
        gridLocal = $("#ListaDocumentos");
        var gr = jQuery("#ListaDocumentos").jqGrid('getGridParam', 'selrow');
        if (gr != null) jQuery("#ListaDocumentos").jqGrid('editGridRow', gr, { editCaption: "Modificar registro", bSubmit: "Aceptar", bCancel: "Cancelar", width: 800, reloadAfterSubmit: false, closeOnEscape: true,
    ...
    $("#delDataDocumentos").click(function () {
        var gr = jQuery("#ListaDocumentos").jqGrid('getGridParam', 'selrow');
        if (gr != null) {
            //jQuery("#Lista").jqGrid('setRowData',gr,{Eliminado:"true"});
            //$("#"+gr).hide();  
            jQuery("#ListaDocumentos").jqGrid('delGridRow', gr, { caption: "Borrar", msg: "Elimina el registro seleccionado?", bSubmit: "Borrar", bCancel: "Cancelar", width: 800, closeOnEscape: true, reloadAfterSubmit: true });
        }
        else alert("Debe seleccionar un item!");
    });
    ...
    $('#ListaUnidades').jqGrid({
        url: ROOT + 'Articulo/DetUnidades/', // no puedo usar el modelo para cargar la grilla?????
        postData: { 'Id': function () { return $("#IdArticulo").val(); } },
        editurl: ROOT + 'Articulo/EditGridData/', // "clientArray",
        datatype: 'json',
        mtype: 'POST',
        //////////////////////////////////////////////
        ...
        colNames: ['Acciones', 'IdDetalleArticuloUnidad', 'IdUnidad', 'Unidad', 'Equivalencia'],
        colModel: [
                        { name: 'act', index: 'act', align: 'left', width: 70, sortable: false, editable: false, hidden: true },
                        { name: 'IdDetalleArticuloUnidad', index: 'IdDetalleArticuloUnidad', label: 'TB', align: 'left',
                            width: 85, editable: false, hidden: true, editoptions: { disabled: 'disabled' }, editrules: { edithidden: false, required: true }
                        },
                        { name: 'IdUnidad', index: 'IdUnidad', label: 'TB', align: 'left',
                            width: 1,
                        ...
                        { name: 'Unidad', index: 'Unidad', align: 'left', width: 80, editable: true, edittype: 'select', editrules: { required: true },
                            editoptions: {
                        ...
                        { name: 'Equivalencia', index: 'Equivalencia', label: 'TB', align: 'left', width: 85, editable: true,
                            hidden: false,
                            editoptions: {
                                //    disabled: 'disabled'
                            },
    ...
    $("#addDataUnidades").click(function () {
        gridLocal = $("#ListaUnidades");

        jQuery("#ListaUnidades").jqGrid('editGridRow', "new", { addCaption: "Agregar registro", bSubmit: "Aceptar", bCancel: "Cancelar", width: 800, reloadAfterSubmit: false, closeOnEscape: true, closeAfterAdd: true,
            recreateForm: true,
            beforeShowForm: function (form) {
                var dlgDiv = $("#editmod" + gridLocal[0].id);
                dlgDiv[0].style.top = 0 //Math.round((parentHeight-dlgHeight)/2) + "px";
    ...
    $("#edtDataUnidades").click(function () {
        gridLocal = $("#ListaUnidades");
        var gr = jQuery("#ListaUnidades").jqGrid('getGridParam', 'selrow');
        if (gr != null) jQuery("#ListaUnidades").jqGrid('editGridRow', gr, { editCaption: "Modificar registro", bSubmit: "Aceptar", bCancel: "Cancelar", width: 800, reloadAfterSubmit: false, closeOnEscape: true,
            closeAfterEdit: true, recreateForm: true, Top: 0,
            beforeShowForm: function (form) {
                // "editmodlist"
                var dlgDiv = $("#editmod" + gridLocal[0].id);
    ...
    $("#delDataUnidades").click(function () {
        var gr = jQuery("#ListaUnidades").jqGrid('getGridParam', 'selrow');
        if (gr != null) {
            //jQuery("#Lista").jqGrid('setRowData',gr,{Eliminado:"true"});
            //$("#"+gr).hide();  
            jQuery("#ListaUnidades").jqGrid('delGridRow', gr, { caption: "Borrar", msg: "Elimina el registro seleccionado?", bSubmit: "Borrar", bCancel: "Cancelar", width: 800, closeOnEscape: true, reloadAfterSubmit: true });
        }
        else alert("Debe seleccionar un item!");
    });
    ...
    $('#Lista').jqGrid({
        ...
        datatype: 'json',
        mtype: 'POST',
        colNames: ['Acciones', 'IdDetalleFactura', 'IdArticulo', 'IdUnidad', 'NumeroItem', 'Codigo', 'Descripcion', 'Cantidad', 'Unidad',
        ...
        colModel: [
                        ...
                        { name: 'IdArticulo', index: 'IdArticulo', label: 'TB', align: 'left', width: 85, editable: true, hidden: true, editoptions: { disabled: 'disabled' }, editrules: { edithidden: false, required: true} },
        ...
        //{ name: 'Eliminado', index: 'Eliminado', label:'TB', align: 'left', width: 85, editable: true, hidden: true, editoptions: { disabled: 'disabled' }, editrules: { edithidden: true} },
                        ...
                        { name: 'Descripcion', index: 'Descripcion', align: 'left', width: 0, editable: true, edittype: 'text',
                            editoptions: {
                                dataInit: function (elem) {
                                    $(elem).autocomplete({
                          ...
                          { name: 'Cantidad', index: 'Cantidad', label: 'TB', align: 'right', width: 0, editable: true, edittype: 'text', editoptions: { maxlength: 20, dataEvents: [{ type: 'change', fn: function (e) { calculateItem(); } }] }, editrules: { required: true} }, //, unformat:numUnformat
                        {name: 'Unidad', index: 'IdUnidad', align: 'left', width: 20, editable: true, edittype: 'select', editrules: { required: true },
                        editoptions: {
                            dataUrl: ROOT + 'Articulo/Unidades',
                          ...
                          { name: 'Costo', index: 'Cantidad', label: 'TB', align: 'right', width: 30, editable: true, edittype: 'text', editoptions: { maxlength: 20, dataEvents: [{ type: 'change', fn: function (e) { calculateItem(); } }] }, editrules: { required: true} }, //, unformat:numUnformat
                          {name: 'PrecioU', index: 'PrecioU', label: 'TB', align: 'right', width: 60, editable: true, edittype: 'text', editoptions: { maxlength: 20, dataEvents: [{ type: 'change', fn: function (e) { calculateItem(); } }] }, editrules: { required: true} }, //, unformat:numUnformat
                          {name: 'Bonificacion', index: 'Bonificacion', label: 'TB', align: 'right', width: 60, editable: true, edittype: 'text', editoptions: { maxlength: 20, dataEvents: [{ type: 'change', fn: function (e) { calculateItem(); } }] }, editrules: { required: true} }, //, unformat:numUnformat
    ...
    $("#grabar2").click(function () {
        ...
        cabecera.DetalleArticulosDocumentos = [];  // el array no existe en el objeto cabecera, lo anexo a lo bestia (ojo que, en verdad, sí hay uno, porque lo creé con HiddenFor)
        var grilla1 = $('#ListaDocumentos');
        var stringData1 = '{"IdArticulo":"' + $("#IdArticulo").val() + '",'

        //////////////////////

        var colModel = grilla1.jqGrid('getGridParam', 'colModel');
        items = [];

        var dataIds = grilla1.jqGrid('getDataIDs');
        for (var i = 0; i < dataIds.length; i++) {
            try {
                grilla1.jqGrid('saveRow', dataIds[i], false, 'clientArray');
                var data = grilla1.jqGrid('getRowData', dataIds[i]);
                ...
                for (var j = 0; j < colModel.length; j++) {
                    ...
                    if (cm.label === 'TB') {
                        ...
                        if (cm.name === 'PathDocumento') valor = valor.replace("\\", "\\\\");
                ...
                // items.push(data2);               
                cabecera.DetalleArticulosDocumentos.push(data2);
            }
            catch (ex) {
                grilla1.jqGrid('restoreRow', dataIds[i]);
        ...
        cabecera.DetalleArticulosUnidades = [];  // el array no existe en el objeto cabecera, lo anexo a lo bestia (ojo que, en verdad, sí hay uno, porque lo creé con HiddenFor)
        grilla1 = $('#ListaUnidades');
        stringData1 = '{"IdArticulo":"' + $("#IdArticulo").val() + '",'

        //////////////////////

        colModel = grilla1.jqGrid('getGridParam', 'colModel');
        items = [];
        ...
        for (var i = 0; i < dataIds.length; i++) {
            try {
                ...
                // items.push(data2);               
                cabecera.DetalleArticulosUnidades.push(data2);
            }
            catch (ex) {
                grilla1.jqGrid('restoreRow', dataIds[i]);
                alert("No se pudo grabar el comprobante. Es probable que hayas incluido en el detalle columnas que no existen. " + ex);
        ...
        $.ajax({
            ...
            url: ROOT + 'Articulo/BatchUpdate',   // 'Factura/UpdateAwesomeGridData',
            dataType: 'json',
            ...
            success: function (result) {
                if (result) {
                    ...
                    window.location = (ROOT + "Articulo/index");
