        ...
        public virtual ActionResult DetObrasPolizas(string sidx, string sord, int? page, int? rows, int? IdObra)
        {
            ...
            var data = (from a in Det
                        ...
                        select new
                        {
                            ...
                            a.Observaciones
                        }).OrderBy(x => x.IdDetalleObraPoliza).Skip((currentPage - 1) * pageSize).Take(pageSize).ToList();
        ...
        public virtual ActionResult DetObrasEquiposInstalados(string sidx, string sord, int? page, int? rows, int? IdObra)
        {
            ...
            var data = (from a in Det
                        ...
                        select new
                        {
                            ...
                            a.Observaciones
                        }).OrderBy(x => x.IdDetalleObraEquipoInstalado).Skip((currentPage - 1) * pageSize).Take(pageSize).ToList();
