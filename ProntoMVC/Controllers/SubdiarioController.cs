  ...
  //  [Authorize(Roles = "Administrador,SuperAdmin,Compras")] //ojo que el web.config tambien te puede bochar hacia el login
    public partial class SubdiarioController : ProntoBaseController
    {
        ...
        void CargarViewBag(Pedido o)
        {
            ...
            try
            {
                ViewBag.CantidadAutorizaciones = db.Autorizaciones_TX_CantidadAutorizaciones((int)Pronto.ERP.Bll.EntidadManager.EnumFormularios.NotaPedido, (o == null) ? 0 : (o.TotalPedido * o.CotizacionMoneda), -1).Count();
            }
            catch (Exception e)
            {
                ErrHandler.WriteError(e);
        ...
        public virtual ActionResult Subdiarios(string sidx, string sord, int? page, int? rows, bool _search, string searchField, string searchOper, string searchString, string FechaInicial, string FechaFinal)
        {
            ...
            var Entidad1 = (from a in Entidad select new { Id = a.IdSubdiario });
            ...
            var jsonData = new jqGridJson()
            {
                ...
                rows = (from a in data
                        select new jqGridRowJson
                        {
                            ...
                            cell = new string[] { 
                                "<a href="+ Url.Action("Edit",new {id = a.IdSubdiario} ) + " target='' >Editar</>" ,
                                a.IdSubdiario.ToString(), 
                                a.IdCuentaSubdiario.NullSafeToString(), 
                                a.IdCuenta.NullSafeToString(), 
                                a.FechaComprobante.GetValueOrDefault().ToString("dd/MM/yyyy"),
