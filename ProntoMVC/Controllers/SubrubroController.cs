...
namespace ProntoMVC.Controllers
{
    public partial class SubrubroController : ProntoBaseController
    {
        ...
        public virtual ViewResult Index()
        {
            ...
            var Subrubros = db.Subrubros;
               // .Where(r => r.NumeroSubrubro > 0)
               // .OrderByDescending(r => r.IdSubrubro); // .NumeroSubrubro);
            ...
            return View(Subrubros.ToList());
        }

        //
        // GET: /Subrubro/Details/5
        ...
        private bool Validar(ProntoMVC.Data.Models.Subrubro o, ref string sErrorMsg)
        {
            ...
            //    // return false;
            //}
            //if (o.NumeroSubrubro == null) sErrorMsg += "\n" + "Falta el número de Subrubro";
        ...
        public virtual JsonResult BatchUpdate(Subrubro Subrubro)
        {
            try
            {
                ...
                if (ModelState.IsValid || true)
                {
                    ...
                    {
                        var originalSubrubro = db.Subrubros;
                            //.Where(p => p.IdSubrubro == Subrubro.IdSubrubro)
                            //.Include(p => p.DetalleSubrubros).SingleOrDefault();
                        var SubrubroEntry = db.Entry(originalSubrubro);
                        SubrubroEntry.CurrentValues.SetValues(Subrubro);

                        //foreach (var dr in Subrubro.DetalleSubrubros)
                        //{
                        //    var originalDetalleSubrubro = originalSubrubro.DetalleSubrubros.Where(c => c.IdDetalleSubrubro == dr.IdDetalleSubrubro).SingleOrDefault();
                        //    // Is original child item with same ID in DB?
                        //    if (originalDetalleSubrubro != null)
                        //    {
                        //        // Yes -> Update scalar properties of child item
                        //        //db.Entry(originalDetalleSubrubro).CurrentValues.SetValues(dr);
                        //        var DetalleSubrubroEntry = db.Entry(originalDetalleSubrubro);
                        //        DetalleSubrubroEntry.CurrentValues.SetValues(dr);
                        //    }
                        //    else
                        //    {
                        //        // No -> It's a new child item -> Insert
                        //        originalSubrubro.DetalleSubrubros.Add(dr);
                        //    }
                        //}
                        ...
                        //foreach (var originalDetalleSubrubro in originalSubrubro.DetalleSubrubros.Where(c => c.IdDetalleSubrubro != 0).ToList())
                        //{
                        //    // Are there child items in the DB which are NOT in the new child item collection anymore?
                        //    if (!Subrubro.DetalleSubrubros.Any(c => c.IdDetalleSubrubro == originalDetalleSubrubro.IdDetalleSubrubro))
                        //        // Yes -> It's a deleted child item -> Delete
                        //        originalSubrubro.DetalleSubrubros.Remove(originalDetalleSubrubro);
                        //    //db.DetalleSubrubros.Remove(originalDetalleSubrubro);
                        //}
                        //db.Entry(originalSubrubro).State = System.Data.Entity.EntityState.Modified;


                        //foreach (DetalleSubrubro dr in Subrubro.DetalleSubrubros)
                        //{
                        //    if (dr.IdDetalleSubrubro > 0)
                        //    {
                        ...
                        //        //db.DetalleSubrubros.Add(dr);
                        //    }
                        //}
                        //db.Entry(Subrubro).State = System.Data.Entity.EntityState.Modified;
                    }
                    //Perform Save
                    else
                    {
                        ...
                        //          where item.Letra == Subrubro.TipoABC && item.PuntoVenta == Subrubro.PuntoVenta
                        //             && item.IdTipoComprobante == (int)Pronto.ERP.Bll.EntidadManager.IdTipoComprobante.Subrubro
                        //          select item).SingleOrDefault();

                        //// var pv = db.PuntosVenta.Where(p => p.IdPuntoVenta == Subrubro.IdPuntoVenta).SingleOrDefault();
                        //pv.ProximoNumero += 1;
                    ...
                    //return RedirectToAction(( "Index",    "Subrubro");
                    //redirectUrl = Url.Action("Index", "Home"), 
                    //isRedirect = true 

                    return Json(new { Success = 1, IdSubrubro = Subrubro.IdSubrubro, ex = "" }); //, DetalleSubrubros = Subrubro.DetalleSubrubros
                }
                else
                {
                    ...
                    res.Message = "La Subrubro es inválida";
                    return Json(res);
        ...
        //[HttpPost]
        //public ActionResult Create(Subrubro Subrubro)
        //{
        ...
        //    ViewBag.IdObra = new SelectList(db.Obras, "IdObra", "NumeroObra", Subrubro.IdObra);
        //    ViewBag.Aprobo = new SelectList(db.Empleados, "IdEmpleado", "Nombre", Subrubro.Aprobo);
        //    ViewBag.IdSolicito = new SelectList(db.Empleados, "IdEmpleado", "Nombre", Subrubro.IdSolicito);
        //    ViewBag.IdSector = new SelectList(db.Sectores, "IdSector", "Descripcion", Subrubro.IdSector);
        //    return View(Subrubro);
        //}
        ...
        public virtual FileResult Imprimir(int id) //(int id)
        {
            ...
            string plantilla = AppDomain.CurrentDomain.BaseDirectory + "Documentos\\" + "SubrubroNET_Hawk.docx";
            /*
            string plantilla = Pronto.ERP.Bll.OpenXML_Pronto.CargarPlantillaDeSQL(OpenXML_Pronto.enumPlantilla.SubrubroA, SC);
            */
            ...
            //Pronto.ERP.BO.Subrubro fac = SubrubroManager.GetItem(SC, id, true);
            //OpenXML_Pronto.SubrubroXML_DOCX(output, fac, SC);

            //byte[] contents = ;
            //return File(contents, "application/octet-stream");

            byte[] contents = System.IO.File.ReadAllBytes(output);
            return File(contents, System.Net.Mime.MediaTypeNames.Application.Octet, "Subrubro.docx");
        }


        void inic(ref Subrubro o)
        {
            //o.PorcentajeIva1 = 21;                  //  mvarP_IVA1_Tomado
            //o.FechaSubrubro = DateTime.Now;
            ...
            //mvarAclaracionAlPie = .Fields("AclaracionAlPieDeSubrubro").Value
        ...
        public virtual ActionResult Edit(int id)
        {
            ...
            if (id <= 0)
            {
                ...
                //o.NumeroSubrubro = (int)Pronto.ERP.Bll.SubrubroManager.ProximoNumeroSubrubroPorIdCodigoIvaYNumeroDePuntoVenta(connectionString, 1, 6);
                //if (o.NumeroSubrubro == -1) o.NumeroSubrubro = null;
            ...
            {
                o = db.Subrubros
                     //   .Include(x => x.DetalleSubrubros)
                     // .Include(x => x.Cliente)
                      .SingleOrDefault(x => x.IdSubrubro == id);
            ...
            //ViewBag.IdSolicito = new SelectList(db.Empleados, "IdEmpleado", "Nombre", Subrubro.IdSolicito);
            //ViewBag.IdSector = new SelectList(db.Sectores, "IdSector", "Descripcion", Subrubro.IdSector);
            // Session.Add("Subrubro", o);
            return View(o);
        }

        void CargarViewBag(Subrubro  o)
        {
            ...
            //                                     where i.IdTipoComprobante == (int)Pronto.ERP.Bll.EntidadManager.IdTipoComprobante.Subrubro
        ...
        // POST: /Subrubro/Edit/5
        [HttpPost]
        public virtual ActionResult Edit(ProntoMVC.Data.Models.Subrubro Subrubro)
        {
            if (ModelState.IsValid)
            {
                db.Entry(Subrubro).State = System.Data.Entity.EntityState.Modified;
            ...
            //ViewBag.IdObra = new SelectList(db.Obras, "IdObra", "NumeroObra", Subrubro.IdObra);
            //ViewBag.IdCliente = new SelectList(db.Clientes, "IdCliente", "RazonSocial", Subrubro.IdCliente);
            //ViewBag.Aprobo = new SelectList(db.Empleados, "IdEmpleado", "Nombre", Subrubro.Aprobo);
            //ViewBag.IdSolicito = new SelectList(db.Empleados, "IdEmpleado", "Nombre", Subrubro.IdSolicito);
            //ViewBag.IdSector = new SelectList(db.Sectores, "IdSector", "Descripcion", Subrubro.IdSector);
            return View(Subrubro);
        ...
        public virtual ActionResult Subrubros(string sidx, string sord, int? page, int? rows, bool _search, string searchField, string searchOper, string searchString,
                                            string FechaInicial, string FechaFinal, string IdObra)
        {
            ...
            var Fac = db.Subrubros.AsQueryable();
            ...
            //    Fac = (from a in Fac where a.FechaSubrubro >= FechaDesde && a.FechaSubrubro <= FechaHasta select a).AsQueryable();
            //}
            if (_search)
            {
            ...
            var Req1 = (from a in Fac
                        select new
                        {
                            ...
                            //NumeroSubrubro = a.NumeroSubrubro,
                            //FechaSubrubro = a.FechaSubrubro,
            ...
            //    case "numeroSubrubro":
            //        if (sord.Equals("desc"))
            //            Fac = Fac.OrderByDescending(a => a.NumeroSubrubro);
            //        else
            //            Fac = Fac.OrderBy(a => a.NumeroSubrubro);
            //        break;
            //    case "fechaSubrubro":
            //        if (sord.Equals("desc"))
            //            Fac = Fac.OrderByDescending(a => a.FechaSubrubro);
            //        else
            //            Fac = Fac.OrderBy(a => a.FechaSubrubro);
            //        break;
            //    case "numeroobra":
            //        if (sord.Equals("desc"))
            ...
            var jsonData = new jqGridJson()
            {
                ...
                rows = (from a in data
                        select new jqGridRowJson
                        {
                            ...
                            //cell = new string[] { 
                            //    "<a href="+ Url.Action("Edit",new {id = a.IdSubrubro} )  +" target='_blank' >Editar</>" +
                            //    "|" +
                            //    "<a href=/Subrubro/Details/" + a.IdSubrubro + ">Detalles</a> ",
                            //    "<a href="+ Url.Action("Imprimir",new {id = a.IdSubrubro} )  +">Imprimir</>" ,
                            //    a.IdSubrubro.ToString(),
                            //    (a.TipoABC ?? string.Empty).ToString(),
                            //    (a.PuntoVenta ?? 0).ToString(),
                            //    (a.NumeroSubrubro ?? 0).ToString(),
                            //    (a.FechaSubrubro ?? DateTime.MinValue).ToString(),
                            //    //a.FechaSubrubro.ToString(),
        ...
        //[HttpPost]
        //public void EditGridData(int? IdSubrubro, int? NumeroItem, decimal? Cantidad, string Unidad, string Codigo, string Descripcion, string oper)
        //{
        //    switch (oper)
        //    {
        ...
        //[HttpPost]
        //public ActionResult Subrubros2(string sidx, string sord, int? page, int? rows, bool _search, string filters) //, string searchField, string searchOper, string searchString)
        //{
        //    var serializer = new JavaScriptSerializer();
        //    Filters f = (!_search || string.IsNullOrEmpty(filters)) ? null : serializer.Deserialize<Filters>(filters);
        //    ObjectQuery<Subrubro> filteredQuery = (f == null ? db.Subrubros : f.FilterObjectSet(db.Subrubros));
        ...
        //                        select new { item.IdSubrubro, item.FechaSubrubro, item.Detalle }).ToList();
        ...
        //                select new[] {
        //                                item.IdSubrubro.ToString(),
        //                                item.FechaSubrubro.ToString(),
        //                                item.Detalle
        //                            }).ToList()
        //    });
        //}
        ...
        public virtual ActionResult OrdenesCompraPendientesSubrubror(string sidx, string sord, int? page, int? rows,
        ...
        {
            Dictionary<int, string> subrubros = new Dictionary<int, string>();
            foreach (ProntoMVC.Data.Models.Subrubro u in db.Subrubros.OrderBy(x => x.Descripcion).ToList())
                subrubros.Add(u.IdSubrubro, u.Descripcion);

            return PartialView("Select", subrubros);
        }
