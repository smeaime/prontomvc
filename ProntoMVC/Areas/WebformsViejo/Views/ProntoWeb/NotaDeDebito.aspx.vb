...
Partial Class NotaDeDebitoABM
    ...
    Private IdNotaDeDebito As Integer = -1
    ...
    Public Property IdEntity() As Integer
        Get
            Return DirectCast(ViewState("IdNotaDeDebito"), Integer)
        End Get
        Set(ByVal Value As Integer)
            ViewState("IdNotaDeDebito") = Value
        End Set
    End Property
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not (Request.QueryString.Get("Id") Is Nothing) Then 'si trajo el parametro ID, lo guardo aparte
            IdNotaDeDebito = Convert.ToInt32(Request.QueryString.Get("Id"))
            Me.IdEntity = IdNotaDeDebito
        End If
        mKey = "NotaDeDebito_" & Me.IdEntity.ToString
        mAltaItem = False
        usuario = New Usuario
        ...
        If Not Page.IsPostBack Then
            ...
            'Carga del objeto
            TextBox1.Text = IdNotaDeDebito
            BindTypeDropDown()


            Dim myNotaDeDebito As Pronto.ERP.BO.NotaDeDebito
            If IdNotaDeDebito > 0 Then
                myNotaDeDebito = EditarSetup()
            ...
            Else
                If Request.QueryString.Get("CopiaDe") IsNot Nothing Then
                    ...
                    myNotaDeDebito = EditarSetup(Request.QueryString.Get("CopiaDe"))
                Else
                    myNotaDeDebito = AltaSetup()
                End If
            End If



            Me.ViewState.Add(mKey, myNotaDeDebito)
            ...
            BloqueosDeEdicion(myNotaDeDebito)
    ...
    Sub BloqueosDeEdicion(ByVal myNotaDeDebito As Pronto.ERP.BO.NotaDeDebito)
        '////////////////////////////////////////////
        '////////////////////////////////////////////
        'Bloqueos de edicion
        '////////////////////////////////////////////
        ...
        If ProntoFuncionesUIWeb.EstaEsteRol("Cliente") Then
            ...
            'encabezado
            'txtNumeroNotaDeDebito1.Enabled = False
            txtNumeroNotaDeDebito2.Enabled = False
    ...
    Function AltaSetup() As Pronto.ERP.BO.NotaDeDebito


        Dim myNotaDeDebito As Pronto.ERP.BO.NotaDeDebito = New Pronto.ERP.BO.NotaDeDebito
        With myNotaDeDebito
            .Id = -1
            ...
            'txtNumeroNotaDeDebito2.Text = ParametroOriginal(SC, "ProximoNotaDeDebito")
            txtFechaIngreso.Text = System.DateTime.Now.ToShortDateString() 'no se puede poner directamente Now?

            'txtNumeroNotaDeDebito1.Text = 1

            ''/////////////////////////////////
            ''/////////////////////////////////
            'agrego renglones vacios. Ver si vale la pena

            Dim mItem As NotaDeDebitoItem = New Pronto.ERP.BO.NotaDeDebitoItem
            mItem.Id = -1
            mItem.Nuevo = True
            ...
            'txtReferencia.Text = ParametroManager.TraerRenglonUnicoDeTablaParametroOriginal(SC).Item("ProximoNotaDeDebitoReferencia").ToString
            'txtFechaNotaDeDebito.Text = System.DateTime.Now.ToShortDateString()


            ViewState("PaginaTitulo") = "Nueva Nota de débito"
        End With
    ...
    Function EditarSetup(Optional ByVal CopiaDeOtroId As Long = -1) As Pronto.ERP.BO.NotaDeDebito
        ...
        If CopiaDeOtroId = -1 Then 'no debería hacer lo de la copia en el Alta en lugar de acá?
            myNotaDeDebito = NotaDeDebitoManager.GetItem(SC, IdNotaDeDebito, True) 'va a editar ese ID
            'myNotaDeDebito = NotaDeDebitoManager.GetItemComPronto(SC, IdNotaDeDebito, True)
        Else
            'está haciendo un alta, pero uso los datos de un ID existente
            myNotaDeDebito = NotaDeDebitoManager.GetCopyOfItem(SC, CopiaDeOtroId)
            IdNotaDeDebito = -1
            'tomar el ultimo de la serie y sumarle uno


            'myNotaDeDebito.SubNumero = NotaDeDebitoManager.ProximoSubNumero(SC, myNotaDeDebito.Numero)

            'limpiar los precios del NotaDeDebito original
            For Each i In myNotaDeDebito.Detalles
                i.ImporteTotalItem = 0
            Next

            'mKey = "NotaDeDebito_" & Me.IdEntity.ToString 'esto es un balurdo. aclarar bien
        End If


        If Not (myNotaDeDebito Is Nothing) Then
            With myNotaDeDebito


                RecargarEncabezado(myNotaDeDebito)

                'TraerDatosCliente(myNotaDeDebito.IdCliente)
                ...
                'Me.Title = "Edición Fondo Fijo " + myNotaDeDebito.Letra + myNotaDeDebito.NumeroComprobante1.ToString + myNotaDeDebito.NumeroComprobante2.ToString
                ViewState("PaginaTitulo") = "Edición Nota de débito " + myNotaDeDebito.Numero.ToString + "/" '+ myNotaDeDebito.SubNumero.ToString
                '/////////////////////////
                '/////////////////////////
        ...
        Else
            MsgBoxAjax(Me, "El comprobante con ID " & IdNotaDeDebito & " no se pudo cargar. Consulte al Administrador")
        End If

        Return myNotaDeDebito
    End Function

    Sub RecargarEncabezado(ByVal myNotaDeDebito As Pronto.ERP.BO.NotaDeDebito)
        With myNotaDeDebito
            'txtReferencia.Text = myNotaDeDebito.Referencia
            'txtLetra.Text = myNotaDeDebito.Letra
            'txtNumeroNotaDeDebito1.Text = .SubNumero
            txtNumeroNotaDeDebito2.Text = .Numero
            ...
            'SelectedReceiver.Value = myNotaDeDebito.IdProveedor
            'txtDescProveedor.Text = myNotaDeDebito.Proveedor
            'TraerDatosProveedor(myNotaDeDebito.IdProveedor) 'guarda con esto, que pisa combos editables (moneda y condicioncompra). Es decir, esta funcion la debo llamar antes que los BuscaIDComboxxxx
            '////////////////////////////////////////////////////////
            ...
            ProntoCheckSINO(.AplicarEnCtaCte, chkAplicarAccDeudores)

            BuscaIDEnCombo(cmbCategoriaIIBB1, .IdIBCondicion)
            BuscaIDEnCombo(cmbCategoriaIIBB2, .IdIBCondicion2)
            BuscaIDEnCombo(cmbCategoriaIIBB3, .IdIBCondicion3)
            ...
            RadioButtonListEsInterna.SelectedValue = IIf(.CtaCte = "SI", 1, 2)
            ...
            'If myNotaDeDebito.Aprobo <> 0 Then
            '    txtLibero.Text = EmpleadoManager.GetItem(SC, myNotaDeDebito.Aprobo).Nombre
            'End If
    ...
    Private Sub BindTypeDropDown()
        ...
        IniciaCombo(SC, cmbCategoriaIIBB2, tipos.IBCondiciones)
        IniciaCombo(SC, cmbCategoriaIIBB3, tipos.IBCondiciones)
    ...
    Sub Rebind()
        Dim myNotaDeDebito As NotaDeDebito = CType(Me.ViewState(mKey), NotaDeDebito)
        'NotaDeDebitoManager.RecalcularTotales(HFSC.Value, myNotaDeDebito)
        RecalcularTotalComprobante()

        GridView1.DataSource = myNotaDeDebito.Detalles 'este bind lo copié
        GridView1.DataBind()             'este bind lo copié   

        GridView1.FooterRow.Cells(3).Text = myNotaDeDebito.SubTotal
    ...
    Sub RefrescarTalonariosDisponibles()
        cmbPuntoVenta.DataSource = EntidadManager.GetListTX(HFSC.Value, "PuntosVenta", "TX_PuntosVentaPorIdTipoComprobanteLetra", EntidadManager.IdTipoComprobante.NotaDebito, lblLetra.Text.ToUpper)
        ...
        If cmbPuntoVenta.Items.Count = 0 Then
            MsgBoxAjax(Me, "No hay talonario para nota de debito")
        Else
            RefrescarNumeroTalonario()
        End If
    End Sub
    ...
    Sub RefrescarNumeroTalonario() 'esto tambien podría estar la mitad en el manager
        ...
        Else
            txtNumeroNotaDeDebito2.Text = NotaDeDebitoManager.ProximoNumeroNotaDebitoPorIdCodigoIvaYNumeroDePuntoVenta(SC, cmbCondicionIVA.SelectedValue, cmbPuntoVenta.SelectedItem.Text) 'ParametroOriginal(SC, "ProximoFactura")
        End If
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            If mOk Then
                ...
                Dim myNotaDeDebito As Pronto.ERP.BO.NotaDeDebito = CType(Me.ViewState(mKey), Pronto.ERP.BO.NotaDeDebito)

                DePaginaHaciaObjeto(myNotaDeDebito)
                ...
                If NotaDeDebitoManager.IsValid(myNotaDeDebito) Then
                    Try
                        If NotaDeDebitoManager.Save(SC, myNotaDeDebito) = -1 Then
                    ...
                    'esto es lo que está mal. no tenés que aumentarlo si es una edicion! -pero si está idNotaDeDebito=-1! sí, pero puedo ser una alta a partir de una copia
                    ...
                    'If IdNotaDeDebito = -1 And myNotaDeDebito.SubNumero = 1 Then 'es un NotaDeDebito nuevo y TAMBIEN empieza el subnumero
                    '    If GrabarRenglonUnicoDeTablaParametroOriginal(SC, "ProximoNotaDeDebito", TraerRenglonUnicoDeTablaParametroOriginal(SC).Item("ProximoNotaDeDebito") + 1) = -1 Then
                    ...
                    IdNotaDeDebito = myNotaDeDebito.Id

                    If myNotaDeDebito.Numero <> StringToDecimal(txtNumeroNotaDeDebito2.Text) Then
                        EndEditing("La comparativa fue grabada con el número " & myNotaDeDebito.Numero) 'me voy 
                    Else
    ...
    Private Sub EndEditing(Optional ByVal MensajeFinal As String = "")
        ...
        If False Then 'If False Then 'If MensajeFinal <> "" Then
        ...
        Else
            ...
            Response.Redirect(String.Format("NotaDeDebitos.aspx"))
        End If
    End Sub
    ...
    Protected Sub ButVolver_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButVolver.Click
        ...
        Response.Redirect(String.Format("NotaDeDebitos.aspx?Imprimir=" & IdNotaDeDebito))
    End Sub
    ...
    Protected Sub ButVolverSinImprimir_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButVolverSinImprimir.Click
        Response.Redirect(String.Format("NotaDeDebitos.aspx")) 'roundtrip al cuete?
    End Sub
    ...
    Protected Sub LinkButton1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LinkAgregarRenglon.Click
        ...
        ViewState("IdDetalleNotaDeDebito") = -1
        ...
        Try
            If e.CommandName.ToLower = "eliminar" Then
                If (Me.ViewState(mKey) IsNot Nothing) Then
                    myNotaDeDebito = CType(Me.ViewState(mKey), Pronto.ERP.BO.NotaDeDebito)

                    'si esta eliminado, lo restaura
                    myNotaDeDebito.Detalles(mIdItem).Eliminado = Not myNotaDeDebito.Detalles(mIdItem).Eliminado
                    RecalcularTotalComprobante()

                    Me.ViewState.Add(mKey, myNotaDeDebito)
                    GridView1.DataSource = myNotaDeDebito.Detalles
                    GridView1.DataBind()
                End If

            ElseIf e.CommandName.ToLower = "editar" Then
                ViewState("IdDetalleNotaDeDebito") = mIdItem
                If (Me.ViewState(mKey) IsNot Nothing) Then
                    'MostrarElementos(True)
                    myNotaDeDebito = CType(Me.ViewState(mKey), Pronto.ERP.BO.NotaDeDebito)
                    With myNotaDeDebito.Detalles(mIdItem)
                        ...
                        '////////////////////////////////////////////////////////////////////////////////
                        'HAY QUE ARREGLAR ESTO: me lo tiene que dar directamente el BO.NotaDeDebito
                        'txtDescProveedor = myNotaDeDebito.Detalles(mIdItem).descripcion
                        ...
                        'txtObservacionesItem.Text = myNotaDeDebito.Detalles(mIdItem).Observaciones.ToString
                        'txtFechaNecesidad.Text = myNotaDeDebito.Detalles(mIdItem).FechaEntrega.ToString
                        'If myNotaDeDebito.Detalles(mIdItem).OrigenDescripcion = 1 Then
                        '    RadioButtonList1.Items(0).Selected = True
                        'ElseIf myNotaDeDebito.Detalles(mIdItem).OrigenDescripcion = 2 Then
                        '    RadioButtonList1.Items(1).Selected = True
                        'ElseIf myNotaDeDebito.Detalles(mIdItem).OrigenDescripcion = 3 Then
    ...
    Protected Sub btnSaveItem_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSaveItem.Click
        ...
        If (Me.ViewState(mKey) IsNot Nothing) Then
            Dim mIdItem As Integer = DirectCast(ViewState("IdDetalleNotaDeDebito"), Integer)
            Dim myNotaDeDebito As Pronto.ERP.BO.NotaDeDebito = CType(Me.ViewState(mKey), Pronto.ERP.BO.NotaDeDebito)
            ...
            If mIdItem = -1 Then 'si es nuevo, inicializo las cosas el item
                ...
                If myNotaDeDebito.Detalles Is Nothing Then 'no debiera ser null si es una edicion, pero...
                    MsgBoxAjax(Me, "Está editando pero el comprobante no tiene detalle. Hay algo mal")
                    Exit Sub
                End If

                mItem.Id = myNotaDeDebito.Detalles.Count
                mItem.Nuevo = True
                mIdItem = mItem.Id
                myNotaDeDebito.Detalles.Add(mItem)
            End If
  ...
  Handles txt_AC_Articulo.TextChanged
    ...
    Sub DePaginaHaciaObjeto(ByRef myNotaDeDebito As Pronto.ERP.BO.NotaDeDebito)
        ...
        With myNotaDeDebito
            ...
            '.IdMoneda = drParam.Item("ProximoNotaDeDebitoReferencia").ToString 'mIdMonedaPesos
            .IdMoneda = cmbMoneda.SelectedValue

            '.SubNumero = StringToDecimal(txtNumeroNotaDeDebito1.Text)
            .TipoABC = txtLetra.Text

            'Qué nombre estandar defino para estas propiedades genericas?
            .Numero = StringToDecimal(txtNumeroNotaDeDebito2.Text)
            '.NumeroNotaDebito = StringToDecimal(txtNumeroNotaDeDebito2.Text)
            .FechaIngreso = txtFechaIngreso.Text
            .Fecha = txtFechaIngreso.Text
            '.FechaNotaDebito = txtFechaIngreso.Text



            '.IdPuntoVenta = cmbPuntoVenta.SelectedValue
            '.IdPuntoVenta = NotaDeDebitoManager.IdPuntoVentaComprobanteNotaDebitoSegunSubnumeroYLetra(SC, cmbPuntoVenta.Text, txtLetra.Text)
            ...
            ProntoCheckSINO(chkAplicarAccDeudores, .AplicarEnCtaCte)
            .CtaCte = IIf(RadioButtonListEsInterna.SelectedItem.Value = 2, "NO", "SI")


            .IdIBCondicion = cmbCategoriaIIBB1.SelectedValue
            .IdIBCondicion2 = cmbCategoriaIIBB2.SelectedValue
            .IdIBCondicion3 = cmbCategoriaIIBB3.SelectedValue
            .NumeroCertificadoPercepcionIIBB = Val(txtNumeroCertificadoIIBB.Text)


            .IdVendedor = cmbVendedor.SelectedValue
            ...
            'myNotaDeDebito.Observaciones = Convert.ToString(txtObservaciones.Text) '???
            .Observaciones = txtObservaciones.Text
    ...
    Sub RecalcularTotalComprobante()
        Dim myNotaDeDebito As Pronto.ERP.BO.NotaDeDebito = CType(Me.ViewState(mKey), Pronto.ERP.BO.NotaDeDebito)
        Try
            'myNotaDeDebito.Bonificacion = StringToDecimal(txtTotBonif.Text)
            myNotaDeDebito.PorcentajeIva1 = IIf(lblLetra.Text = "A", 21, 0) 'HORROR!
            DePaginaHaciaObjeto(myNotaDeDebito)
    ...
    '    'Dim myNotaDeDebito As Pronto.ERP.BO.NotaDeDebito = CType(Me.ViewState(mKey), Pronto.ERP.BO.NotaDeDebito)
    '    'myNotaDeDebito.ConfirmadoPorWeb = "SI"
    ...
    '    Dim myNotaDeDebito As Pronto.ERP.BO.NotaDeDebito = CType(Me.ViewState(mKey), Pronto.ERP.BO.NotaDeDebito)
    '    If Not NotaDeDebitoManager.IsValid(myNotaDeDebito) Then 'hago la validacion manualmente porque no me está andando el CustomValidator que controla la grilla (aunque sí se dispara si aprieto "AgrgarItem", probablemente por el UpdatePanel
    '        mOk = False
    ...
    '        'MsgBoxAjax(Me, "El objeto no es válido")
    '    End If
    '    'myNotaDeDebito.Aprobo = "SI" 'este es cuando lo aprueba el usario pronto
    'End Sub
    ...
    Protected Sub RadioButtonPendientes_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles RadioButtonPendientes.CheckedChanged
        ...
        'NotaDeDebitos_TX_Pendientes1 'P' 
    End Sub
    ...
    Protected Sub RadioButtonAlaFirma_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles RadioButtonAlaFirma.CheckedChanged
        'NotaDeDebitos_TX_PendientesDeFirma
    End Sub
    ...
    Protected Sub LinkButton1_Click1(ByVal sender As Object, ByVal e As System.EventArgs) Handles LinkButton1.Click
        ...
        'ObjGrillaConsulta.TypeName = "Pronto.ERP.Bll.NotaDeDebitoManager"
        ...
        '            oRs = Aplicacion.NotaDeDebitos.TraerFiltrado("_PendientesDeFirma")
        '        ElseIf mOrigen = "Compras" Then
        'oRs = Aplicacion.NotaDeDebitos.TraerFiltrado("_Pendientes1", mTiposComprobante)
        '        Else
        'oRs = Aplicacion.NotaDeDebitos.TraerFiltrado("_PendientesPlaneamiento", mTiposComprobante)
        '        End If
        'Lista.DataSource = oRs
        '    Case 1
        'lblLabels(5).Visible = False
        ''         rchObservaciones.Visible = False
        'oRs = Aplicacion.NotaDeDebitos.TraerFiltrado("_PendientesPorRM1", mTiposComprobante)
        'Lista.DataSource = oRs
        '    Case 2
    ...
    Protected Sub GridView3_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles GridView3.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            ObjectDataSource3.SelectParameters(1).DefaultValue = DataBinder.Eval(e.Row.DataItem, "IdNotaDeDebito")
            Try
    ...
    Protected Sub Button3_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button3.Click
        ...
        Response.Redirect(String.Format("NotaDeDebito.aspx?Id=-1&CopiaDe=" & IdSeleccionado))
    ...
    Protected Sub txtNumeroNotaDeDebito2_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtNumeroNotaDeDebito2.TextChanged
        'txtNumeroNotaDeDebito1.Text = NotaDeDebitoManager.ProximoSubNumero(SC, txtNumeroNotaDeDebito2.Text)
    End Sub
    ...
    Protected Sub LinkImprimir_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LinkImprimir.Click
        ...
        Dim mPrinter = ""
        Dim mCopias = 1
        'output = ImprimirWordDOT("NotaDeDebito_" & session(SESSIONPRONTO_NombreEmpresa) & ".dot", Me, SC, Session, Response, IdNotaDeDebito)
        Dim mvaragrupar = 0 '1 agrupa, <>1 no agrupa
        Dim p = DirApp() & "\Documentos\" & "NotaDebito1_Hawk.dot"
        output = ImprimirWordDOT(p, Me, SC, Session, Response, IdNotaDeDebito, mvarClausula, mPrinter, mCopias, System.IO.Path.GetTempPath & "NotaDebito.doc")
        Try
    ...
    Protected Sub btnAnularOk_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAnularOk.Click
        ...
        If bPassOK Then
            ...
            With myNotaDeDebito
                ...
                DeObjetoHaciaPagina(myNotaDeDebito)

            End With


            Me.ViewState.Add(mKey, myNotaDeDebito) 'guardo en el viewstate el objeto
            NotaDeDebitoManager.Anular(SC, IdNotaDeDebito, cmbUsuarioAnulo.SelectedValue, txtAnularMotivo.Text)
    ...
    Sub DeObjetoHaciaPagina(ByVal myNotaDeDebito As Pronto.ERP.BO.NotaDeDebito)
        RecargarEncabezado(myNotaDeDebito)
    ...
    Protected Sub cmbCategoriaIIBB1_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbCategoriaIIBB1.SelectedIndexChanged
        RecalcularTotalComprobante()
    End Sub
    Protected Sub cmbCategoriaIIBB2_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbCategoriaIIBB2.SelectedIndexChanged
        RecalcularTotalComprobante()
    End Sub
    Protected Sub cmbCategoriaIIBB3_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbCategoriaIIBB3.SelectedIndexChanged
        RecalcularTotalComprobante()
    End Sub
End Class
