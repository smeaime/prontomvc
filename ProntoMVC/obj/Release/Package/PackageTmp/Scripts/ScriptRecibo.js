...
function calculateItem() {
    ...
    var pi = parseFloat($("#PorcentajeIva1").val());  // parseFloat($("#PorcentajeIva").val()); para el porcentaje de iva, usamos el global de la Recibo
    ...
    //////////////////////////////////////////////////////////////////////
    // TOTAL ITEM
    // en Recibos, el total del item se pone sin el iva (por lo menos el que se muestra en pantalla) 
    //-efectivamente... esta columna solo es de la grilla: la tabla detRecibos no desnormaliza en una columna 
...
$(document).ready(function () {
    ...
    $('#Lista').jqGrid({
        url: ROOT + 'Recibo/DetRecibos/',
        postData: { 'IdRecibo': function () { return $("#IdRecibo").val(); } },
        editurl: ROOT + 'Recibo/EditGridData/',
        datatype: 'json',
        mtype: 'POST',
        colNames: ['Imputaciones', 'Fecha', 'Importe', 'Saldo', 'NumeroItem', 'Codigo', 'Descripcion', 'Cantidad', 'Un.',
        ...
        colModel: [
                        ...
                        { name: 'IdDetalleRecibo', index: 'IdDetalleRecibo', label: 'TB', align: 'left', width: 85, editable: true, hidden: true, editoptions: { disabled: 'disabled' }, editrules: { edithidden: false, required: true} },
        ...
        //{ name: 'Eliminado', index: 'Eliminado', label:'TB', align: 'left', width: 85, editable: true, hidden: true, editoptions: { disabled: 'disabled' }, editrules: { edithidden: true} },
                        ...
                        { name: 'Codigo', index: 'Codigo', align: 'left', width: 50, editable: true, edittype: 'text',
                            editoptions: {
                        ...
                        { name: 'Descripcion', index: 'Descripcion', align: 'left', width: 300, editable: true, edittype: 'text',
                            editoptions: {
                                dataInit: function (elem) {
                                    $(elem).autocomplete({
                        ...
                        editoptions: {
                          ...
                          {name: 'PrecioUnitario', index: 'PrecioUnitario', label: 'TB', align: 'right', width: 60, editable: true, edittype: 'text', editoptions: { maxlength: 20, dataEvents: [{ type: 'change', fn: function (e) { calculateItem(); } }] }, editrules: { required: true} }, //, unformat:numUnformat
    ...
    $("#addData").click(function () {
        jQuery("#Lista").jqGrid('editGridRow', "new", { addCaption: "Agregar registro", bSubmit: "Aceptar", bCancel: "Cancelar", width: 800, reloadAfterSubmit: false, closeOnEscape: true, closeAfterAdd: true,
            ...
            //beforeShowForm: function (form) { $('#tr_IdDetalleRecibo', form).hide(); }
        });
    });
    ...
    $("#ListaDrag").jqGrid({
        url: ROOT + 'Recibo/OrdenesCompraPendientesRecibor',
        ...
        caption: '<b>Ordenes de compra pendientes de Recibor</b>',
        autowidth: true,
    ...
    $("#ListaDrag2").jqGrid({
        url: ROOT + 'Recibo/RemitosPendienteRecibocion',
        ...
        altRows: false,
        emptyrecords: 'No hay registros para mostrar',
        caption: '<b>Remitos pendientes de Recibor</b>',
        gridComplete: function () {
    ...
    $("#grabar2").click(function () {
        ...
        // como no le encontre la vuelta a automatizar la serializacion de inputs disableados, los pongo manualmente
        // como no le encontre la vuelta a automatizar la serializacion de inputs disableados, los pongo manualmente
        cabecera.NumeroRecibo = $("#NumeroRecibo").val();
        ...
        var items = [];
        cabecera.DetalleRecibos = [];  // el array no existe en el objeto cabecera, lo anexo a lo bestia (ojo que, en verdad, sí hay uno, porque lo creé con HiddenFor)
        var dataIds = $('#Lista').jqGrid('getDataIDs');
        for (var i = 0; i < dataIds.length; i++) {
            try {
        ...
        // cabecera.DetalleRecibos.push(items);
        ...
        $.ajax({
            ...
            url: ROOT + 'Recibo/BatchUpdate',   // 'Recibo/UpdateAwesomeGridData',
            dataType: 'json',
    ...
    $('#ListaDocumentos').jqGrid({
        ...
        colNames: ['Asiento contable', 'Debe', 'Haber'],
        colModel: [
                        { name: 'act', index: 'act', align: 'left', width: 70, sortable: false, editable: false, hidden: true },
    ...
    $('#ListaUnidades').jqGrid({
        ...
        colNames: [' Valores', 'n° int', 'n° cheque', 'Importe', 'Vence'],
        colModel: [
                        { name: 'act', index: 'act', align: 'left', width: 70, sortable: false, editable: false, hidden: true },
