...
Begin VB.Form frmComprobantesPrv 
   ...
   End
   Begin MSComCtl2.DTPicker DTFields 
      DataField       =   "FechaPrestacionServicio"
      Height          =   330
      Index           =   5
      Left            =   11115
      TabIndex        =   161
...
Private mvarGrabado As Boolean, mSenialIVA As Boolean, mCondicionDesdePedido As Boolean, mvarModoCodigoBarra As Boolean, mvarLeyoCodigoBarra As Boolean, mvarPrimeraVez As Boolean
Private mvarGrabacionAutomatica As Boolean, mActivarCamposParaFF As Boolean, mActivarCamposParaFF1 As Boolean, mTomarCuentaDePresupuesto As Boolean
Private mvarTipoABC As String, mvarExigirCAI As String, mCadena As String, mvarConfirmado As String, mvarExigirCAIComp As String, mvarAnticipo_O_Devolucion As String
Private mvarDebitoAutomaticoPorDefecto As String
Private mvarP_IVA1 As Double, mvarP_IVA2 As Double, mvarIVA1 As Double, mvarIVA2 As Double, mvarIVANoDiscriminado As Double, mvarDecimales As Double, mvarCotizacion As Double
Private mvarTotalComprobanteProveedor As Double, mvarSubTotal As Double, mvarCotizacionEuro As Double, mvarAjusteIVA As Double, mvarCotizacionDolar As Double, TotalAnterior As Double
Private mvarIVAs(10, 1) As Double
Private mvarPorcentajeAnticipo As Single
Private mvarTipoIVA As Integer, mvarIdProveedor As Integer, mvarIdTipoCuentaGrupoIVA As Integer, mvarIdMonedaDolar As Integer, mvarIdMonedaPesos As Integer, mvarIdMonedaOP_FF As Integer
Private mvarIdTipoComprobanteNDInternaAcreedores As Integer, mvarIdTipoComprobanteNCInternaAcreedores As Integer, mNivelAcceso As Integer, mvarTipoProveedor As Integer
   ...
   With oF
      ...
      If .Aceptado Then
         ...
         With oL
            ...
            .SubItems(14) = "" & oF.DataCombo1(16).Text
         End With
...
Private Sub Check2_Click()
   ...
   If Check2.Value = 0 Then RecargarCuentas
...
Private Sub cmd_Click(Index As Integer)
   ...
   Select Case Index
      Case 0
         ...
         Dim mvarNumero As Long, mvarRegistros As Long, mvarIdOriginal As Long, mIdProv As Long, mIdImpuestoDirecto As Long
         Dim mvarConfirma As Integer, mvarSeguro As Integer, i As Integer
         Dim mvarMonedaOP_FF As String, mvarError As String, mvarError2 As String, mAuxS1 As String, mAuxS2 As String, mAuxS3 As String, mADistribuir As String, mJerarquia As String
         Dim mInscriptoRegistroFiscalOperadoresGranos As String
         ...
         Dim mOk As Boolean, mvarImputarAPresupuestoDeObra1 As Boolean, mvarImputarAPresupuestoDeObra2 As Boolean
         Dim oRs As ADOR.Recordset
         ...
         If mvarId > 0 And Option1.Value Then
            If TotalAnterior <> mvarTotalComprobanteProveedor Then
               mvarSeguro = MsgBox("El importe total del comprobante a cambiado, esta seguro de continuar?", vbYesNo, "Comprobante de proveedores")
            ...
            If oRs.RecordCount > 0 Then
               If oRs.Fields("ImporteTotal").Value <> oRs.Fields("Saldo").Value Then mOk = False
            End If
            oRs.Close
            Set oRs = Nothing
            If Not mOk Then
               mvarSeguro = MsgBox("El comprobante se encuentra parcial o totalmente cancelado en cta. cte., esta seguro de continuar?", vbYesNo, "Comprobante de proveedores")
               If mvarSeguro = vbNo Then Exit Sub
            End If
         End If
         
         If txtLetra.Text <> "C" And _
               Not (dcfields(1).BoundText = mvarIdTipoComprobanteNDInternaAcreedores Or dcfields(1).BoundText = mvarIdTipoComprobanteNCInternaAcreedores) And mvarTipoProveedor <> 3 Then
         ...
         If Val(txtNumeroComprobante1.Text) = 0 Then
            If BuscarClaveINI("Exigir punto de venta en comprobantes de proveedores") = "SI" Then
               ...
               Exit Sub
            Else
               mvarSeguro = MsgBox("El punto de venta del comprobante es 0," & vbCrLf & "esta seguro de continuar?", vbYesNo, "Comprobante de proveedores")
               If mvarSeguro = vbNo Then Exit Sub
            End If
         End If
         ...
         With origen.Registro
            ...
            If DTFields(5).Visible And IsNull(.Fields(DTFields(5).DataField).Value) And BuscarClaveINI("Control de carga de fecha de prestacion de servicio en comprobante de proveedores", -1) = "SI" Then
               MsgBox "Debe ingresar la fecha de prestacion de servicio", vbExclamation
               Exit Sub
            End If
            ...
            .Fields("DebitoAutomatico").Value = Null
            If mvarDebitoAutomaticoPorDefecto = "SI" Then
               mvarSeguro = MsgBox("El comprobante se cancela por debito automatico?", vbYesNo, "Debito automatico")
               If mvarSeguro = vbYes Then .Fields("DebitoAutomatico").Value = "SI"
            End If
         End With
         
         If IsNumeric(dcfields(0).BoundText) Then
            Set oRs = Aplicacion.ComprobantesProveedores.TraerFiltrado("_PorNumeroComprobante", _
                        Array(dcfields(0).BoundText, txtLetra.Text, txtNumeroComprobante1.Text, txtNumeroComprobante2.Text, mvarId, dcfields(1).BoundText))
            ...
            If BuscarClaveINI("Controlar fecha de comprobantes de proveedores") = "SI" Then
               Set oRs = Aplicacion.ComprobantesProveedores.TraerFiltrado("_ComprobanteAnterior", _
                           Array(dcfields(0).BoundText, txtLetra.Text, txtNumeroComprobante1.Text, txtNumeroComprobante2.Text, dcfields(1).BoundText))
               If oRs.RecordCount > 0 Then
         ...
         If Not IsNull(origen.Registro.Fields("IdProveedorEventual").Value) Then
            Set oRs = Aplicacion.ComprobantesProveedores.TraerFiltrado("_PorNumeroComprobante", _
                        Array(origen.Registro.Fields("IdProveedorEventual").Value, txtLetra.Text, txtNumeroComprobante1.Text, txtNumeroComprobante2.Text, mvarId, dcfields(1).BoundText))
            If Not oRs Is Nothing Then
               If oRs.RecordCount > 0 Then
                  oRs.Close
                  Set oRs = Nothing
...
'         End If
         ...
         If Not IsNull(origen.Registro.Fields("IdProveedor").Value) Then
            ...
            If oRs.RecordCount > 0 Then
               mInscriptoRegistroFiscalOperadoresGranos = IIf(IsNull(oRs.Fields("InscriptoRegistroFiscalOperadoresGranos").Value), "NO", oRs.Fields("InscriptoRegistroFiscalOperadoresGranos").Value)
                     ...
                     If Not .Fields("Eliminado").Value Then
                        If mvarControlarRubrosContablesEnCP = "SI" Then
                           If IsNull(.Fields("IdRubroContable").Value) Or .Fields("IdRubroContable").Value = 0 Then
                              mvarError = mvarError & vbCrLf & "El item " & IIf(IsNull(.Fields("Item").Value), "", .Fields("Item").Value) & " " & "no tiene ingresado el rubro financiero"
                           End If
                        End If
                        If IsNull(.Fields("IdCuenta").Value) Or .Fields("IdCuenta").Value = 0 Then
                           mvarError = mvarError & vbCrLf & "El item " & IIf(IsNull(.Fields("Item").Value), "", .Fields("Item").Value) & " " & "no tiene ingresada la cuenta contable"
                        End If
                        If mAuxS1 = "SI" And mAuxS2 = "SI" And IsNull(.Fields("IdPresupuestoObrasNodo").Value) Then
                           mvarImputarAPresupuestoDeObra1 = True
                           mvarImputarAPresupuestoDeObra2 = True
                           
                           Set oRs1 = Aplicacion.Cuentas.TraerFiltrado("_PorId", IIf(IsNull(.Fields("IdCuenta").Value), 0, .Fields("IdCuenta").Value))
                           mJerarquia = ""
                           If oRs1.RecordCount > 0 Then
                              If IIf(IsNull(oRs1.Fields("ImputarAPresupuestoDeObra").Value), "NO", oRs1.Fields("ImputarAPresupuestoDeObra").Value) = "NO" Then
                                 mvarImputarAPresupuestoDeObra1 = False
                              End If
                           End If
                           oRs1.Close
                           ...
                           If Not IsNull(.Fields("IdArticulo").Value) Then
                              ...
                              If oRs1.RecordCount > 0 Then
                                 If IIf(IsNull(oRs1.Fields("RegistrarStock").Value), "SI", oRs1.Fields("RegistrarStock").Value) = "SI" Then
                                    mvarImputarAPresupuestoDeObra2 = False
                                 End If
                              End If
                              oRs1.Close
                           End If
                           
                           If mvarImputarAPresupuestoDeObra1 And mvarImputarAPresupuestoDeObra2 And Not IsNull(.Fields("IdObra").Value) Then
                              ...
                              If oRs1.RecordCount > 0 Then
                                 If mTomarCuentaDePresupuesto Then
                                    mvarError = mvarError & "El item " & IIf(IsNull(.Fields("Item").Value), "", .Fields("Item").Value) & " " & "no tiene asignado presupuesto de obra" & vbCrLf
                                 Else
                                    mvarError2 = mvarError2 & IIf(IsNull(.Fields("Item").Value), "", .Fields("Item").Value) & "  "
                                 End If
                              End If
                              oRs1.Close
                           End If
...
'                                 If oRs1.RecordCount > 0 Then
'                                    mvarError = mvarError & vbCrLf & "El item " & IIf(IsNull(.Fields("Item").Value), "", .Fields("Item").Value) & " " & "no tiene ingresada la etapa"
'                                 End If
'                                 oRs1.Close
'                              End If
...
'                                 Set oRs1 = Aplicacion.Obras.TraerFiltrado("_DestinosParaComboPorIdObra", .Fields("IdObra").Value)
'                                 If oRs1.RecordCount > 0 Then
'                                    mvarError = mvarError & vbCrLf & "El item " & IIf(IsNull(.Fields("Item").Value), "", .Fields("Item").Value) & " " & "no tiene ingresado el rubro para presupuesto de obra"
'                                 End If
'                                 oRs1.Close
'                              End If
                        End If
                        If mPorcentajeTolerancia > 0 Then
                           ...
                           If mImporteItem <> 0 Then
                              If Abs((mImporte - mImporteItem) / mImporteItem * 100) > mPorcentajeTolerancia Then
                                 mvarError = mvarError & vbCrLf & "El item " & IIf(IsNull(.Fields("Item").Value), "", .Fields("Item").Value) & " " & _
                        ...
                        If glbModeloContableSinApertura = "SI" And IsNull(.Fields("IdObra").Value) Then
                           ...
                           If mId(mJerarquia, 1, 1) = "5" Then
                              mvarError = mvarError & vbCrLf & "El item " & IIf(IsNull(.Fields("Item").Value), "", .Fields("Item").Value) & " " & "no tiene ingresada la obra/centro de costos"
                           End If
                        End If
                     End If
                     .MoveNext
         ...
         If Len(mvarError) > 0 Then
            ...
            Exit Sub
         End If
         If Len(mvarError2) > 0 Then
            mvarSeguro = MsgBox("Cuidado, los items : " & mvarError2 & vbCrLf & "No tienen asignados etapa de obra, esta seguro de continuar ?", vbYesNo, "Presupuesto de obra")
            If mvarSeguro = vbNo Then Exit Sub
         End If
         ...
         If mvarTipoIVA = 1 Then
            ...
            If txtLetra.Text = "C" And BuscarClaveINI("Desactivar control letra comprobante proveedor") <> "SI" Then
               ...
               Exit Sub
            End If
            If glbActividadComercializacionGranos Then
               Set oF = New frm_Aux
               With oF
                  .Caption = "Retencion IVA"
                  .Text1.Visible = False
                  With .Label1
                     .Caption = "Indique tipo de retencion IVA s/pago :"
                     .Width = .Width * 2
                  End With
                  With .dcfields(0)
                     .Top = oF.Text1.Top
                     ...
                     Set .RowSource = Aplicacion.ImpuestosDirectos.TraerFiltrado("_PorTipoParaCombo", Array("RETENCION IVA", mInscriptoRegistroFiscalOperadoresGranos))
                     If Not IsNull(origen.Registro.Fields("IdImpuestoDirecto").Value) Then .BoundText = origen.Registro.Fields("IdImpuestoDirecto").Value
                     .Visible = True
                  End With
                  .Width = .Width * 1.7
                  .Height = .Height * 0.7
                  .cmd(0).Top = .Label2(1).Top
                  .cmd(0).Left = .Width / 2 - (.cmd(0).Width / 2)
                  .cmd(0).Height = .cmd(0).Height * 0.75
                  .cmd(1).Visible = False
                  .Show vbModal, Me
                  If .Ok And IsNumeric(.dcfields(0).BoundText) Then mIdImpuestoDirecto = .dcfields(0).BoundText
               End With
               Unload oF
               Set oF = Nothing
               If mIdImpuestoDirecto = 0 Then
                  MsgBox "Debe indicar el tipo de retencion IVA a practicar", vbExclamation
                  Exit Sub
               End If
            End If
         ElseIf mvarTipoIVA = 6 Then
            If BuscarClaveINI("Activar control por tope de monotributistas", -1) = "SI" Then
...
'         VerificarProvinciasDestino
         ...
         With origen.Registro
            ...
            End If
            If mIdImpuestoDirecto > 0 Then
               .Fields("IdImpuestoDirecto").Value = mIdImpuestoDirecto
            Else
               .Fields("IdImpuestoDirecto").Value = Null
            End If
         End With
...
'         End If
      ...
      Case 7
         MostrarPolizas
         If Not IsNull(origen.Registro.Fields("IdPoliza").Value) Then
            cmd(7).BackColor = &HC0FFC0
         Else
            cmd(7).BackColor = &HC0C0FF
         End If
   
      Case 8
         If IsNull(origen.Registro.Fields("IdLiquidacionFlete").Value) Then
            MsgBox "No hay liquidacion de flete activa, arrastre una sobre el comprobante", vbExclamation
            Exit Sub
         End If
         VerLiquidacionFlete
   End Select
...
'   End If
   ...
   mAux1 = TraerValorParametro2("TomarCuentaDePresupuestoEnComprobantesProveedores")
   If Not IsNull(mAux1) And mAux1 = "SI" Then mTomarCuentaDePresupuesto = True
   ...
   If mvarId = -1 Then
      ...
      For Each dtf In DTFields
         If dtf.Index <> 5 Then dtf.Value = Date
      Next
   ...
   Else
      ...
      With origen.Registro
         ...
         If Not IsNull(.Fields("IdLiquidacionFlete").Value) Then
            cmd(8).BackColor = &HC0FFC0
         Else
            cmd(8).BackColor = &HC0C0FF
         End If
         TotalAnterior = .Fields("TotalComprobante").Value
      
         For Each oL In Lista.ListItems
            If Not IsNull(origen.DetComprobantesProveedores.Item(oL.Tag).Registro.Fields("IdUsuarioModificoCuenta").Value) Then
               oL.ForeColor = vbRed
               For i = 0 To oL.ListSubItems.Count
                  oL.ListSubItems(i).ForeColor = vbRed
               Next
            End If
         Next
      End With
...
Private Sub DataCombo1_Change(Index As Integer)
   ...
   Select Case Index
      Case 0
         If IsNumeric(DataCombo1(Index).BoundText) And DataCombo1(Index).BoundText <> DataCombo1(Index).Text Then
      ...
      Case 2
         If DataCombo1(Index).Text <> DataCombo1(Index).BoundText Then
            ...
            If Me.Visible Then RecargarCuentas
         End If
...
Private Sub dcfields_Validate(Index As Integer, Cancel As Boolean)
   ...
   If Index = 2 And IsNumeric(dcfields(Index).BoundText) Then origen.Registro.Fields(dcfields(Index).DataField).Value = dcfields(Index).BoundText
...
Private Sub DTFields_Change(Index As Integer)
   ...
   If Index = 1 Then
      ...
      If mvarId <= 0 Then
         ...
         End If
      End If
   ElseIf Index = 0 And Option3.Value And Me.Visible And DTFields(4).Visible Then
      DTFields(4).Value = DTFields(Index).Value
   ElseIf Index = 5 And Option4.Value And Me.Visible And DTFields(4).Visible Then
      If Not IsNull(DTFields(Index).Value) Then DTFields(4).Value = DTFields(Index).Value
   End If
...
Private Sub DTFields_Click(Index As Integer)
   ...
   If Index = 1 Then
      ...
      End If
   ElseIf Index = 5 Then
      If IsNull(origen.Registro.Fields(DTFields(5).DataField).Value) Then
         origen.Registro.Fields(DTFields(5).DataField).Value = Date
         DTFields(5).Value = Date
         DTFields(4).Value = Date
      End If
   End If
...
Private Sub Form_Activate()
   ...
   If Not IsNull(origen.Registro.Fields("Confirmado").Value) And origen.Registro.Fields("Confirmado").Value = "NO" Then
      Dim oRs As ADOR.Recordset
   ...
   If mvarId > 0 And Me.NivelAcceso = Alto And mvarTotalComprobanteProveedor = 0 And (DTFields(1).Value > gblFechaUltimoCierre Or AccesoHabilitado(Me.OpcionesAcceso, DTFields(1).Value)) Then
      cmd(4).Enabled = True
   Else
      cmd(4).Enabled = False
      If mvarPrimeraVez And glbIdCuentaFFUsuario <> -1 And IsNull(origen.Registro.Fields("IdProveedorEventual").Value) Then OtrosProveedores
   End If
      ...
      If Len(mCadena) > 0 And Len(mCadena) <= 40 Then
         ...
         If oRs.RecordCount > 0 Then
            With origen.Registro
               ...
               .Fields("FechaVencimientoCAI").Value = DateSerial(CInt(mId(mCadena, 32, 4)), CInt(mId(mCadena, 36, 2)), CInt(mId(mCadena, 38, 2)))
...
'   End If
   ...
   If Data.GetFormat(ccCFText) Then
      ...
      ElseIf InStr(1, Columnas(LBound(Columnas) + 1), "Codigo") <> 0 And InStr(1, Columnas(LBound(Columnas) + 3), "Proveedor") <> 0 Then
         If UBound(Filas) > 1 Then
            MsgBox "No puede copiar mas de un comprobante a la vez!", vbCritical
            Exit Sub
         End If
...
'            Next
      ...
      ElseIf InStr(1, Columnas(LBound(Columnas) + 1), "Transportista") <> 0 Then
         If UBound(Filas) > 1 Then
            MsgBox "No puede copiar mas de un comprobante a la vez!", vbCritical
            Exit Sub
         End If
         ...
         IncorporarDesdeLiquidacionFlete Columnas(2), mError
...
Private Sub MostrarDatos(Index As Integer)
   ...
   Dim mvarLocalidad As Integer, mvarProvincia As Integer, mvarIBCondicion As Integer, mvarIdIBCondicion As Integer, mvarIGCondicion As Integer
   Dim mvarIdImpuestoDirectoSUSS As Integer, mvarIdTipoRetencionGanancia As Integer, mvarIdCondicionCompra As Integer
   Dim mIdProv As Long
   ...
   With oRs
      ...
      If mvarId <= 0 And Not IsNull(.Fields("FechaLimiteCondicionIVA").Value) And .Fields("FechaLimiteCondicionIVA").Value <= Date Then
      ...
      mvarDebitoAutomaticoPorDefecto = IIf(IsNull(.Fields("DebitoAutomaticoPorDefecto").Value), "", .Fields("DebitoAutomaticoPorDefecto").Value)
      ...
      If mvarIBCondicion = 2 Or mvarIBCondicion = 3 Then
         If mvarIdIBCondicion <> 0 And (IsNull(origen.Registro.Fields(dcfields(4).DataField).Value) Or mvarId <= 0) Then
      ...
      If mvarIGCondicion = 2 Or mvarIGCondicion = 3 Then
         If mvarIdTipoRetencionGanancia <> 0 And (IsNull(origen.Registro.Fields(dcfields(6).DataField).Value) Or mvarId <= 0) Then
...
'      End If
      ...
      If (mvarId <= 0 Or IsNull(origen.Registro.Fields(dcfields(7).DataField).Value)) And Not mCondicionDesdePedido And mvarIdCondicionCompra <> 0 Then
      ...
      If Not IsNull(.Fields("FechaLimiteExentoGanancias").Value) Then
         If .Fields("FechaLimiteExentoGanancias").Value < DTFields(0).Value Then
            MsgBox "El proveedor tiene vencida su condicion de exento" & vbCrLf & _
                     "al impuesto a las ganancias (" & .Fields("FechaLimiteExentoGanancias").Value & "), revise la condicion", vbInformation
         End If
      End If
      If Not IsNull(.Fields("FechaLimiteExentoIIBB").Value) Then
         If .Fields("FechaLimiteExentoIIBB").Value < DTFields(0).Value Then
            MsgBox "El proveedor tiene vencida su condicion de exento" & vbCrLf & _
                     "al impuesto a los ingresos brutos (" & .Fields("FechaLimiteExentoGanancias").Value & "), revise la condicion", vbInformation
         End If
      End If
   ...
   If mvarId <= 0 And mIdProv <> 0 And Not mvarLeyoCodigoBarra Then
      ...
      If oRs.RecordCount > 0 Then
         With origen.Registro
            If IsNull(oRs.Fields("NumeroCAE").Value) And Not IsNull(oRs.Fields("NumeroCAI").Value) Then 'And IsNull(.Fields("NumeroCAI").Value) Then
               .Fields("NumeroCAI").Value = oRs.Fields("NumeroCAI").Value
               If Not IsNull(oRs.Fields("FechaVencimientoCAI").Value) And (IsNull(.Fields("FechaVencimientoCAI").Value) Or .Fields("FechaVencimientoCAI").Value = Date) Then
                  .Fields("FechaVencimientoCAI").Value = oRs.Fields("FechaVencimientoCAI").Value
...
Private Sub Option4_Click()
   ...
   If Option4.Value Then
      With Label3
         .Top = cmdImportaciones.Top ' txtCodigoPostal.Top + txtCodigoPostal.Height + 80
         .Left = cmdImportaciones.Left + cmdImportaciones.Width + 10
         .Visible = True
      End With
      With DTFields(5)
         .Top = Label3.Top
         .Left = Label3.Left + Label3.Width + 5
         .Visible = True
      End With
   End If
...
Private Sub txtImputaComprobante_Validate(Cancel As Boolean)
   ...
   If Len(txtImputaComprobante.Text) <> 0 Then
      If IsNumeric(txtImputaComprobante.Text) And IsNumeric(dcfields(0).BoundText) Then
         ...
         Set oRs = Nothing
      Else
         MsgBox "El campo debe ser numerico y debe tener ingresado el proveedor"
...
Public Sub MostrarPolizas()
   ...
   With oF
      ...
      End If
      If Not IsNull(origen.Registro.Fields("IdPoliza").Value) Then
         .IdParametro2 = origen.Registro.Fields("IdPoliza").Value
      Else
         .IdParametro2 = -1
      End If
      If Not IsNull(origen.Registro.Fields("NumeroCuotaPoliza").Value) Then
         .IdParametro3 = origen.Registro.Fields("NumeroCuotaPoliza").Value
      Else
         .IdParametro3 = -1
      End If
      .Id = 17
      .Show vbModal, Me
      If .IdParametro2 > 0 Then
         origen.Registro.Fields("IdPoliza").Value = .IdParametro2
      Else
         origen.Registro.Fields("IdPoliza").Value = Null
      End If
      If .IdParametro3 > 0 Then
         origen.Registro.Fields("NumeroCuotaPoliza").Value = .IdParametro3
      Else
         origen.Registro.Fields("NumeroCuotaPoliza").Value = Null
      End If
   End With
   Set oF = Nothing
...
Public Sub EliminarComprobante()
   ...
   If mvarId <= 0 Then
      MsgBox "El comprobante debe estar previamente grabado para poder eliminarlo", vbExclamation
      Exit Sub
   End If
   ...
   If Option1.Value Then
      ...
      If oRs.RecordCount > 0 Then
         If oRs.Fields("ImporteTotal").Value <> oRs.Fields("Saldo").Value Then
            oRs.Close
            Set oRs = Nothing
            MsgBox "El comprobante se encuentra parcial o totalmente cancelado en cta. cte." & vbCrLf & "Eliminacion abortada", vbExclamation
            Exit Sub
         End If
      Else
         oRs.Close
...
Public Sub IncorporarDesdeLiquidacionFlete(ByVal mIdLiquidacionFlete As Long, ByRef mError As String)

   Dim oPr As ComPronto.ComprobanteProveedor
   Dim oRs As ADOR.Recordset
   Dim oRs1 As ADOR.Recordset
   Dim mIdCodigoIva As Integer
   Dim iFilas As Long, iColumnas As Long, mIdCuentaIvaCompras1 As Long, mIdTipoComprobante As Long, i As Long, j As Long, mIdCuentaLiquidacionFletes As Long
   Dim mCodigoCuenta As Long, mIdCuentaIvaCompras(10) As Long
   Dim mIVAComprasPorcentaje1 As Single, mIVAComprasPorcentaje(10) As Single
   Dim mvarImporte As Double
   Dim s As String, mvarConIVA As String, mAplicaIVA As String, mAuxS1 As String
   Dim mProcesar As Boolean, mOk As Boolean
   Dim mAux1
   ...
   mAux1 = TraerValorParametro2("IdCuentaLiquidacionFletes")
   mIdCuentaLiquidacionFletes = IIf(IsNull(mAux1), 0, mAux1)
   If mIdCuentaLiquidacionFletes = 0 Then
         mError = mError & "No existe en parametros la cuenta liquidacion fletes, ingresela" & vbCrLf
         mProcesar = False
   End If
   
   Set oRs = Aplicacion.Cuentas.TraerFiltrado("_PorId", mIdCuentaLiquidacionFletes)
   mCodigoCuenta = 0
   If oRs.RecordCount > 0 Then mCodigoCuenta = IIf(IsNull(oRs.Fields("Codigo").Value), 0, oRs.Fields("Codigo").Value)
   oRs.Close
   
   Set oRs = Aplicacion.LiquidacionesFletes.TraerFiltrado("_PorIdConDatos", mIdLiquidacionFlete)
   
   With origen.Registro
      If Not IsNull(.Fields("IdProveedor").Value) And Not IsNull(oRs.Fields("IdProveedor").Value) And .Fields("IdProveedor").Value <> oRs.Fields("IdProveedor").Value And Lista.ListItems.Count > 0 Then
         mError = mError & "La liquidacion no se tomo porque no es del proveedor actual." & vbCrLf
         mProcesar = False
      Else
         If Not IsNull(oRs.Fields("IdProveedor").Value) Then .Fields("IdProveedor").Value = oRs.Fields("IdProveedor").Value
      End If
      If Not IsNull(oRs.Fields("NumeroReferencia").Value) Then
         mError = mError & "La liquidacion ya esta ingresada en el comprobante " & oRs.Fields("NumeroReferencia").Value & vbCrLf
         mProcesar = False
      End If
      .Fields("IdProveedor").Value = oRs.Fields("IdProveedor").Value
   End With
   ...
   If mProcesar Then
      ...
      mIdCodigoIva = 0
      If Not IsNull(origen.Registro.Fields("IdProveedor").Value) Then
         Set oRs1 = Aplicacion.Proveedores.TraerFiltrado("_PorId", origen.Registro.Fields("IdProveedor").Value)
         If oRs1.RecordCount > 0 Then
            If Not IsNull(oRs1.Fields("IdCodigoIva").Value) And _
                  (oRs1.Fields("IdCodigoIva").Value = 3 Or oRs1.Fields("IdCodigoIva").Value = 5 Or oRs1.Fields("IdCodigoIva").Value = 8 Or oRs1.Fields("IdCodigoIva").Value = 9) Then
               mAplicaIVA = "NO"
      ...
      mIVAComprasPorcentaje1 = 0
      For i = 1 To 10
         If mIVAComprasPorcentaje(i) = mvarP_IVA1 Then
            mIdCuentaIvaCompras1 = mIdCuentaIvaCompras(i)
            mIVAComprasPorcentaje1 = mIVAComprasPorcentaje(i)
            Exit For
         End If
      ...
      mvarImporte = IIf(IsNull(oRs.Fields("SubtotalGravado").Value), 0, oRs.Fields("SubtotalGravado").Value)
      If mvarImporte <> 0 Then
         With origen.DetComprobantesProveedores.Item(-1)
            With .Registro
               .Fields("IdCuenta").Value = mIdCuentaLiquidacionFletes
      ...
      mvarImporte = IIf(IsNull(oRs.Fields("SubtotalNoGravado").Value), 0, oRs.Fields("SubtotalNoGravado").Value)
      If mvarImporte <> 0 Then
         With origen.DetComprobantesProveedores.Item(-1)
            With .Registro
               .Fields("IdCuenta").Value = mIdCuentaLiquidacionFletes
      ...
      With origen.Registro
         .Fields("IdLiquidacionFlete").Value = mIdLiquidacionFlete
      End With
      cmd(8).BackColor = &HC0FFC0
   End If
   oRs.Close
...
Public Sub VerLiquidacionFlete()

   Dim oF As frmLiquidacionesFletes
   Set oF = New frmLiquidacionesFletes
   With oF
      .InhabilitarEdicion = True
      .Id = origen.Registro.Fields("IdLiquidacionFlete").Value
      .Show vbModal, Me
   End With
   Unload oF
   Set oF = Nothing
...
Public Sub RecargarCuentas()

   Dim mIdCuenta As Long
   Dim mAuxS1 As String
   
   DataCombo1(0).BoundText = ""
   DataCombo1(1).BoundText = ""
   ...
   With DataCombo1(0)
      ...
      Else
         ...
         Else
            If IsNumeric(DataCombo1(2).BoundText) Then
               Set .RowSource = Aplicacion.Cuentas.TraerFiltrado("_PorFechaParaCombo", Array(DTFields(0).Value, DataCombo1(2).BoundText, "SI", "NO", "NO"))
            Else
               Set .RowSource = Aplicacion.Cuentas.TraerFiltrado("_PorFechaParaCombo", Array(DTFields(0).Value, -1, "SI", "NO", "SI"))
            End If
         End If
      End If
   
      If mIdCuenta > 0 Then origen.Registro.Fields(.DataField).Value = mIdCuenta
      .Enabled = True
   End With
   With DataCombo1(3)
      .BoundText = ""
