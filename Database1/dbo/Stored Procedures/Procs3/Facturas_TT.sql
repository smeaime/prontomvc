CREATE  Procedure [dbo].[Facturas_TT]
...
/*  CURSOR  */
DECLARE @IdFactura int, @OCompra varchar(8), @Corte int, @P varchar(2000)
SET @Corte=0
SET @P=''
   ...
   BEGIN
	IF @Corte<>@IdFactura
	   BEGIN
		IF @Corte<>0
			UPDATE #Auxiliar3
			SET Remitos = SUBSTRING(@P,1,2000)
			WHERE IdFactura=@Corte
		SET @P=''
		SET @Corte=@IdFactura
	   END
...
SET @vector_T='00912F111F055BB544444425126703433533400'
...
SELECT 
 ...
 Facturas.ImporteTotal-Facturas.ImporteIva1-Facturas.ImporteIva2-Facturas.RetencionIBrutos1-Facturas.RetencionIBrutos2-Facturas.RetencionIBrutos3+
	IsNull(Facturas.ImporteBonificacion,0)-IsNull(Facturas.IvaNoDiscriminado,0)-IsNull(Facturas.PercepcionIVA,0)-IsNull(Facturas.OtrasPercepciones1,0)-
	IsNull(Facturas.OtrasPercepciones2,0)-IsNull(Facturas.OtrasPercepciones3,0) as [Subtotal],
