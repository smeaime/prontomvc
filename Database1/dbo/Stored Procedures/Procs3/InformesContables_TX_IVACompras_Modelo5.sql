CREATE PROCEDURE [dbo].[InformesContables_TX_IVACompras_Modelo5]
...
-- ESTE ES IGUAL AL STANDAR CON EL AGREGADO DE DISCRIMINAR EL IVA BIENES DE USO AL FINAL DEL INFORME
-- INDICANDO LOS CODIGOS DE CUENTA CONTABLE DESDE HASTA PARA BIENES DE USO
...
DECLARE @IdTipoCuentaGrupoIVA int, @IvaCompras_DesglosarNOGRAVADOS varchar(2), @IdCuentaImpuestosInternos int, @AgenteRetencionIVA varchar(2), @IdComprobanteAEmitir int, 
		@CuentasBienesUsoDesde int, @CuentasBienesUsoHasta int, @IdCtaAdicCol1 int,@IdCtaAdicCol2 int,@IdCtaAdicCol3 int,@IdCtaAdicCol4 int, @IdCtaAdicCol5 int,
...
SET @CuentasBienesUsoDesde=IsNull((Select Top 1 Valor From Parametros2 Where Campo='CuentasBienesUsoDesde'),0)
SET @CuentasBienesUsoHasta=IsNull((Select Top 1 Valor From Parametros2 Where Campo='CuentasBienesUsoHasta'),0)
...
INSERT INTO #Auxiliar1 
 ...
 SELECT 
	...
	Case When Cuentas.Codigo between @CuentasBienesUsoDesde and @CuentasBienesUsoHasta
		Then Round(dcp.ImporteIVA1*cp.CotizacionMoneda,4) Else 0 End
 FROM DetalleComprobantesProveedores dcp 
 LEFT OUTER JOIN ComprobantesProveedores cp ON dcp.IdComprobanteProveedor = cp.IdComprobanteProveedor
 LEFT OUTER JOIN Proveedores ON cp.IdProveedor = Proveedores.IdProveedor
 LEFT OUTER JOIN TiposComprobante ON  cp.IdTipoComprobante = TiposComprobante.IdTipoComprobante
 ...
 SELECT 
	...
	dcp.Importe*cp.CotizacionMoneda,
	Case When Cuentas.Codigo between @CuentasBienesUsoDesde and @CuentasBienesUsoHasta
		Then Round(dcp.ImporteIVA2*cp.CotizacionMoneda,4) Else 0 End
 FROM DetalleComprobantesProveedores dcp 
 LEFT OUTER JOIN ComprobantesProveedores cp ON dcp.IdComprobanteProveedor = cp.IdComprobanteProveedor
 LEFT OUTER JOIN Proveedores ON cp.IdProveedor = Proveedores.IdProveedor
 LEFT OUTER JOIN TiposComprobante ON  cp.IdTipoComprobante = TiposComprobante.IdTipoComprobante
 ...
 SELECT 
	...
	0,
	dcp.Importe*cp.CotizacionMoneda,	Case When Cuentas.Codigo between @CuentasBienesUsoDesde and @CuentasBienesUsoHasta
		Then Round(dcp.ImporteIVA3*cp.CotizacionMoneda,4) Else 0 End
 FROM DetalleComprobantesProveedores dcp 
 LEFT OUTER JOIN ComprobantesProveedores cp ON dcp.IdComprobanteProveedor = cp.IdComprobanteProveedor
 LEFT OUTER JOIN Proveedores ON cp.IdProveedor = Proveedores.IdProveedor
 LEFT OUTER JOIN TiposComprobante ON  cp.IdTipoComprobante = TiposComprobante.IdTipoComprobante
 ...
 SELECT 
	...
	dcp.Importe*cp.CotizacionMoneda,
	Case When Cuentas.Codigo between @CuentasBienesUsoDesde and @CuentasBienesUsoHasta
		Then Round(dcp.ImporteIVA4*cp.CotizacionMoneda,4) Else 0 End
 FROM DetalleComprobantesProveedores dcp 
 LEFT OUTER JOIN ComprobantesProveedores cp ON dcp.IdComprobanteProveedor = cp.IdComprobanteProveedor
 LEFT OUTER JOIN Proveedores ON cp.IdProveedor = Proveedores.IdProveedor
 LEFT OUTER JOIN TiposComprobante ON  cp.IdTipoComprobante = TiposComprobante.IdTipoComprobante
 ...
 SELECT 
	...
	dcp.Importe*cp.CotizacionMoneda,
	Case When Cuentas.Codigo between @CuentasBienesUsoDesde and @CuentasBienesUsoHasta
		Then Round(dcp.ImporteIVA5*cp.CotizacionMoneda,4) Else 0 End
 FROM DetalleComprobantesProveedores dcp 
 LEFT OUTER JOIN ComprobantesProveedores cp ON dcp.IdComprobanteProveedor = cp.IdComprobanteProveedor
 LEFT OUTER JOIN Proveedores ON cp.IdProveedor = Proveedores.IdProveedor
 LEFT OUTER JOIN TiposComprobante ON  cp.IdTipoComprobante = TiposComprobante.IdTipoComprobante
 ...
 SELECT 
	...
	dcp.Importe*cp.CotizacionMoneda,
	Case When Cuentas.Codigo between @CuentasBienesUsoDesde and @CuentasBienesUsoHasta
		Then Round(dcp.ImporteIVA6*cp.CotizacionMoneda,4) Else 0 End
 FROM DetalleComprobantesProveedores dcp 
 LEFT OUTER JOIN ComprobantesProveedores cp ON dcp.IdComprobanteProveedor = cp.IdComprobanteProveedor
 LEFT OUTER JOIN Proveedores ON cp.IdProveedor = Proveedores.IdProveedor
 LEFT OUTER JOIN TiposComprobante ON  cp.IdTipoComprobante = TiposComprobante.IdTipoComprobante
 ...
 SELECT 
	...
	dcp.Importe*cp.CotizacionMoneda,
	Case When Cuentas.Codigo between @CuentasBienesUsoDesde and @CuentasBienesUsoHasta
		Then Round(dcp.ImporteIVA7*cp.CotizacionMoneda,4) Else 0 End
 FROM DetalleComprobantesProveedores dcp 
 LEFT OUTER JOIN ComprobantesProveedores cp ON dcp.IdComprobanteProveedor = cp.IdComprobanteProveedor
 LEFT OUTER JOIN Proveedores ON cp.IdProveedor = Proveedores.IdProveedor
 LEFT OUTER JOIN TiposComprobante ON  cp.IdTipoComprobante = TiposComprobante.IdTipoComprobante
 ...
 SELECT 
	...
	dcp.Importe*cp.CotizacionMoneda,
	Case When Cuentas.Codigo between @CuentasBienesUsoDesde and @CuentasBienesUsoHasta
		Then Round(dcp.ImporteIVA8*cp.CotizacionMoneda,4) Else 0 End
 FROM DetalleComprobantesProveedores dcp 
 LEFT OUTER JOIN ComprobantesProveedores cp ON dcp.IdComprobanteProveedor = cp.IdComprobanteProveedor
 LEFT OUTER JOIN Proveedores ON cp.IdProveedor = Proveedores.IdProveedor
 LEFT OUTER JOIN TiposComprobante ON  cp.IdTipoComprobante = TiposComprobante.IdTipoComprobante
 ...
 SELECT 
	...
	dcp.Importe*cp.CotizacionMoneda,
	Case When Cuentas.Codigo between @CuentasBienesUsoDesde and @CuentasBienesUsoHasta
		Then Round(dcp.ImporteIVA9*cp.CotizacionMoneda,4) Else 0 End
 FROM DetalleComprobantesProveedores dcp 
 LEFT OUTER JOIN ComprobantesProveedores cp ON dcp.IdComprobanteProveedor = cp.IdComprobanteProveedor
 LEFT OUTER JOIN Proveedores ON cp.IdProveedor = Proveedores.IdProveedor
 LEFT OUTER JOIN TiposComprobante ON  cp.IdTipoComprobante = TiposComprobante.IdTipoComprobante
 ...
 SELECT 
	...
	dcp.Importe*cp.CotizacionMoneda,
	Case When Cuentas.Codigo between @CuentasBienesUsoDesde and @CuentasBienesUsoHasta
		Then Round(dcp.ImporteIVA10*cp.CotizacionMoneda,4) Else 0 End
 FROM DetalleComprobantesProveedores dcp 
 LEFT OUTER JOIN ComprobantesProveedores cp ON dcp.IdComprobanteProveedor = cp.IdComprobanteProveedor
 LEFT OUTER JOIN Proveedores ON cp.IdProveedor = Proveedores.IdProveedor
 LEFT OUTER JOIN TiposComprobante ON  cp.IdTipoComprobante = TiposComprobante.IdTipoComprobante
 ...
 WHERE (TiposComprobante.VaAlLibro='SI' or TiposComprobante.VaAlLibro is null) and 
	...
	(dcp.IdCuenta<>@IdCtaAdicCol1 and dcp.IdCuenta<>@IdCtaAdicCol2 and dcp.IdCuenta<>@IdCtaAdicCol3 and 
	 dcp.IdCuenta<>@IdCtaAdicCol4 and dcp.IdCuenta<>@IdCtaAdicCol5) and 
	(@IdComprobanteAEmitir=-1 or cp.IdComprobanteProveedor=@IdComprobanteAEmitir) and 
		Not (cp.Letra='B' and IsNull(Proveedores.ResolucionAfip3668,'')='SI')
 ...
 WHERE (TiposComprobante.VaAlLibro='SI' or TiposComprobante.VaAlLibro is null) and 
	...
	(Cuentas.IdTipoCuentaGrupo is not null and Cuentas.IdTipoCuentaGrupo=@IdTipoCuentaGrupoIVA) and 
	IsNull(cp.Confirmado,'')<>'NO' and 
	(dcp.IdCuenta=@IdCtaAdicCol1 or dcp.IdCuenta=@IdCtaAdicCol2 or dcp.IdCuenta=@IdCtaAdicCol3 or 
	 dcp.IdCuenta=@IdCtaAdicCol4 or dcp.IdCuenta=@IdCtaAdicCol5) and 
...
INSERT INTO #Auxiliar2 
 SELECT 
	...
	0,
	SUM(#Auxiliar1.A_IVA_BienesUso*#Auxiliar1.A_Coeficiente)
 FROM #Auxiliar1
 GROUP BY #Auxiliar1.A_IdComprobante,#Auxiliar1.A_Tasa,#Auxiliar1.A_MostrarTodo
	...
	Case When #Auxiliar2.A_MostrarTodo=1 
		Then 	Case 	When cp.IdProveedor is not null Then Proveedores.RazonSocial
				When cp.IdProveedorEventual is not null 
				 Then (Select Top 1 P.RazonSocial From Proveedores P Where P.IdProveedor=cp.IdProveedorEventual)
				When cp.IdCuentaOtros is not null 
				 Then (Select Top 1 C.Descripcion From Cuentas C Where C.IdCuenta=cp.IdCuentaOtros)
				Else Null
			End
		Else Null 
	End,
...
--		Else Null--	End,
	...
	Case When #Auxiliar2.A_MostrarTodo=1 
		Then Round(cp.TotalComprobante*cp.CotizacionMoneda*TiposComprobante.Coeficiente-0.000001,2)
		Else Null
	End,
	...
	Round(#Auxiliar2.A_IVA_BienesUso-0.000001,2)
 FROM #Auxiliar2
...
UNION ALL 
 ...
 SELECT 
	...
	'S/D',
	das.Cuit,
	Case When das.IdCuenta<>@IdCtaAdicCol1 and das.IdCuenta<>@IdCtaAdicCol2 and das.IdCuenta<>@IdCtaAdicCol3 and 
			das.IdCuenta<>@IdCtaAdicCol4 and das.IdCuenta<>@IdCtaAdicCol5 and 
		...
		Then (Select Sum(IsNull(DetalleAsientos.Debe,0)-IsNull(DetalleAsientos.Haber,0)) 
			...
			Where DetalleAsientos.IdAsiento=das.IdAsiento and DetalleAsientos.NumeroComprobante=das.NumeroComprobante and 
				DetalleAsientos.FechaComprobante=das.FechaComprobante and DetalleAsientos.TipoImporte='N')
		Else 0
	End,
	0,
	Case When das.IdCuenta<>@IdCtaAdicCol1 and das.IdCuenta<>@IdCtaAdicCol2 and das.IdCuenta<>@IdCtaAdicCol3 and 
	...
	End,
	Case When das.IdCuenta<>@IdCtaAdicCol1 and das.IdCuenta<>@IdCtaAdicCol2 and das.IdCuenta<>@IdCtaAdicCol3 and 
			das.IdCuenta<>@IdCtaAdicCol4 and das.IdCuenta<>@IdCtaAdicCol5 
		Then (Select Sum(IsNull(DetalleAsientos.Debe,0)-IsNull(DetalleAsientos.Haber,0)) 
			From DetalleAsientos 
			Where DetalleAsientos.IdAsiento=das.IdAsiento and DetalleAsientos.NumeroComprobante=das.NumeroComprobante and 
				DetalleAsientos.FechaComprobante=das.FechaComprobante and DetalleAsientos.TipoImporte='G')
		Else 0
	End,
 ...
 SELECT 
	...
	Case When Valores.IdCuentaContable<>@IdCtaAdicCol1 and Valores.IdCuentaContable<>@IdCtaAdicCol2 and 
		...
		Then 	Case When Iva=0 
				Then Round((Valores.Importe * TiposComprobante.Coeficiente * IsNull(Valores.CotizacionMoneda,1))-0.000001,2)
				Else 0 
			End
		Else 0
	End,
	...
	Case When Valores.IdCuentaContable<>@IdCtaAdicCol1 and Valores.IdCuentaContable<>@IdCtaAdicCol2 and 
		...
		Then 	Case When Iva=0 
				Then 0 
				Else Round(((Valores.Importe-Valores.Iva) * TiposComprobante.Coeficiente * IsNull(Valores.CotizacionMoneda,1))-0.000001,2) 
			End
		Else 0
	End,
...
INSERT INTO #Auxiliar7 
 SELECT #Auxiliar.A_IdComprobante, Sum(IsNull(#Auxiliar.A_NetoNoGravado,0)), 
	...
	Sum(IsNull(#Auxiliar.A_CtaAdicCol4,0)), Sum(IsNull(#Auxiliar.A_CtaAdicCol5,0)), 
	0
 FROM #Auxiliar
...
UPDATE #Auxiliar7
SET A_Diferencia=(A_Total-(A_NetoNoGravado+A_NetoNoGravadoComprobantesC+A_NetoNoGravadoImpInt+A_NetoGravado+A_IVA+A_CtaAdicCol1+
				A_CtaAdicCol2+A_CtaAdicCol3+A_CtaAdicCol4+A_CtaAdicCol5))

UPDATE #Auxiliar
SET A_NetoGravado=A_NetoGravado+IsNull((Select Top 1 A_Diferencia From #Auxiliar7 
					Where #Auxiliar.A_IdComprobante=#Auxiliar7.A_IdComprobante and A_Diferencia<>0),0)
WHERE IsNull(#Auxiliar.A_NetoGravado,0)<>0

UPDATE #Auxiliar2
SET A_NetoGravado=A_NetoGravado+IsNull((Select Top 1 A_Diferencia From #Auxiliar7 
					Where #Auxiliar2.A_IdComprobante=#Auxiliar7.A_IdComprobante and A_Diferencia<>0),0)
WHERE IsNull(#Auxiliar2.A_NetoGravado,0)<>0

UPDATE #Auxiliar2
SET A_NetoGravadoPorTasa=A_NetoGravadoPorTasa+IsNull((Select Top 1 A_Diferencia From #Auxiliar7 
							Where #Auxiliar2.A_IdComprobante=#Auxiliar7.A_IdComprobante and A_Diferencia<>0),0)
...
INSERT INTO #Auxiliar4 
 SELECT 
	...
	SUM(Round(#Auxiliar2.A_IVA_BienesUso-0.000001,2))
 FROM #Auxiliar2
...
INSERT INTO #Auxiliar5 
 SELECT 
	...
	IsNull(#Auxiliar.A_IVA_BienesUso,0)
 FROM #Auxiliar
...
SET @vector_X='00000111111116616666666611133'
SET @vector_T='00000144942003333129999939900'
...
SELECT 
	...
	Null as [Nro.CAI],
	Null as [Condicion],
	'Total iva Bienes de Uso' as [Cuit],
	Null as [No Gravados],
	Null as [No Grav.Comp.C],
	Null as [No Grav.Imp.Int.],
	Null as [Gravados],
	Null as [Tasa],
	SUM(IsNull(A_IVA_BienesUso,0)) as [Iva],
	Null as [Col1],
	Null as [Col2],
	Null as [Col3],
	Null as [Col4],
	...
	@Vector_X as Vector_X
FROM #Auxiliar41
WHERE IsNull(#Auxiliar41.A_IVA_BienesUso,0)<>0
