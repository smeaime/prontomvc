CREATE PROCEDURE [dbo].[InformesContables_TX_IVAVentas_Modelo6]
...
-- Modelo para Corredor Central

SET NOCOUNT ON

DECLARE @IdCtaAdicCol1 int, @IdCtaAdicCol2 int, @IdCtaAdicCol3 int, @IdCtaAdicCol4 int, @IdCtaAdicCol5 int, @Cuentas varchar(100), @CtaAdicCol1 varchar(50), @CtaAdicCol2 varchar(50), 
	@CtaAdicCol3 varchar(50), @CtaAdicCol4 varchar(50), @CtaAdicCol5 varchar(50), @IdCliente int
...
SET @Cuentas='('+Convert(varchar,@IdCtaAdicCol1)+')('+Convert(varchar,@IdCtaAdicCol2)+')('+Convert(varchar,@IdCtaAdicCol3)+')'+'('+Convert(varchar,@IdCtaAdicCol4)+')('+Convert(varchar,@IdCtaAdicCol5)+')'
...
INSERT INTO #Auxiliar0 
 ...
 WHERE OrdenesCompra.IdObra is not null and IsNull(Fac.Anulada,'')<>'SI' and 
 ...
 WHERE DetalleRemitos.IdObra is not null and IsNull(Fac.Anulada,'')<>'SI' and 
	(Case When IsNull(Fac.ContabilizarAFechaVencimiento,'NO')='NO' Then Fac.FechaFactura Else Fac.FechaVencimiento End between @Desde and DATEADD(n,1439,@hasta))
 ...
 WHERE DetDev.IdObra is not null and IsNull(Dev.Anulada,'')<>'SI' and (Dev.FechaDevolucion between @Desde and DATEADD(n,1439,@hasta))
 ...
 WHERE Deb.IdObra is not null and IsNull(Deb.Anulada,'')<>'SI' and (Deb.FechaNotaDebito between @Desde and DATEADD(n,1439,@hasta)) and Deb.CtaCte='SI' and Deb.IdNotaCreditoVenta_RecuperoGastos is null
 ...
 LEFT OUTER JOIN OrdenesCompra ON doc.IdOrdenCompra = OrdenesCompra.IdOrdenCompra
 WHERE OrdenesCompra.IdObra is not null and IsNull(Cre.Anulada,'')<>'SI' and (Cre.FechaNotaCredito between @Desde and DATEADD(n,1439,@hasta)) and Cre.CtaCte='SI' and Cre.IdFacturaVenta_RecuperoGastos is null
...
INSERT INTO #Auxiliar2 
 SELECT 
	...
	Case 	When Patindex('%('+Convert(varchar,IsNull(Rubros.IdCuenta,-1))+')%', @Cuentas)=0
		 Then Case When Fac.TipoABC='B' and IsNull(Fac.IdCodigoIva,Cli.IdCodigoIva)<>8 and Fac.PorcentajeIva1<>0
				 Then ((Det.Cantidad*Det.PrecioUnitario)*(1-(Det.Bonificacion/100))) / (1+(Fac.PorcentajeIva1/100)) * Fac.CotizacionMoneda
				When Fac.TipoABC='E' or IsNull(Fac.IdCodigoIva,Cli.IdCodigoIva)=8 or IsNull(Fac.IdCodigoIva,Cli.IdCodigoIva)=9 or (Fac.PorcentajeIva1=0 and IsNull(Fac.AjusteIva,0)=0)
				 Then 0
	...
	Case 	When Patindex('%('+Convert(varchar,IsNull(Rubros.IdCuenta,-1))+')%', @Cuentas)=0
		 Then	Case 	When (Fac.TipoABC='E' and Substring(IsNull(Cuentas.Jerarquia,''),1,1)<>'4') or 
					IsNull(Fac.IdCodigoIva,Cli.IdCodigoIva)=8 or IsNull(Fac.IdCodigoIva,Cli.IdCodigoIva)=9 or (Fac.PorcentajeIva1=0 and IsNull(Fac.AjusteIva,0)=0)
	...
	Case 	When Patindex('%('+Convert(varchar,IsNull(Rubros.IdCuenta,-1))+')%', @Cuentas)=0
		...
		Else 0
	End,
	Case When IsNull(Rubros.IdCuenta,-1)=@IdCtaAdicCol1 Then ((Det.Cantidad*Det.PrecioUnitario)*(1-(Det.Bonificacion/100))) * Fac.CotizacionMoneda Else 0 End,
	Case When IsNull(Rubros.IdCuenta,-1)=@IdCtaAdicCol2 Then ((Det.Cantidad*Det.PrecioUnitario)*(1-(Det.Bonificacion/100))) * Fac.CotizacionMoneda Else 0 End,
	Case When IsNull(Rubros.IdCuenta,-1)=@IdCtaAdicCol3 Then ((Det.Cantidad*Det.PrecioUnitario)*(1-(Det.Bonificacion/100))) * Fac.CotizacionMoneda Else 0 End,
	Case When IsNull(Rubros.IdCuenta,-1)=@IdCtaAdicCol4 Then ((Det.Cantidad*Det.PrecioUnitario)*(1-(Det.Bonificacion/100))) * Fac.CotizacionMoneda Else 0 End,
	Case When IsNull(Rubros.IdCuenta,-1)=@IdCtaAdicCol5 Then ((Det.Cantidad*Det.PrecioUnitario)*(1-(Det.Bonificacion/100))) * Fac.CotizacionMoneda Else 0 End
 ...
 WHERE IsNull(Fac.Anulada,'')<>'SI' and (Case When IsNull(Fac.ContabilizarAFechaVencimiento,'NO')='NO' Then Fac.FechaFactura Else Fac.FechaVencimiento End between @Desde and DATEADD(n,1439,@hasta))
...
UNION ALL
 ...
 SELECT 
	...
	0
 FROM Facturas Fac 
 WHERE IsNull(Fac.Anulada,'')<>'SI' and (Case When IsNull(Fac.ContabilizarAFechaVencimiento,'NO')='NO' Then Fac.FechaFactura Else Fac.FechaVencimiento End between @Desde and DATEADD(n,1439,@hasta)) and 
 ...
 SELECT 
	...
	Case 	When Patindex('%('+Convert(varchar,IsNull(Rubros.IdCuenta,-1))+')%', @Cuentas)=0
		 Then 	Case 	When Dev.TipoABC='B' and IsNull(Dev.IdCodigoIva,Cli.IdCodigoIva)<>8 and Dev.PorcentajeIva1<>0
				 Then ((Det.Cantidad*Det.PrecioUnitario)*(1-(Det.Bonificacion/100))) / (1+(Dev.PorcentajeIva1/100)) * Dev.CotizacionMoneda
	...
	Case 	When Patindex('%('+Convert(varchar,IsNull(Rubros.IdCuenta,-1))+')%', @Cuentas)=0
		...
		Else 0
	End * -1,
	Case When IsNull(Rubros.IdCuenta,-1)=@IdCtaAdicCol1 Then ((Det.Cantidad*Det.PrecioUnitario)*(1-(Det.Bonificacion/100))) * Dev.CotizacionMoneda Else 0 End * -1,
	Case When IsNull(Rubros.IdCuenta,-1)=@IdCtaAdicCol2 Then ((Det.Cantidad*Det.PrecioUnitario)*(1-(Det.Bonificacion/100))) * Dev.CotizacionMoneda Else 0 End * -1,
	Case When IsNull(Rubros.IdCuenta,-1)=@IdCtaAdicCol3 Then ((Det.Cantidad*Det.PrecioUnitario)*(1-(Det.Bonificacion/100))) * Dev.CotizacionMoneda Else 0 End * -1,
	Case When IsNull(Rubros.IdCuenta,-1)=@IdCtaAdicCol4 Then ((Det.Cantidad*Det.PrecioUnitario)*(1-(Det.Bonificacion/100))) * Dev.CotizacionMoneda Else 0 End * -1,
	Case When IsNull(Rubros.IdCuenta,-1)=@IdCtaAdicCol5 Then ((Det.Cantidad*Det.PrecioUnitario)*(1-(Det.Bonificacion/100))) * Dev.CotizacionMoneda Else 0 End * -1
...
UNION ALL
 ...
 WHERE IsNull(Dev.Anulada,'')<>'SI' and (Dev.FechaDevolucion between @Desde and DATEADD(n,1439,@hasta)) and (IsNull(Dev.OtrasPercepciones1,0)<>0 or IsNull(Dev.OtrasPercepciones2,0)<>0)
...
UNION ALL
 ...
 SELECT 
	...
	End,
	Case 	When Patindex('%('+Convert(varchar,IsNull(Conceptos.IdCuenta,-1))+')%', @Cuentas)=0
		 Then	Case 	When (Deb.TipoABC='E' and Substring(IsNull(Cuentas.Jerarquia,''),1,1)<>'4') or IsNull(Deb.IdCodigoIva,Cli.IdCodigoIva)=8 or IsNull(Deb.IdCodigoIva,Cli.IdCodigoIva)=9
				 Then IsNull(Det.Importe,0) * Deb.CotizacionMoneda
				When Det.Gravado='NO' 
				 Then IsNull(Det.Importe,0) * Deb.CotizacionMoneda
				Else 0
	...
	Case 	When Patindex('%('+Convert(varchar,IsNull(Conceptos.IdCuenta,-1))+')%', @Cuentas)=0
		...
		Else 0
	End,
	Case When IsNull(Conceptos.IdCuenta,-1)=@IdCtaAdicCol1 Then IsNull(Det.Importe,0) * Deb.CotizacionMoneda Else 0 End,
	Case When IsNull(Conceptos.IdCuenta,-1)=@IdCtaAdicCol2 Then IsNull(Det.Importe,0) * Deb.CotizacionMoneda Else 0 End,
	Case When IsNull(Conceptos.IdCuenta,-1)=@IdCtaAdicCol3 Then IsNull(Det.Importe,0) * Deb.CotizacionMoneda Else 0 End,
	Case When IsNull(Conceptos.IdCuenta,-1)=@IdCtaAdicCol4 Then IsNull(Det.Importe,0) * Deb.CotizacionMoneda Else 0 End,
	Case When IsNull(Conceptos.IdCuenta,-1)=@IdCtaAdicCol5 Then IsNull(Det.Importe,0) * Deb.CotizacionMoneda Else 0 End
 ...
 WHERE IsNull(Deb.Anulada,'')<>'SI' and (Deb.FechaNotaDebito between @Desde and DATEADD(n,1439,@hasta)) and Deb.CtaCte='SI' and Deb.IdNotaCreditoVenta_RecuperoGastos is null
...
UNION ALL
 ...
 SELECT 
	...
	End * -1,
	Case 	When Patindex('%('+Convert(varchar,IsNull(Conceptos.IdCuenta,-1))+')%', @Cuentas)=0
		 Then	Case 	When (Cre.TipoABC='E' and Substring(IsNull(Cuentas.Jerarquia,''),1,1)<>'4') or IsNull(Cre.IdCodigoIva,Cli.IdCodigoIva)=8 or IsNull(Cre.IdCodigoIva,Cli.IdCodigoIva)=9
				 Then IsNull(Det.Importe,0) * Cre.CotizacionMoneda
				When Det.Gravado='NO' 
				 Then IsNull(Det.Importe,0) * Cre.CotizacionMoneda
				Else 0
	...
	Case 	When Patindex('%('+Convert(varchar,IsNull(Conceptos.IdCuenta,-1))+')%', @Cuentas)=0
		...
		Else 0
	End * -1,
	Case When IsNull(Conceptos.IdCuenta,-1)=@IdCtaAdicCol1 Then IsNull(Det.Importe,0) * Cre.CotizacionMoneda Else 0 End * -1,
	Case When IsNull(Conceptos.IdCuenta,-1)=@IdCtaAdicCol2 Then IsNull(Det.Importe,0) * Cre.CotizacionMoneda Else 0 End * -1,
	Case When IsNull(Conceptos.IdCuenta,-1)=@IdCtaAdicCol3 Then IsNull(Det.Importe,0) * Cre.CotizacionMoneda Else 0 End * -1,
	Case When IsNull(Conceptos.IdCuenta,-1)=@IdCtaAdicCol4 Then IsNull(Det.Importe,0) * Cre.CotizacionMoneda Else 0 End * -1,
	Case When IsNull(Conceptos.IdCuenta,-1)=@IdCtaAdicCol5 Then IsNull(Det.Importe,0) * Cre.CotizacionMoneda Else 0 End * -1
 ...
 LEFT OUTER JOIN Conceptos ON Conceptos.IdConcepto=Det.IdConcepto
 LEFT OUTER JOIN Cuentas ON Cuentas.IdCuenta=Conceptos.IdCuenta
 WHERE IsNull(Cre.Anulada,'')<>'SI' and (Cre.FechaNotaCredito between @Desde and DATEADD(n,1439,@hasta)) and Cre.CtaCte='SI' and Cre.IdFacturaVenta_RecuperoGastos is null
...
INSERT INTO #Auxiliar3 
 SELECT IdTipoComprobante, IdComprobante, Sum(IsNull(A_NetoGravado,0)), Sum(IsNull(A_NetoNoGravado,0)), Sum(IsNull(A_NetoNoGravadoExportacion,0)), 
	Sum(IsNull(A_CtaAdicCol1,0)), Sum(IsNull(A_CtaAdicCol2,0)), Sum(IsNull(A_CtaAdicCol3,0)), Sum(IsNull(A_CtaAdicCol4,0)), Sum(IsNull(A_CtaAdicCol5,0))
 FROM #Auxiliar2
...
CREATE TABLE #Auxiliar 
			(
			 ...
			 A_ComprobanteIni VARCHAR(20),
			 A_ComprobanteFin VARCHAR(20),
			 A_Cuenta INTEGER,
...
INSERT INTO #Auxiliar 
 ...
 SELECT 
	...
	'TIC '+Fac.TipoABC+'-'+Substring('0000',1,4-Len(Convert(varchar,Fac.PuntoVenta)))+Convert(varchar,Fac.PuntoVenta)+'-'+
		Substring('00000000',1,8-Len(Convert(varchar,IsNull(Fac.NumeroFacturaInicial,Fac.NumeroFactura))))+
		Convert(varchar,IsNull(Fac.NumeroFacturaInicial,Fac.NumeroFactura)),
	'TIC '+Fac.TipoABC+'-'+Substring('0000',1,4-Len(Convert(varchar,Fac.PuntoVenta)))+Convert(varchar,Fac.PuntoVenta)+'-'+
		Substring('00000000',1,8-Len(Convert(varchar,IsNull(Fac.NumeroFacturaFinal,Fac.NumeroFactura))))+
		Convert(varchar,IsNull(Fac.NumeroFacturaFinal,Fac.NumeroFactura)),
	Cuentas.Codigo,
	Case When Fac.IdCodigoIva=4 or (Fac.IdCliente=@IdCliente and IsNull(Fac.NumeroFacturaInicial,0)>0) Then 'CONSUMIDOR FINAL' Else Cli.RazonSocial End,
	Case When Fac.IdCodigoIva=4 or (Fac.IdCliente=@IdCliente and IsNull(Fac.NumeroFacturaInicial,0)>0) Then 'CONSUMIDOR FINAL' Else DescripcionIva.Descripcion End,
	Case When Fac.IdCodigoIva=4 or (Fac.IdCliente=@IdCliente and IsNull(Fac.NumeroFacturaInicial,0)>0) Then '00-00000000-0' Else Cli.Cuit End,
	...
	Case When Fac.TipoABC='B' and IsNull(Fac.IdCodigoIva,Cli.IdCodigoIva)<>8 and IsNull(Fac.IdCodigoIva,Cli.IdCodigoIva)<>9 and Fac.PorcentajeIva1<>0 
		 Then Case When IsNull(Fac.IVANoDiscriminado,0)<>0 Then IsNull(Fac.IVANoDiscriminado,0)
				Else ((Fac.ImporteTotal - Fac.RetencionIBrutos1 - Fac.RetencionIBrutos2 - Fac.RetencionIBrutos3 - 
						IsNull(Fac.OtrasPercepciones1,0) - IsNull(Fac.OtrasPercepciones2,0) - IsNull(Fac.OtrasPercepciones3,0) - 
						IsNull(Fac.PercepcionIVA,0)) / (1+(Fac.PorcentajeIva1/100)) * (Fac.PorcentajeIva1 / 100) + IsNull(Fac.AjusteIva,0)) End * Fac.CotizacionMoneda
		 When Fac.TipoABC='E' or IsNull(Fac.IdCodigoIva,Cli.IdCodigoIva)=8 or IsNull(Fac.IdCodigoIva,Cli.IdCodigoIva)=9 or (Fac.PorcentajeIva1=0 and IsNull(Fac.AjusteIva,0)=0)
		 Then 0
	...
	IsNull(#Auxiliar1.IdObra,Fac.IdObra),
	Substring('0000',1,4-Len(Convert(varchar,Fac.PuntoVenta)))+Convert(varchar,Fac.PuntoVenta)+'-'+Substring('00000000',1,8-Len(Convert(varchar,Fac.NumeroFactura)))+Convert(varchar,Fac.NumeroFactura),
 ...
 WHERE (Fac.Anulada is null or Fac.Anulada<>'SI') and (Case When IsNull(Fac.ContabilizarAFechaVencimiento,'NO')='NO' Then Fac.FechaFactura Else Fac.FechaVencimiento End between @Desde and DATEADD(n,1439,@hasta))
...
UNION ALL 
 ...
 SELECT 
	...
	0
 FROM Facturas Fac 
 WHERE (Fac.Anulada is not null and Fac.Anulada='SI') and (Case When IsNull(Fac.ContabilizarAFechaVencimiento,'NO')='NO' Then Fac.FechaFactura Else Fac.FechaVencimiento End between @Desde and DATEADD(n,1439,@hasta))
...
UNION ALL 
 ...
 SELECT 
	...
	Case 	When Dev.TipoABC='B' and IsNull(Dev.IdCodigoIva,Cli.IdCodigoIva)<>8 
		 Then (Dev.ImporteTotal - Dev.RetencionIBrutos1 - Dev.RetencionIBrutos2 - Dev.RetencionIBrutos3) / (1+(Dev.PorcentajeIva1/100)) * (Dev.PorcentajeIva1/100) * Dev.CotizacionMoneda * -1
...
UNION ALL 
 ...
 SELECT 
	...
	IsNull(#Auxiliar1.IdObra,Deb.IdObra),
 ...
 WHERE (Deb.FechaNotaDebito between @Desde and DATEADD(n,1439,@hasta)) and (Deb.Anulada is null or Deb.Anulada<>'SI') and Deb.CtaCte='SI' and Deb.IdNotaCreditoVenta_RecuperoGastos is null
...
UNION ALL 
 ...
 SELECT 
	...
	0
 FROM NotasDebito Deb 
 WHERE (Deb.FechaNotaDebito between @Desde and DATEADD(n,1439,@hasta)) and IsNull(Deb.Anulada,'')='SI' and Deb.CtaCte='SI' and Deb.IdNotaCreditoVenta_RecuperoGastos is null
...
UNION ALL 
 ...
 SELECT 
	...
	'TIC '+Cre.TipoABC+'-'+Substring('0000',1,4-Len(Convert(varchar,Cre.PuntoVenta)))+Convert(varchar,Cre.PuntoVenta)+'-'+
		Substring('00000000',1,8-Len(Convert(varchar,IsNull(Cre.NumeroFacturaOriginal,Cre.NumeroNotaCredito))))+Convert(varchar,IsNull(Cre.NumeroFacturaOriginal,Cre.NumeroNotaCredito)),
	'TIC '+Cre.TipoABC+'-'+Substring('0000',1,4-Len(Convert(varchar,Cre.PuntoVenta)))+Convert(varchar,Cre.PuntoVenta)+'-'+
		Substring('00000000',1,8-Len(Convert(varchar,IsNull(Cre.NumeroFacturaOriginal,Cre.NumeroNotaCredito))))+Convert(varchar,IsNull(Cre.NumeroFacturaOriginal,Cre.NumeroNotaCredito)),
	Cuentas.Codigo,
	Case When Cre.IdCodigoIva=4 or (Cre.IdCliente=@IdCliente and IsNull(Cre.NumeroFacturaOriginal,0)>0) Then 'CONSUMIDOR FINAL' Else Cli.RazonSocial End,
	Case When Cre.IdCodigoIva=4 or (Cre.IdCliente=@IdCliente and IsNull(Cre.NumeroFacturaOriginal,0)>0) Then 'CONSUMIDOR FINAL' Else DescripcionIva.Descripcion End,
	Case When Cre.IdCodigoIva=4 or (Cre.IdCliente=@IdCliente and IsNull(Cre.NumeroFacturaOriginal,0)>0) Then '00-00000000-0' Else Cli.Cuit End,
	IsNull(#Auxiliar3.A_NetoGravado,0),
	IsNull(#Auxiliar3.A_NetoNoGravado,0),
	IsNull(#Auxiliar3.A_NetoNoGravadoExportacion,0),
	...
	IsNull(#Auxiliar1.IdObra,Cre.IdObra),
	Substring('0000',1,4-Len(Convert(varchar,Cre.PuntoVenta)))+Convert(varchar,Cre.PuntoVenta)+'-'+Substring('00000000',1,8-Len(Convert(varchar,Cre.NumeroNotaCredito)))+Convert(varchar,Cre.NumeroNotaCredito), 
	IsNull(#Auxiliar3.A_CtaAdicCol1,0),
	IsNull(#Auxiliar3.A_CtaAdicCol2,0),
	IsNull(#Auxiliar3.A_CtaAdicCol3,0),
	IsNull(#Auxiliar3.A_CtaAdicCol4,0),
 ...
 WHERE (Cre.FechaNotaCredito between @Desde and DATEADD(n,1439,@hasta)) and (Cre.Anulada is null or Cre.Anulada<>'SI') and Cre.CtaCte='SI' and Cre.IdFacturaVenta_RecuperoGastos is null
...
UNION ALL 
 ...
 SELECT 
	...
	0
 FROM NotasCredito Cre 
 WHERE (Cre.FechaNotaCredito between @Desde and DATEADD(n,1439,@hasta)) and IsNull(Cre.Anulada,'')='SI' and Cre.CtaCte='SI' and Cre.IdFacturaVenta_RecuperoGastos is null
...
UPDATE #Auxiliar
SET A_AjusteDiferencia=A_Total-(A_NetoGravado+A_NetoNoGravado+A_NetoNoGravadoExportacion+A_Iva+A_Percepcion+A_PercepcionIVA+A_CtaAdicCol1+A_CtaAdicCol2+A_CtaAdicCol3+A_CtaAdicCol4+A_CtaAdicCol5)
UPDATE #Auxiliar
...
SET @vector_X='00001111111116661666'
SET @vector_T='00009477732153330333'
SET @vector_E=' FON:8,CEN | FON:8,CEN | FON:8,CEN | FON:8,CEN | FON:8,CEN | FON:8,LEF | FON:8,CEN | FON:8,CEN | '+
		'FON:8,NUM:#COMMA##0.00 | FON:8,NUM:#COMMA##0.00 | FON:8,NUM:#COMMA##0.00 | FON:8,NUM:#COMMA##0.00 | FON:8,NUM:#COMMA##0.00 | FON:8,NUM:#COMMA##0.00 | FON:8,NUM:#COMMA##0.00 '
SET @vector_E1=' ANC:10,FON:8,CEN | ANC:16,FON:8,CEN | ANC:16,FON:8,CEN | ANC:16,FON:8,CEN | ANC:10,FON:8,CEN | ANC:35,FON:8,LEF | ANC:16,FON:8,CEN | ANC:16,FON:8,CEN | '+
		' ANC:10,FON:8,NUM:#COMMA##0.00 | ANC:10,FON:8,NUM:#COMMA##0.00 | ANC:10,FON:8,NUM:#COMMA##0.00 | ANC:6,FON:8,NUM:#COMMA##0.00 | ANC:10,FON:8,NUM:#COMMA##0.00 | ANC:10,FON:8,NUM:#COMMA##0.00 | ANC:10,FON:8,NUM:#COMMA##0.00 '
IF @IdCtaAdicCol1>0
    BEGIN
	SET @vector_X=@vector_X+'6'
	SET @vector_T=@vector_T+'3'
	SET @vector_E=@vector_E+'| FON:8,NUM:#COMMA##0.00 '
	SET @vector_E1=@vector_E1+'| ANC:12,FON:8,NUM:#COMMA##0.00,VAL:1;16;'+@CtaAdicCol1+' '
    END
ELSE
    BEGIN
...
IF @IdCtaAdicCol2>0
	...
	SET @vector_T=@vector_T+'3'
	SET @vector_E=@vector_E+'| FON:8,NUM:#COMMA##0.00 '
	SET @vector_E1=@vector_E1+'| ANC:12,FON:8,NUM:#COMMA##0.00,VAL:1;17;'+@CtaAdicCol2+' '
    END
ELSE
    BEGIN
	SET @vector_X=@vector_X+'1'
...
IF @IdCtaAdicCol3>0
	...
	SET @vector_T=@vector_T+'3'
	SET @vector_E=@vector_E+'| FON:8,NUM:#COMMA##0.00 '
	SET @vector_E1=@vector_E1+'| ANC:12,FON:8,NUM:#COMMA##0.00,VAL:1;18;'+@CtaAdicCol3+' '
    END
ELSE
    BEGIN
	SET @vector_X=@vector_X+'1'
...
IF @IdCtaAdicCol4>0
	...
	SET @vector_T=@vector_T+'3'
	SET @vector_E=@vector_E+'| FON:8,NUM:#COMMA##0.00 '
	SET @vector_E1=@vector_E1+'| ANC:12,FON:8,NUM:#COMMA##0.00,VAL:1;19;'+@CtaAdicCol4+' '
    END
ELSE
    BEGIN
	SET @vector_X=@vector_X+'1'
...
IF @IdCtaAdicCol5>0
	...
	SET @vector_T=@vector_T+'3'
	SET @vector_E=@vector_E+'| FON:8,NUM:#COMMA##0.00 '
	SET @vector_E1=@vector_E1+'| ANC:12,FON:8,NUM:#COMMA##0.00,VAL:1;20'+@CtaAdicCol5+' '
    END
ELSE
    BEGIN
	SET @vector_X=@vector_X+'1'
...
SET @vector_E=@vector_E+'| FON:8,NUM:#COMMA##0.00 '
SET @vector_E1=@vector_E1+'| ANC:12,FON:8,NUM:#COMMA##0.00 '
...
SELECT 
 ...
 A_ComprobanteIni as [Ticket inicial],
 A_ComprobanteFin as [Ticket final],
...
SELECT 
 ...
 Null as [Ticket inicial],
 Null as [Ticket final],
 Null as [.],
 Null as [Fecha],
 Null as [Comprobante],
 Null as [Obra],
...
SELECT 
 ...
 Null as [IVA],
 Null as [Perc.IVA],
 Null as [Perc.I.B.], Null as [CtaAdicCol1],
 Null as [CtaAdicCol2],
 Null as [CtaAdicCol3],
 Null as [CtaAdicCol4],
 Null as [CtaAdicCol5],
