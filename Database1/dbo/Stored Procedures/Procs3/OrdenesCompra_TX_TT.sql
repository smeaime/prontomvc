CREATE  Procedure [dbo].[OrdenesCompra_TX_TT]
...
INSERT INTO #Auxiliar4 
 ...
 WHERE Det.IdOrdenCompra=@IdOrdenCompra
...
/*  CURSOR  */
DECLARE @IdOrdenCompra1 int, @Remito varchar(13), @Corte int, @P varchar(500)
SET @Corte=0
SET @P=''
...
OPEN Cur
FETCH NEXT FROM Cur INTO @IdOrdenCompra1, @Remito
WHILE @@FETCH_STATUS = 0
   BEGIN
	IF @Corte<>@IdOrdenCompra1
	   BEGIN
		IF @Corte<>0
			UPDATE #Auxiliar3
...
DECLARE Cur CURSOR LOCAL FORWARD_ONLY 
	FOR	SELECT IdOrdenCompra, Factura
		...
		ORDER BY IdOrdenCompra
OPEN Cur
FETCH NEXT FROM Cur INTO @IdOrdenCompra1, @Factura
WHILE @@FETCH_STATUS = 0
   BEGIN
	IF @Corte<>@IdOrdenCompra1
	   BEGIN
...
WHERE OrdenesCompra.IdOrdenCompra=@IdOrdenCompra
