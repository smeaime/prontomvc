...
CREATE Procedure [dbo].[DiferenciasCambio_TX_PorCobranzasPendientes]
...
SET @Cotizacion=(Select Top 1 CotizacionLibre From Cotizaciones
			Where 	Year(Cotizaciones.Fecha)=Year(GETDATE()) And 
				...
				Cotizaciones.IdMoneda=(Select Parametros.IdMonedaDolar
...
SET @vector_T='0292043444444444226699600'
SET @Marca='NO'
...
SELECT 
 ...
 Case When Recibos.IdMoneda=@IdMonedaPesos 
	Then 	Case When CuentasCorrientesDeudores.IdTipoComp=@IdTipoComprobanteFacturaVenta
			Then Round(DetalleRecibos.Importe*Recibos.CotizacionMoneda/
					...
					(Recibos.Cotizacion/
						Case When IsNull(Facturas.CotizacionDolar,0)<>0
			...
			Else Round(DetalleRecibos.Importe*Recibos.CotizacionMoneda/
					...
					(Recibos.Cotizacion/
						Case When IsNull(NotasDebito.CotizacionDolar,0)<>0
 ...
 End as [Generado],
 @Marca as [Marcado],
 Null as [Tipo comp.gen.],
 Null as [Nro.comp.gen.],
 Recibos.IdCliente,
 IdDiferenciaCambio as [IdDif],
 Null as [Fecha comp.gen.],
...
WHERE DiferenciasCambio.Estado is null And 
