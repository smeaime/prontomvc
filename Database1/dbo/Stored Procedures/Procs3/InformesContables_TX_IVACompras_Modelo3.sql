CREATE PROCEDURE [dbo].[InformesContables_TX_IVACompras_Modelo3]
...
INSERT INTO #Auxiliar 
 SELECT 
	...
	Case 	When cp.IdProveedor is not null Then Proveedores.CodigoEmpresa
		When cp.IdProveedorEventual is not null 
		 Then (Select Top 1 P.CodigoEmpresa From Proveedores P
...
--		Else Null--	End,
	...
	#Auxiliar2.A_NetoGravadoPorTasa
 FROM #Auxiliar2
...
UNION ALL 
 ...
 SELECT 
	...
	End,
	Null,
	0 FROM DetalleOrdenesPagoCuentas dopc 
...
SET A_NetoGravado=A_NetoGravado+IsNull((Select Top 1 A_Diferencia From #Auxiliar7 
					Where #Auxiliar.A_IdComprobante=#Auxiliar7.A_IdComprobante and 
						A_Diferencia<>0),0)
WHERE IsNull(#Auxiliar.A_NetoGravado,0)<>0

UPDATE #Auxiliar2
SET A_NetoGravado=A_NetoGravado+IsNull((Select Top 1 A_Diferencia From #Auxiliar7 
					Where #Auxiliar2.A_IdComprobante=#Auxiliar7.A_IdComprobante and 
						A_Diferencia<>0),0)
...
INSERT INTO #Auxiliar41 
 SELECT 
	...
	SUM(#Auxiliar4.A_CtaAdicCol5) FROM #Auxiliar4
...
CREATE TABLE #Auxiliar8 
			(
			 ...
			 A_KTipoComprobante1 VARCHAR(2),
			 A_Comprobante VARCHAR(30),
			 A_PercepcionesRetenciones NUMERIC(18, 2),
			 A_NetoNoGravado NUMERIC(18, 2),
			 A_NetoNoGravadoImpInt NUMERIC(18, 2)
			)
INSERT INTO #Auxiliar8 
 SELECT A_IdComprobante, A_KFecha, A_KTipoComprobante, A_KTipoComprobante1, A_Comprobante, 
	Sum(IsNull(A_CtaAdicCol1,0)+IsNull(A_CtaAdicCol2,0)+IsNull(A_CtaAdicCol3,0)+
		IsNull(A_CtaAdicCol4,0)+IsNull(A_CtaAdicCol5,0)), 
	Sum(IsNull(A_NetoNoGravado,0)+IsNull(A_NetoNoGravadoComprobantesC,0)), 
	Sum(IsNull(A_NetoNoGravadoImpInt,0))
 FROM #Auxiliar
 GROUP BY A_IdComprobante, A_KFecha, A_KTipoComprobante, A_KTipoComprobante1, A_Comprobante
...
INSERT INTO #Auxiliar9 
 SELECT A_IdComprobante, A_KFecha, A_KTipoComprobante, A_KTipoComprobante1, A_Comprobante, A_Tasa, 
	Sum(IsNull(A_Iva,0)), Sum(IsNull(A_NetoGravadoPorTasa,0))
 FROM #Auxiliar
 GROUP BY A_IdComprobante, A_KFecha, A_KTipoComprobante, A_KTipoComprobante1, A_Comprobante, A_Tasa

SET NOCOUNT OFF

Declare @vector_X varchar(40),@vector_T varchar(40),@vector_E varchar(1000)
Set @vector_X='000000111116166133'
Set @vector_T='000000423020033900'
Set @vector_E='  '
...
SELECT 
	...
	Null as [Cond.IVA],
	Null as [Nro.CUIT], 	Null as [Imp.mp.Int.],
	Null as [Imp.Total],
	'FinTransporte' as Vector_E,
	@Vector_T as Vector_T,
	@Vector_X as Vector_X
FROM #Auxiliar
WHERE A_KTipoComprobante1<>'OP'
GROUP BY A_KFecha
...
SELECT 
	...
	A_Comprobante as [A_KComprobante],
	'02' as [Orden],
	Null as [Fecha],
	Max(IsNull(A_TipoComprobante,'')) as [Cod.Com.],
	A_Comprobante as [N.Comprobante],
	Max(IsNull(A_CodigoProveedor,'')) as [Provee],
	Max(IsNull(A_Proveedor,'')) as [Razon Social],
	Max(IsNull(A_CondicionIVA,'')) as [Cond.IVA],
	Max(IsNull(A_Cuit,'')) as [Nro.CUIT], 	Null as [Imp.mp.Int.],
	Null as [Imp.Total],
	'  |  |  |  |  |  |  |  |  ' as Vector_E,
	@Vector_T as Vector_T,
	@Vector_X as Vector_X
FROM #Auxiliar
WHERE A_KTipoComprobante1<>'OP'
GROUP BY A_IdComprobante, A_KFecha, A_KTipoComprobante, A_Comprobante
...
SELECT 
	...
	Null as [Provee],
	Null as [Razon Social],
	Convert(varchar,IsNull(A_PercepcionesRetenciones,0)) as [Cond.IVA],
	Convert(varchar,IsNull(A_NetoNoGravado,0)) as [Nro.CUIT], 
	IsNull(A_NetoNoGravadoImpInt,0) as [Imp.mp.Int.],
	Null as [Imp.Total],
	...
	@Vector_X as Vector_X
FROM #Auxiliar8
WHERE A_KTipoComprobante1<>'OP' and 
	(IsNull(A_PercepcionesRetenciones,0)<>0 or IsNull(A_NetoNoGravado,0)<>0 or 
	 IsNull(A_NetoNoGravadoImpInt,0)<>0)
...
SELECT 
	...
	Null as [Fecha],
	Null as [Cod.Com.],
	Convert(varchar,A_NetoGravadoPorTasa) as [N.Comprobante],
	Convert(varchar,A_Tasa) as [Provee],
	Convert(varchar,A_IVA) as [Razon Social],
	Null as [Cond.IVA],
	Null as [Nro.CUIT], 
	Null as [Imp.mp.Int.],
	Null as [Imp.Total],
	...
	@Vector_X as Vector_X
FROM #Auxiliar9
WHERE A_KTipoComprobante1<>'OP' and (IsNull(A_NetoGravadoPorTasa,0)<>0 or IsNull(A_IVA,0)<>0)
...
SELECT 
	...
	Null as [Fecha],
	'TC' as [Cod.Com.],
	Convert(varchar,Sum(IsNull(A_NetoGravadoPorTasa,0))) as [N.Comprobante],
	Null as [Provee],
	Convert(varchar,Sum(IsNull(A_IVA,0))) as [Razon Social],
	Convert(varchar,Sum(IsNull(A_CtaAdicCol1,0)+IsNull(A_CtaAdicCol2,0)+
		IsNull(A_CtaAdicCol3,0)+IsNull(A_CtaAdicCol4,0)+IsNull(A_CtaAdicCol5,0))) as [Cond.IVA],
	Convert(varchar,Sum(IsNull(A_NetoNoGravado,0)+IsNull(A_NetoNoGravadoComprobantesC,0))) as [Nro.CUIT], 
	Sum(IsNull(A_NetoNoGravadoImpInt,0)) as [Imp.mp.Int.],
	Sum(IsNull(A_Total,0)) as [Imp.Total],
	'  |  | RIG,NUM:#COMMA##0.00 |  | RIG,NUM:#COMMA##0.00 | NUM:#COMMA##0.00 |'+
...
SELECT 
	...
	0 as [Tipo],
	@Hasta as [A_KFecha],
	'zzz' as [A_KTipoComprobante],
	'zzz' as [A_KComprobante],
	'07' as [Orden],
	Null as [Fecha],
	'Totales :' as [Cod.Com.],
	Convert(varchar,Sum(IsNull(A_NetoGravadoPorTasa,0))) as [N.Comprobante],
	...
	Sum(IsNull(A_NetoNoGravadoImpInt,0)) as [Imp.mp.Int.],
	Sum(IsNull(A_Total,0)) as [Imp.Total],
	' EBH, CO2, AL2:42, AN2:4;10, AN2:5;30, AN2:6;19, AN2:7;14, '+
		'AV2:1;1, AV2:2;1, AV2:3;1, AV2:4;1, AV2:5;1, AV2:6;1, AV2:7;1, AV2:8;3, AV2:9;3, '+
		'AH2:3;1, AH2:4;1, AH2:5;1, AH2:6;1, AH2:7;1, '+
		'FN2:1;10, FN2:2;10, FN2:3;10, FN2:4;10, FN2:5;10, FN2:6;10, FN2:7;10, FN2:8;10, FN2:9;10, '+
		'VAL:1;3;Nro.Comproba;          Importe;          Neto Grav., VAL:1;4;Provee;     Tasa  C;     IVA    I, '+
		'VAL:1;5;Razon Social;                    Importe;                    I.V.A., VAL:1;6;          Cond.IVA;Percepciones;Retenciones, '+
		'VAL:1;7;     Nro.CUIT.;Importes     Cod;Exentos      I.Int, VAL:1;8;Importe;Impto.Int., VAL:1;9;Importe;Total'+
...
ORDER BY [Tipo], [A_KFecha], [A_KTipoComprobante], [A_KComprobante], [A_IdComprobante], [Orden]
