CREATE PROCEDURE [dbo].[InformesContables_TX_IVACompras_Modelo6]
...
/*
 20/04/2009
	Este modelo se definio para la cooperativa, no se tiene en cuenta el modificador @AgenteRetencionIVA='SI' 
	para incluir los recibos de cobranza cuando tocan alguna de las cuentas contables parametrizadas como
	columnas adicionales (en este caso la de retencion iva).
*/
...
DECLARE @IdTipoCuentaGrupoIVA int, @IvaCompras_DesglosarNOGRAVADOS varchar(2), @IdCuentaImpuestosInternos int, @AgenteRetencionIVA varchar(2), @IdComprobanteAEmitir int, 
	@IdCtaAdicCol1 int, @IdCtaAdicCol2 int, @IdCtaAdicCol3 int, @IdCtaAdicCol4 int, @IdCtaAdicCol5 int,
...
SET @CtaAdicCol1=IsNull((Select Descripcion From Cuentas Where IdCuenta=@IdCtaAdicCol1),'')
SET @CtaAdicCol2=IsNull((Select Descripcion From Cuentas Where IdCuenta=@IdCtaAdicCol2),'')
SET @CtaAdicCol3=IsNull((Select Descripcion From Cuentas Where IdCuenta=@IdCtaAdicCol3),'')
SET @CtaAdicCol4=IsNull((Select Descripcion From Cuentas Where IdCuenta=@IdCtaAdicCol4),'')
SET @CtaAdicCol5=IsNull((Select Descripcion From Cuentas Where IdCuenta=@IdCtaAdicCol5),'')
...
INSERT INTO #Auxiliar 
 ...
 WHERE Recibos.FechaRecibo between @Desde and DATEADD(n,1439,@hasta) and IsNull(Recibos.Anulado,'')<>'SI' and 
...
SET A_Diferencia=(A_Total-(A_NetoNoGravado+A_NetoNoGravadoComprobantesC+
				A_NetoNoGravadoImpInt+A_NetoGravado+A_IVA+A_CtaAdicCol1+A_CtaAdicCol2+A_CtaAdicCol3+A_CtaAdicCol4+A_CtaAdicCol5))
...
DECLARE @vector_X varchar(50), @vector_T varchar(50), @vector_E varchar(500), @vector_E1 varchar(500), @Pos int

SET @vector_X='00000111111116616666666661133'
SET @vector_T='0000014494200333312'
SET @vector_E=' FON:8 | FON:8 | FON:8 | FON:8 | FON:8 | FON:8 | FON:8 | FON:8,RIG,NUM:#COMMA##0.00 | FON:8,RIG,NUM:#COMMA##0.00 '+
		'| FON:8,RIG,NUM:#COMMA##0.00 | FON:8,RIG,NUM:#COMMA##0.00 | FON:8,RIG,NUM:#COMMA##0.00 | FON:8,RIG,NUM:#COMMA##0.00 '
SET @vector_E1=''
SET @Pos=14

IF Len(@CtaAdicCol1)>0
    BEGIN
	SET @vector_T=@vector_T+'3'
	SET @vector_E=@vector_E+'| RIG,NUM:#COMMA##0.00 '
	SET @vector_E1=@vector_E1+' VAL:1;'+Convert(varchar,@Pos)+';'+@CtaAdicCol1+', '
	SET @Pos=@Pos+1
    END
ELSE
    BEGIN
    ...
    END
IF Len(@CtaAdicCol2)>0
    BEGIN
	SET @vector_T=@vector_T+'3'
	SET @vector_E=@vector_E+'| RIG,NUM:#COMMA##0.00 '
	SET @vector_E1=@vector_E1+' VAL:1;'+Convert(varchar,@Pos)+';'+@CtaAdicCol2+', '
	SET @Pos=@Pos+1
    END
ELSE
    BEGIN
	SET @vector_T=@vector_T+'9'
    END
IF Len(@CtaAdicCol3)>0
    BEGIN
	SET @vector_T=@vector_T+'3'
	SET @vector_E=@vector_E+'| RIG,NUM:#COMMA##0.00 '
	SET @vector_E1=@vector_E1+' VAL:1;'+Convert(varchar,@Pos)+';'+@CtaAdicCol3+', '
	SET @Pos=@Pos+1
    END
ELSE
    BEGIN
	SET @vector_T=@vector_T+'9'
    END
IF Len(@CtaAdicCol4)>0
    BEGIN
	SET @vector_T=@vector_T+'3'
	SET @vector_E=@vector_E+'| RIG,NUM:#COMMA##0.00 '
	SET @vector_E1=@vector_E1+' VAL:1;'+Convert(varchar,@Pos)+';'+@CtaAdicCol4+', '
	SET @Pos=@Pos+1
    END
ELSE
    BEGIN
	SET @vector_T=@vector_T+'9'
    END
IF Len(@CtaAdicCol5)>0
    BEGIN
	SET @vector_T=@vector_T+'3'
	SET @vector_E=@vector_E+'| RIG,NUM:#COMMA##0.00 '
	SET @vector_E1=@vector_E1+' VAL:1;'+Convert(varchar,@Pos)+';'+@CtaAdicCol5+', '
	SET @Pos=@Pos+1
    END
ELSE
    BEGIN
	SET @vector_T=@vector_T+'9'
    END
SET @vector_T=@vector_T+'39900'

SET @vector_E1=@vector_E1+@vector_E
