CREATE  Procedure [dbo].[ValesSalida_TXFecha]
...
INSERT INTO #Auxiliar9 
 ...
 LEFT OUTER JOIN ValesSalida ON Det.IdValeSalida = ValesSalida.IdValeSalida
 ...
 WHERE ValesSalida.FechaValeSalida between @Desde and @hasta
...
/*  CURSOR  */
DECLARE @IdValeSalida int, @P varchar(100), @Corte int, @Salida varchar(13)
SET @Corte=0
SET @P=''

DECLARE Cur CURSOR LOCAL FORWARD_ONLY FOR SELECT IdValeSalida, Salida FROM #Auxiliar9 ORDER BY IdValeSalida
OPEN Cur
FETCH NEXT FROM Cur INTO @IdValeSalida, @Salida
...
WHERE ValesSalida.FechaValeSalida between @Desde and @hasta and 
	(@IdObraAsignadaUsuario=-1 or ValesSalida.IdObra=@IdObraAsignadaUsuario)
