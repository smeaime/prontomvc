CREATE  Procedure [dbo].[SalidasMateriales_TXFecha]

@Desde datetime,
@Hasta datetime,
@IdObraAsignadaUsuario int = Null,
@ConCostos varchar(2) = Null,
@DescartarRecepcionadas varchar(2) = Null
...
SET @DescartarRecepcionadas=IsNull(@DescartarRecepcionadas,'NO')
...
IF (SELECT COUNT(*) FROM #Auxiliar10)>0
   BEGIN
	SET @sql1='SELECT DISTINCT DetSal.IdSalidaMateriales, a.Descripcion, a.NumeroInventario 
			...
			WHERE a.IdArticulo is not null and sm.FechaSalidaMateriales between Convert(datetime,'''+Convert(varchar,@Desde,103)+''',103) and Convert(datetime,'''+Convert(varchar,@hasta,103)+''',103)'
...
ELSE
   BEGIN
	INSERT INTO #Auxiliar1 
	 ...
	 WHERE Articulos.IdArticulo is not null and SalidasMateriales.FechaSalidaMateriales between @Desde and @hasta
    END
...
INSERT INTO #Auxiliar3 
 ...
 WHERE SalidasMateriales.FechaSalidaMateriales between @Desde and @hasta
...
SET @vector_X='01111111111111111111111111111111111111133'
IF @MostrarEntreCalles='SI'
	SET @vector_T='0795331540512222255253A043314142253861F00'
ELSE
	SET @vector_T='0795331540512222255253A043314142253861900'
...
SELECT 
 ...
 Case When @ConCostos='SI' 
	Then (Select Sum(IsNull(dsm.Cantidad,0)*IsNull(dsm.CostoUnitario,0)*IsNull(dsm.CotizacionMoneda,1))
 ...
 ArchivosATransmitirDestinos.Descripcion as [Origen SAT],
 ...
 (Select Count(*) From DetalleSalidasMateriales 
	Where DetalleSalidasMateriales.IdSalidaMateriales=SalidasMateriales.IdSalidaMateriales) as [Cant.Items],
 ...
 SalidasMateriales.RecibidosEnDestino as [Recibido en destino],
 SalidasMateriales.RecibidosEnDestinoFecha as [Fecha recepcion],
 E6.Nombre as [Marco recepcion en destino],
...
LEFT OUTER JOIN Empleados E6 ON SalidasMateriales.RecibidosEnDestinoIdUsuario = E6.IdEmpleado
...
WHERE SalidasMateriales.FechaSalidaMateriales between @Desde and @hasta and 
	(@IdObraAsignadaUsuario=-1 or SalidasMateriales.IdObra=@IdObraAsignadaUsuario or IsNull(SalidasMateriales.IdObraOrigen,-1)=@IdObraAsignadaUsuario) and 
	(@DescartarRecepcionadas='NO' or 
	 (@DescartarRecepcionadas='SI' and IsNull(SalidasMateriales.Anulada,'NO')<>'SI' and SalidasMateriales.TipoSalida=1 and 
