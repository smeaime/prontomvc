...
IF @EquipoDestinoDesdePRONTO_MANTENIMIENTO_RM_VA='SI' and Len(@BasePRONTOMANT)>0
	...
	SET @sql1='Select Distinct dp.IdPedido, a.IdArticulo, a.Descripcion, a.NumeroInventario 
			...
			Where Pedidos.FechaPedido Between Convert(datetime,'+''''+Convert(varchar,@Desde,103)+''''+',103) and Convert(datetime,'+''''+Convert(varchar,@Hasta,103)+''''+',103) and 
				a.IdArticulo is not null and IsNull(A.ParaMantenimiento,'+''''+''+''''+')='+''''+'SI'+''''+' and IsNull(A.Activo,'+''''+''+''''+')<>'+''''+'NO'+''''
...
SET @vector_T='0EQQHFC555555055595G1033311433A8241G00'
...
WHERE (@VerTodosLosPedidos=-1 or IsNull(Pedidos.Cumplido,'NO')='NO') and (Pedidos.FechaPedido Between @Desde And @Hasta) and 
	(@IdUsuario=-1 or IsNull((Select Top 1 e.PermitirAccesoATodasLasObras From Empleados e Where e.IdEmpleado=@IdUsuario),'SI')='SI' or 
	 Exists(Select Top 1 deo.IdObra From DetalleEmpleadosObras deo Where deo.IdEmpleado=@IdUsuario and Patindex('%('+Convert(varchar,deo.IdObra)+')%', dbo.Pedidos_IdObras(Pedidos.IdPedido))<>0))
