...
create procedure ProduccionParte_TX_ArticulosPorOrdenPorProcesoParaComboSinFiltrarStock
@IdProduccionOrden int,
@IdProceso int,

@IdProduccionParte int=-1  --si es un alta, filtro los materiales que alcanzaron la cantidad necesaria

--,@IdProceso int --si no aclarás de que ficha toma los datos, no va a saber qué tolerancia usar.
...
SELECT TOP 1 @idproduccionFicha=idproduccionFicha FROM produccionfichas
			WHERE IdArticuloAsociado=(SELECT idArticulogenerado FROM produccionordenes WHERE idproduccionorden=@idproduccionorden)
				AND idcolor=(SELECT idcolor FROM produccionordenes WHERE idproduccionorden=@idproduccionorden)

IF ISNULL(@idproduccionFicha,0)=0
begin
SELECT TOP 1 @idproduccionFicha=idproduccionFicha FROM produccionfichas
			WHERE IdArticuloAsociado=(SELECT idArticulogenerado FROM produccionordenes WHERE idproduccionorden=@idproduccionorden)
END
...
,isnull(dbo.fProduccionAvanzadoMaterial(@IdProduccionOrden,Det.IdArticulo,det.idcolor),0) as Avanzado,
...
left join ProduccionFichas CabFicha on  CabOP.IdArticuloGenerado = CabFicha.IdArticuloAsociado and CabOP.idColor=CabFicha.idColor

left join ProduccionFichas CabFicha2 on cabficha2.IdProduccionFicha=@idproduccionFicha --CabOP.IdArticuloGenerado = CabFicha2.IdArticuloAsociado and CabFicha2.idColor=ISNULL(CabOP.idColor,965)
left JOIN DetalleProduccionFichas DetFicha ON CabFicha2.idProduccionFicha=DetFicha.idProduccionFicha AND detficha.idarticulo=det.IdArticulo
...
WHERE Det.IdProduccionOrden=@IdProduccionOrden 
	...
	and 
		(@IdProduccionParte<>-1 
			or 
		(Det.Cantidad>isnull(dbo.fProduccionAvanzadoMaterial(@IdProduccionOrden,Det.IdArticulo,det.idcolor),0)) ) --o es un alta o todavia no alcance el material necesario

--ORDER by det.idDetalleProduccionorden --  
ORDER BY DetFicha.IdDetalleProduccionFicha  --Articulos.Descripcion
