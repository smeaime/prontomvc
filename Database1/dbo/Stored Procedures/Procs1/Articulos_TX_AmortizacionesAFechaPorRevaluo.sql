...
CREATE Procedure [dbo].[Articulos_TX_AmortizacionesAFechaPorRevaluo]
...
INSERT INTO #Auxiliar1 
 SELECT 
  ...
  Case When DetAFijos.ImporteRevaluo is not null
	Then DetAFijos.ImporteRevaluo
	Else 0
  End,
/*  Case When Revaluos.FechaRevaluo is null or 
  ...
  End,
  1,
  Case When DetAFijos.VidaUtilRevaluo is not null and DetAFijos.VidaUtilRevaluo<>0
	Then 12/DetAFijos.VidaUtilRevaluo
	Else 0
  End,
  0,
  Case 	When DetAFijos.Fecha is not null and 
		Year(DetAFijos.Fecha)=Year(@AFecha)
	 Then 'A'
	When Articulos.FechaBajaAmortizacion is not null and 
		Year(Articulos.FechaBajaAmortizacion)=Year(@AFecha)
	 Then 'B'
	 Else ' '
  End,
  Case When DetAFijos.VidaUtilRevaluo is not null and DetAFijos.VidaUtilRevaluo<>0
	Then DetAFijos.VidaUtilRevaluo
	Else 0
  End,
  0,
  Case 	When DetAFijos.Fecha is null or 
		DetAFijos.Fecha<@FechaUltimoCalculo
	 Then DateDiff(m,@FechaUltimoCalculo,@AFecha) + 1
	When DetAFijos.Fecha>@AFecha
	 Then 0
/*	Else DateDiff(m,DetAFijos.Fecha,@AFecha)+1	TOMAR TODO EL AÑO CUANDO SON ALTAS DEL PERIODO*/
 ...
 WHERE DetAFijos.IdRevaluo is not null and DetAFijos.IdRevaluo=@IdRevaluo
...
WHERE FechaDetalleRevaluo is not null and 
	DateAdd(m,VidaUtilContable,FechaDetalleRevaluo)<@FechaUltimoCalculo
