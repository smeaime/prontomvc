CREATE PROCEDURE [dbo].[CuboSaldosComprobantesPorObraProveedor]
...
INSERT INTO _TempCuboSaldosComprobantesPorObraProveedor 
 ...
 LEFT OUTER JOIN CuentasCorrientesAcreedores CtaCte ON cp.IdComprobanteProveedor=CtaCte.IdComprobante and cp.IdTipoComprobante=CtaCte.IdTipoComp
 ...
 LEFT OUTER JOIN RubrosContables ON RubrosContables.IdRubroContable=CuentasGastos.IdRubroContable
 LEFT OUTER JOIN CuentasCorrientesAcreedores CtaCte ON cp.IdComprobanteProveedor=CtaCte.IdComprobante and cp.IdTipoComprobante=CtaCte.IdTipoComp
 WHERE cp.FechaRecepcion<=@FechaHasta and IsNull(cp.Confirmado,'')<>'NO' and IsNull(CtaCte.Saldo,0)<>0 and DetCP.IdObra is not null and 
	IsNull(Cuentas.IdObra,-1)=-1 and -- Esto da error de paralelismo si pregunto : Cuentas.IdObra is null
	Cuentas.IdCuentaGasto is null and 
 ...
 SELECT 
  ...
  '',
  'Vto.: '+Convert(varchar,IsNull(cp.FechaVencimiento,CtaCte.Fecha),103)+' '+Convert(varchar,CtaCte.Fecha,103)+' '+
	TiposComprobante.DescripcionAb+' '+Substring('00000000',1,8-Len(Convert(varchar,CtaCte.NumeroComprobante)))+Convert(varchar,CtaCte.NumeroComprobante),
  CtaCte.ImporteTotal * TiposComprobante.Coeficiente * -1,
  CtaCte.ImporteTotal,
  CtaCte.Saldo,
  0
