CREATE Procedure [dbo].[Articulos_TX_BD_ProntoMantenimientoPorNumeroInventario]
...
IF (SELECT COUNT(*) FROM #Auxiliar1)>0
   BEGIN
	...
	IF @IdObraPronto is not null
	   BEGIN
		SET @NumeroObra=IsNull((Select Top 1 O.NumeroObra
						From Obras O 
						Where O.IdObra=@IdObraPronto),'')
		SET @sql1='Select Top 1 O.IdObra 
				From '+@BasePRONTOMANT+'.dbo.Obras O
	...
	SET @sql1='SELECT Art.IdArticulo, Art.Descripcion as [Material], Art.Codigo as [Codigo], 
			Art.NumeroInventario as [NumeroInventario], Art.IdArticulo as [Id], 
			R.Descripcion as [Rubro], S.Descripcion as [Subrubro],
			Art.ParaMantenimiento as [P/mant.?], O.Descripcion as [Ubicacion actual],
			...
			Where ('+Convert(varchar,@IdObraMantenimiento)+'=-1 or 
				...
				Art.NumeroInventario='+''''+@NumeroInventario+''''
