CREATE Procedure [dbo].[Cuentas_CuadroGastosPorObraDetallado]
...
INSERT INTO #Auxiliar2 
 SELECT 
  ...
  Case When cp.IdProveedor is not null
	Then 	Case When dcp.IdProvinciaDestino1 is null or dcp.IdProvinciaDestino1=0
			Then Proveedores.IdProvincia
  ...
  Case When IsNull(#Auxiliar1.Importe,0)<>0 
	Then IsNull(dcp.PorcentajeProvinciaDestino1,100) * 
			((dcp.Importe*cp.CotizacionMoneda) / 
			 Case When IsNull(cp.TotalIvaNoDiscriminado,0)<>0 Then 1+(IsNull(dcp.IvaComprasPorcentaje1,0)/100) Else 1 End / Abs(#Auxiliar1.Importe))
	Else 100 
  End
 ...
 SELECT 
  dcp.IdComprobanteProveedor,
  Case When cp.IdProveedor is not null
	Then 	Case When dcp.IdProvinciaDestino2 is null or dcp.IdProvinciaDestino2=0
			Then Proveedores.IdProvincia
  ...
  Case When IsNull(#Auxiliar1.Importe,0)<>0 
	Then IsNull(dcp.PorcentajeProvinciaDestino2,100) * 
			((dcp.Importe*cp.CotizacionMoneda) / 
			 Case When IsNull(cp.TotalIvaNoDiscriminado,0)<>0 Then 1+(IsNull(dcp.IvaComprasPorcentaje1,0)/100) Else 1 End / #Auxiliar1.Importe)
	Else 100 
  End
 FROM DetalleComprobantesProveedores dcp
 LEFT OUTER JOIN ComprobantesProveedores cp ON cp.IdComprobanteProveedor=dcp.IdComprobanteProveedor  
...
SELECT 
 ...
 Case 	When RubrosContables.IdRubroContable is not null Then RubrosContables.IdRubroContable
	 Else IsNull((Select Top 1 cg.IdRubroContable From Cuentas C 
			Left Outer Join CuentasGastos cg On cg.IdCuentaGasto=C.IdCuentaGasto
			Where cg.IdCuentaMadre=Subdiarios.IdCuenta),0)
 End,
 ...
 Case 	When Subdiarios.IdTipoComprobante=@IdTipoComprobanteFacturaVenta
	 Then (Select Provincias.Nombre From Provincias Where Provincias.IdProvincia=dfp.IdProvinciaDestino)
	When Subdiarios.IdTipoComprobante=@IdTipoComprobanteDevoluciones
	 Then (Select Provincias.Nombre From Provincias Where Provincias.IdProvincia=Devoluciones.IdProvinciaDestino)
	When Subdiarios.IdTipoComprobante=@IdTipoComprobanteNotaDebito
	 Then (Select Provincias.Nombre From Provincias Where Provincias.IdProvincia=dndp.IdProvinciaDestino)
	When Subdiarios.IdTipoComprobante=@IdTipoComprobanteNotaCredito
	 Then (Select Provincias.Nombre From Provincias Where Provincias.IdProvincia=dncp.IdProvinciaDestino)
	When Subdiarios.IdTipoComprobante=@IdTipoComprobanteOrdenPago
	 Then (Select Provincias.Nombre From Provincias Where Provincias.IdProvincia=OrdenesPago.IdProvinciaDestino)
	When Subdiarios.IdTipoComprobante=@IdTipoComprobanteRecibo
	 ...
	 Else 	Case When IsNull(TiposComprobante.Agrupacion1,' ')='PROVEEDORES'
			Then (Select Provincias.Nombre From Provincias Where Provincias.IdProvincia=#Auxiliar2.IdProvinciaDestino)
			When IsNull(TiposComprobante.Agrupacion1,'')='GASTOSBANCOS'
			Then (Select Provincias.Nombre From Provincias Where Provincias.IdProvincia=CuentasBancarias.IdProvincia)
			Else 'Capital Federal'
		End
 End,
 Case 	When IsNull(TiposComprobante.Agrupacion1,' ')='PROVEEDORES'
	 Then Convert(varchar,cp.FechaRecepcion,103)+' '+
...
--  +' - '+'Cta.: '+Convert(varchar,Cuentas.Codigo)
	When Subdiarios.IdTipoComprobante=@IdTipoComprobanteFacturaVenta
	 Then Convert(varchar,Subdiarios.FechaComprobante,103)+' '+
...
--  +' - '+'Cta.: '+Convert(varchar,Cuentas.Codigo)
	When Subdiarios.IdTipoComprobante=@IdTipoComprobanteOrdenPago
	 Then Convert(varchar,Subdiarios.FechaComprobante,103)+' '+
		TiposComprobante.DescripcionAb+' ('+OrdenesPago.Tipo COLLATE Modern_Spanish_CI_AS+') '+
...
SELECT 
 ...
 End,
 Provincias.Nombre,
 Convert(varchar,Asientos.FechaAsiento,103)+' AS '+
	Substring('00000000',1,8-Len(Convert(varchar,Asientos.NumeroAsiento)))+Convert(varchar,Asientos.NumeroAsiento),
...
IF @IdObra>0 
	DELETE _TempCuadroGastosParaCubo WHERE IdObra<>@IdObra
