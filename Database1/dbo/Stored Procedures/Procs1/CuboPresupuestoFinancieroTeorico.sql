CREATE Procedure [dbo].[CuboPresupuestoFinancieroTeorico]
...
INSERT INTO #Auxiliar1 
 SELECT 
  ...
  Case When cp.Letra='A' or cp.Letra='M' 
	Then (DetCP.Importe + IsNull(DetCP.ImporteIVA1,0) +  IsNull(DetCP.ImporteIVA2,0) +  IsNull(DetCP.ImporteIVA3,0) + 
...
INSERT INTO _TempCuboPresupuestoFinancieroTeorico 
 SELECT 
  ...
  Detalle,
  Importe * IsNull(Tmp.Porcentaje,100)/100
 FROM #Auxiliar1
 LEFT OUTER JOIN _TempCondicionesCompra Tmp ON #Auxiliar1.IdCondicionCompra=Tmp.IdCondicionCompra
 WHERE DateAdd(day,IsNull(Tmp.Dias,0),Fecha)>=@VencimientosPosterioresA
