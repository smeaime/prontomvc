'Tal como lo hablamos telefónicamente , te paso un resumen de cómo es la
'operatoria de Fertilizantes, luego te envío los archivos para que lo vayas
'mirando.



'PASO 1  -  CUPO   (Solapa 1 reemplazaría la solapa de posición)



'-          LDC nos pasa vía mail una planilla con los cupos del día para
'cargar los camiones, esa planilla es un Excel la cual nosotros tendríamos
'que hacer la pegatina al sistema nuestro

'Tendrían alguna planilla con más ejemplos? Porque justo esa tiene un solo
'dato cargado y así podemos ver los diferentes casos que pueden suceder.

'Si es posible también les pido que nos den una pequeña descripción de qué es
'cada columna.

'-          Una vez pegada la planilla de cupo en nuestro sistema , se genera
'una OC (orden de carga) con los datos del cupo , la cual se imprime y se le
'entrega al chofer para que vaya a cargar.

'Les pido si pueden pasarnos el modelo de Orden de Carga, indicando dónde va
'cada dato del cupo.

'-          Esa OC emitida ya se la asigna un producto  y kilos parciales
'(ídem kilos procedencia).

'Recién al emitir la OC se le asigna un producto o ya en la importación del
'Excel se sabe a partir de algún dato? Lo mismo con los kilos.

'Cuando decís que esa OC le asigna los kilos parciales, te referís a que
'recién al emitir la OC el cupo va a las planillas?

'Necesitaríamos el listado de productos.



'PASO 2 – INGRESO Y EGRESO (Solapa 2 reemplazaría la solapa de descarga)



'-          Una vez cargado el camión el chofer vuelve con un ticket de carga
'del depósito el cual nosotros lo tenemos que volcar al sistema.

'Podrían pasarnos algunos ticket de ejemplo? Les pido que nos lo detallen y
'nos indiquen qué datos se cargarán en el sistema.

'-          Luego teniendo la opción de ingreso o egreso según la operación
'con los kilos registrados del tickets del depósito generamos un REMITO DE
'LDC (adjunto imagen).

'Creo que no me llegó la imagen del remito, más allá de eso, el remito se
'generaría desde el sistema o se los da LDC de alguna manera?

'-          Una vez ingresado el remito con los KILOS (final) , ahora si
'descuenta los kilos en el stock del producto.

'-          Imprimir remito (x cuadruplicado)

'-          Importar imagen del remito (ídem imagen de ccpp)

'En estos dos últimos puntos  me confundo, el remito se imprime desde el
'sistema? Si se imprime desde el sistema, para qué se vuelve a importar una
'imagen del mismo?



'PASO 3 – Informes (ídem informes)



'-          En este paso lo usaremos para sacar todos los informes que se
'necesiten (por día, por producto, etc.) (te mando las planillas excel que
'necesitamos)

'-          Y también necesitamos uno de Movimiento (para embarques,
'transferencias, etc.)
...
'Todo esto que te paso es lo más grueso para ir desarrollando y mirando,
'después vamos viendo lo más fino,  nos vamos hablando lo que necesites

'En otros mail te paso los archivos
...
#Const Ancho = "1000px"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            'Me.ViewState.Add(mKey, New FertilizantesCupos)
            ...
            Else
                ...
                Else
                    ...
                    Else
                        'pruebas de debug 
                        'pruebas de debug
                        myCartaDePorte = New FertilizantesCupos   'pruebas de debug
                        ' RangeValidatorFechaDescarga.MinimumValue() = Today.AddDays(-3).ToShortDateString
                        'pruebas de debug
                        'pruebas de debug
                    End If
    ...
    Function TraerLogDeCartaPorte(ByVal idcarta As Long, bTraerImputaciones As Boolean) As Data.DataTable
        ...
        'EntidadManager.LogPronto(SC, idfactura, "Imputacion de IdCartaPorte" & oCDP.Id & "CDP:" & oCDP.NumeradorTexto & " " & oCDP.SubnumeroVagon & "  IdFacturaImputada " & idfactura, nombreUsuario)
    ...
    Function AltaSetup() As FertilizantesCupos
        Dim myCartaDePorte = New FertilizantesCupos
        myCartaDePorte.IdFertilizanteCupo = -1
        ...
        'txtNumeradorTexto.Text = Pronto.ERP.Dal.GeneralDB.TraerDatos(SC, "wParametros_T").Tables(0).Rows(0).Item("ProximoNumeradorTexto").ToString
        'myCartaDePorte.Fecha = System.DateTime.Now.ToShortDateString()
        'myCartaDePorte.Numero = Pronto.ERP.Dal.GeneralDB.TraerDatos(SC, "wParametros_T").Tables(0).Rows(0).Item("ProximoNumeradorTexto").ToString
        ...
        'Dim mItem As CartaDePorteItem = New FertilizantesCuposItem
        'mItem.Id = -1
        'mItem.Nuevo = True
        'mItem.Cantidad = 0
        ...
        ViewState("PaginaTitulo") = "Nuevo Cupo"
    ...
    Function EditarSetup(Optional ByVal CopiaDeOtroId As Long = -1) As FertilizantesCupos

        Dim myCartaDePorte As ProntoMVC.Data.Models.FertilizantesCupos


        If CopiaDeOtroId = -1 Then 'no debería hacer lo de la copia en el Alta en lugar de acá?
            myCartaDePorte = FertilizanteManager.GetItem(SC, IdCartaDePorte, True)
            'FertilizanteManager.DuplicarCartaporteConOtroSubnumeroDeFacturacion(SC, myCartaDePorte)
        ...
        Else
            ...
            'IdCartaDePorte = -1
            'myCartaDePorte = FertilizanteManager.GetItem(SC, CopiaDeOtroId, True)
            'myCartaDePorte.Id = -1
            'myCartaDePorte.SubnumeroDeFacturacion = FertilizanteManager.ProximoSubNumeroParaNumeroCartaPorte(SC, myCartaDePorte)
            'myCartaDePorte.IdClienteAFacturarle = -1
            'MostrarLinksAFamiliaDeDuplicados(myCartaDePorte)

            ''hago que directamente ya se grabe
            'FertilizanteManager.Save(SC, myCartaDePorte, Session(SESSIONPRONTO_glbIdUsuario), Session(SESSIONPRONTO_UserName))
        ...
        If Not (myCartaDePorte Is Nothing) Then
            ...
            ViewState("PaginaTitulo") = "Cupo " + myCartaDePorte.NumeradorTexto.ToString
        End If
    ...
    Sub BloqueosDeEdicion()
        ...
        Dim myCartaDePorte As FertilizantesCupos = FertilizanteManager.GetItem(SC, IdCartaDePorte, True)
        If IdCartaDePorte = -1 Then myCartaDePorte = New FertilizantesCupos
        ...
        With myCartaDePorte
            ...
            Catch ex As Exception
                ...
                ErrHandler2.WriteError(ex)
            End Try
            'If .IdFertilizanteCupo <> -1 And pventa <> 0 And .PuntoVenta <> pventa Then
            ...
            Else
                ...
                If .IdFacturaImputada <> 0 Or .Anulada = "SI" Then
                    ...
                    'txtNumeradorTexto.Enabled = False
    ...
    '        Dim myCartaDePorte As FertilizantesCupos = CType(Me.ViewState(mKey), FertilizantesCupos)
    ...
    Protected Sub btnDuplicarEscondido_Click() Handles btnDuplicarEscondido.Click
        ...
        If myCartaDePorte.IdFertilizanteCupo > 0 Then
            ...
            '////////////////////////////////////////////////////////////////////////////////////////////////////////
            FertilizanteManager.Save(SC, myCartaDePorte, Session(SESSIONPRONTO_glbIdUsuario), Session(SESSIONPRONTO_UserName), False)
    ...
    Sub CargarObjeto(ByRef myCartaDePorte As FertilizantesCupos)
        ...
        With myCartaDePorte
            ...
            'de servicios)
            '     FertilizanteManager.ReasignoTarifaSubcontratistas(SC, myCartaDePorte)




            .NumeradorTexto = txtNumeroCDP.Text
            ...
            txtNumeroCDP.Text = .NumeradorTexto

            .Mezcla = Convert.ToInt32(cmbPuntoVenta.Text)



            .FechaIngreso = Convert.ToDateTime(iisValidSqlDate(txtFechaCarga.Text))
            .FechaIngreso = Convert.ToDateTime(iisValidSqlDate(txtFechaArribo.Text))

            .Puro = IIf(chkEsPuro.Checked, 1, 0)


            .Cliente = BuscaIdClientePrecisoConCUIT(txtTitular.Text, SC)
            If .Cliente = -1 Then .Cliente = Nothing
            .CuentaOrden = BuscaIdClientePrecisoConCUIT(txtIntermediario.Text, SC)
            If .CuentaOrden = -1 Then .CuentaOrden = Nothing

            .IdArticulo = BuscaIdArticuloPreciso(txt_AC_Articulo.Text, SC)
            .IdArticuloComponente1 = BuscaIdArticuloPreciso(txt_AC_Articulo1.Text, SC)
            .IdArticuloComponente2 = BuscaIdArticuloPreciso(txt_AC_Articulo2.Text, SC)
            .IdArticuloComponente3 = BuscaIdArticuloPreciso(txt_AC_Articulo3.Text, SC)
            .IdArticuloComponente4 = BuscaIdArticuloPreciso(txt_AC_Articulo4.Text, SC)
            If .IdArticulo = -1 Then .IdArticulo = Nothing
            If .IdArticuloComponente1 = -1 Then .IdArticuloComponente1 = Nothing
            If .IdArticuloComponente2 = -1 Then .IdArticuloComponente2 = Nothing
            If .IdArticuloComponente3 = -1 Then .IdArticuloComponente3 = Nothing
            If .IdArticuloComponente4 = -1 Then .IdArticuloComponente4 = Nothing

            .TipoEgresoIngreso = cmbMovimiento.SelectedValue


            .FormaDespacho = cmbFormaDespacho.SelectedValue

            .Porcentaje1 = StringToDecimal(txtPorcentaje1.Text)
            .Porcentaje2 = StringToDecimal(txtPorcentaje2.Text)
            .Porcentaje3 = StringToDecimal(txtPorcentaje3.Text)
            .Porcentaje4 = StringToDecimal(txtPorcentaje4.Text)

            .IdLocalidadTransportista = BuscaIdLocalidadPreciso(txtOrigen.Text, SC)
            If .IdLocalidadTransportista = -1 Then .IdLocalidadTransportista = Nothing
            .Destino = BuscaIdWilliamsDestinoPreciso(txtDestino.Text, SC)
            If .Destino = -1 Then .Destino = Nothing
            .IdTransportista = BuscaIdTransportistaPrecisoConCUIT(txtTransportista.Text, SC)
            If .IdTransportista = -1 Then .IdTransportista = Nothing
            .IdChofer = BuscaIdChoferPrecisoConCUIT(txtChofer.Text, SC)
            If .IdChofer = -1 Then .IdChofer = Nothing

            .Cantidad = StringToDecimal(txtNetoFinalTotalMenosMermas.Text)


            .Contrato = txtContrato.Text
            .Recorrido = StringToDecimal(txtKmRecorrer.Text)
            .Observaciones = txtObservaciones.Text
            .Chasis = txtPatenteCamion.Text
            .Acoplado = txtPatenteAcoplado.Text




            .HoraArribo = Convert.ToDateTime(iisValidSqlDate(txtFechaArribo.Text + " " + txthoraarribo.Text))
            .HoraAutorizacion = Convert.ToDateTime(iisValidSqlDate(txtFechaArribo.Text + " " + txthoraautoriza.Text))
            .HoraCarga = Convert.ToDateTime(iisValidSqlDate(txtFechaArribo.Text + " " + txthoracarga.Text))
            .HoraDespacho = Convert.ToDateTime(iisValidSqlDate(txtFechaArribo.Text + " " + txthoradespacho.Text))
            .NumeroRemito = txtNumeroRemito.Text
            .NumeroCotizacion = txtNumeroCotizacion.Text
            .Despacho = cmbDespacho.Text
            .PuntoDespacho = cmbPuntoDespacho.Text
            .KilosMaximo = StringToDecimal(txtKgMaximo.Text)


            .Tara = StringToDecimal(txtTaraFinal.Text)
            .Bruto = StringToDecimal(txtBrutoFinal.Text)

            .LitrosFinal = StringToDecimal(txtLitrosFinal.Text)
    ...
    Sub RecargarEncabezado(ByVal myCartaDePorte As FertilizantesCupos, Optional ByVal CopiarSoloLosGenerales As Boolean = False)
        ...
        Try
            ...
            With myCartaDePorte
                ...
                txtFechaCarga.Text = iisNull(.FechaIngreso, "")
                txtFechaArribo.Text = iisNull(.FechaIngreso, "")

                chkEsPuro.Checked = (.Puro = 1)
                txt_AC_Articulo1.Enabled = Not chkEsPuro.Checked
                txtPorcentaje1.Enabled = Not chkEsPuro.Checked
                txt_AC_Articulo2.Enabled = Not chkEsPuro.Checked
                txtPorcentaje2.Enabled = Not chkEsPuro.Checked
                txt_AC_Articulo3.Enabled = Not chkEsPuro.Checked
                txtPorcentaje3.Enabled = Not chkEsPuro.Checked
                txt_AC_Articulo4.Enabled = Not chkEsPuro.Checked
                txtPorcentaje4.Enabled = Not chkEsPuro.Checked

                BuscaTextoEnCombo(cmbPuntoVenta, .Mezcla)

                If .Cliente > 0 Then txtTitular.Text = EntidadManager.GetItem(SC, "Clientes", .Cliente).Item("RazonSocial")


                If .CuentaOrden > 0 Then txtIntermediario.Text = EntidadManager.GetItem(SC, "Clientes", .CuentaOrden).Item("RazonSocial")


                If .IdArticuloComponente1 > 0 Then txt_AC_Articulo1.Text = ArticuloManager.GetItem(SC, .IdArticuloComponente1).Descripcion
                If .IdArticuloComponente2 > 0 Then txt_AC_Articulo2.Text = ArticuloManager.GetItem(SC, .IdArticuloComponente2).Descripcion
                If .IdArticuloComponente3 > 0 Then txt_AC_Articulo3.Text = ArticuloManager.GetItem(SC, .IdArticuloComponente3).Descripcion
                If .IdArticuloComponente4 > 0 Then txt_AC_Articulo4.Text = ArticuloManager.GetItem(SC, .IdArticuloComponente4).Descripcion
                txtPorcentaje1.Text = If(.Porcentaje1, 0)
                txtPorcentaje2.Text = If(.Porcentaje2, 0)
                txtPorcentaje3.Text = If(.Porcentaje3, 0)
                txtPorcentaje4.Text = If(.Porcentaje4, 0)
                ...
                cmbMovimiento.SelectedValue = .TipoEgresoIngreso


                Dim aaa = .FormaDespacho
                cmbFormaDespacho.SelectedValue = If(.FormaDespacho, 1)




                'TextBox5.Text = .
                txtNetoFinalTotalMenosMermas.Text = If(.Cantidad, 0)
                ...
                linkPDF.NavigateUrl = "\CartaDePorteEncriptada.aspx&Id=" & EntidadManager.encryptQueryString(ID).ToString()
                ...
                If .HoraArribo IsNot Nothing Then txthoraarribo.Text = iisNull(Left(.HoraArribo.Value.TimeOfDay.ToString, 5), "")
                If .HoraAutorizacion IsNot Nothing Then txthoraautoriza.Text = iisNull(Left(.HoraAutorizacion.Value.TimeOfDay.ToString, 5), "")
                If .HoraCarga IsNot Nothing Then txthoracarga.Text = iisNull(Left(.HoraCarga.Value.TimeOfDay.ToString, 5), "")
                If .HoraDespacho IsNot Nothing Then txthoradespacho.Text = iisNull(Left(.HoraDespacho.Value.TimeOfDay.ToString, 5), "")
                txtNumeroRemito.Text = .NumeroRemito
                txtNumeroCotizacion.Text = .NumeroCotizacion
                cmbDespacho.Text = .Despacho
                cmbPuntoDespacho.Text = .PuntoDespacho
                txtKgMaximo.Text = If(.KilosMaximo, 0)


                txtTaraFinal.Text = If(.Tara, 0)
                txtBrutoFinal.Text = If(.Bruto, 0)


                txtLitrosFinal.Text = If(.LitrosFinal, 0)
                ...
                If .IdLocalidadTransportista > 0 Then txtOrigen.Text = EntidadManager.ExecDinamico(SC, "select * from Localidades where IdLocalidad=" & .IdLocalidadTransportista).Rows(0).Item("Nombre")
                ...
                txtKmRecorrer.Text = If(.Recorrido, 0)
    ...
    Sub DeObjetoHaciaPagina(ByVal myCartaDePorte As FertilizantesCupos)
        RecargarEncabezado(myCartaDePorte)
    ...
    Function ValidarCamposDeLosQueNoSeHaceAltaAlVuelo() As String
        ...
        If BuscaIdArticuloPreciso(txt_AC_Articulo.Text, SC) = -1 And txt_AC_Articulo.Text <> "" Then
    ...
    '                If myCartaDePorte Is Nothing Then myCartaDePorte = New FertilizantesCupos
    ...
    '                '//////////////////////////////////////////////////
    '                '//////////////////////////////////////////////////
    '                Dim cdp As FertilizantesCupos
    '                Try
    '                    cdp = FertilizanteManager.GetItemPorNumero(SC, txtNumeroCDP.Text)

    '                    If cdp.IdFertilizanteCupo <> -1 Then
    '                        If IdEntity = -1 Then 'estoy haciendo un alta
    '                            MsgBoxAjax(Me, "El numero/vagon ya existe, " & cdp.NumeradorTexto & "/")
    '                            Return
    '                        Else
    '                            If IdEntity <> cdp.IdFertilizanteCupo Then 'esta editando esa ahora? si no...
    '                                MsgBoxAjax(Me, "El numero/vagon ya existe, " & cdp.NumeradorTexto & "/")
    '                                Return
    '                            End If
    '                        End If
    ...
    '                If FertilizanteManager.IsValid(SC, myCartaDePorte, ms, advertencias) Then
    ...
    '                    Try
    '                        Select Case FertilizanteManager.Save(SC, myCartaDePorte, Session(SESSIONPRONTO_glbIdUsuario), Session(SESSIONPRONTO_UserName))
    '                            Case -1
    ...
    '                'If IdCartaDePorte = -1 And ParametroManager.GrabarRenglonUnicoDeTablaParametroOriginal(SC, "ProximoNumeradorTexto", myCartaDePorte.NumeradorTexto + 1) = -1 Then MsgBoxAjax(Me, "Hubo un error al modificar los parámetros")
    '                '////////////////////////////////////////////////////
    '                '////////////////////////////////////////////////////

    '                Session("UltimaIdCDPeditada") = myCartaDePorte.IdFertilizanteCupo
    ...
    '                    If True Then
    '                        Label1.Text = "<a href= 'CartaDePorte.aspx?Id=" & myCartaDePorte.IdFertilizanteCupo & "' target='_blank'> Carta anterior " & myCartaDePorte.NumeradorTexto & " grabada a las " & Now.ToString("h:mm:ss") & "</a>"
    ...
    '                    Response.Redirect(String.Format("CartaDePorte.aspx?Id=" & myCartaDePorte.IdFertilizanteCupo), False)
    '                    Exit Sub
    ...
    '                'If myCartaDePorte.NumeradorTexto <> StringToDecimal(txtNumeroCDP.Text) Then
    '                '    'EndEditing("El CartaDePorte fue grabada con el número " & myCartaDePorte.NumeradorTexto) 'me voy 
    ...
    '            End If
    '        Else
    '            MsgBoxAjax(Me, "El objeto no es válido. (mOk falso)")
    '            mAltaItem = False
    ...
    '    Dim myCartaDePorte As FertilizantesCupos = CType(Me.ViewState(mKey), FertilizantesCupos)
    '    If Not FertilizanteManager.IsValid(myCartaDePorte) Then 'hago la validacion manualmente porque no me está andando el CustomValidator que controla la grilla (aunque sí se dispara si aprieto "AgrgarItem", probablemente por el UpdatePanel
    '        mOk = False
        ...
        If bPassOK Then
            ...
            With myCartaDePorte
                ...
                Try
                    ...
                    If .Anulada = "SI" Then
                        'des-anula
                        r = FertilizanteManager.DesAnular(SC, myCartaDePorte, Session(SESSIONPRONTO_glbIdUsuario), Session(SESSIONPRONTO_UserName))

                        '.Anulada = "NO"
                    Else
                        'loguear aca que id se le esta pasando
                        ErrHandler2.WriteError(myCartaDePorte.IdFertilizanteCupo & "  llamo a anular " & Session(SESSIONPRONTO_UserName))
                        r = FertilizanteManager.Anular(SC, myCartaDePorte, Session(SESSIONPRONTO_glbIdUsuario), Session(SESSIONPRONTO_UserName))
                ...
                Catch ex As Exception
                    ErrHandler2.WriteError(myCartaDePorte.IdFertilizanteCupo & " " & myCartaDePorte.NumeradorTexto & " " & " " & ex.ToString)
                    ErrHandler2.WriteAndRaiseError("Error al anular. Casi seguro pasa porque aprietan varias veces el Ok. Le parece que es un duplicado y no deja anularla. O quizás tiró error de concurrencia. " & ex.ToString)
            ...
            If r = -1 Then
                MsgBoxAjax(Me, "No se pudo anular el cupo")
            Else
    ...
    Protected Sub lnkRepetirUltimaCDP_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkRepetirUltimaCDP.Click
        ...
        If Session("UltimaIdCDPeditada") Is Nothing Then
            ...
            Try
                ' Session("UltimaIdCDPeditada") = FertilizanteManager.UltimaCDP(SC)
            Catch ex As Exception
        ...
        If False Then
            ...
            Dim c = FertilizanteManager.GetItem(SC, Session("UltimaIdCDPeditada"), True)
        ...
        Try
            If IsNumeric(Session("UltimaIdCDPeditada")) Then
                RecargarEncabezado(FertilizanteManager.GetItem(SC, Session("UltimaIdCDPeditada"), True), True)
            End If
        Catch ex As Exception
            ErrHandler2.WriteError(ex)
        End Try
    ...
    Protected Sub txtPorcentajeHumedad_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txtPorcentajeHumedad.TextChanged
        ...
        If idarticulo = -1 Then
            ...
            Return
        End If
        ' Dim porcentajemerma = FertilizanteManager.BuscaMermaSegunHumedadArticulo(SC, idarticulo, StringToDecimal(txtPorcentajeHumedad.Text))
    ...
    Sub RefrescarValidadorDuplicidad()
        ...
        Try
            ...
            Dim actualCartaDePorte As FertilizantesCupos = CType(Me.ViewState(mKey), FertilizantesCupos)
            ' lblErrorUnicidad.Visible = FertilizanteManager.validarUnicidad(SC, txtNumeroCDP.Text, txtSubNumeroVagon.Text, IdEntity, actualCartaDePorte)
    ...
    Protected Sub AsyncFileUpload1_UploadedComplete(ByVal sender As Object, ByVal e As AjaxControlToolkit.AsyncFileUploadEventArgs) Handles AsyncFileUpload1.UploadedComplete
        ...
        Try
            ...
            '   nombre = FertilizanteManager.AdjuntarImagen(SC, AsyncFileUpload1, IdCartaDePorte, sError, DirApp(), NameOnlyFromFullPath(AsyncFileUpload1.PostedFile.FileName))
        Catch ex As Exception
            ErrHandler2.WriteError(ex)
        End Try
    ...
    Protected Sub AsyncFileUpload2_UploadedComplete(ByVal sender As Object, ByVal e As AjaxControlToolkit.AsyncFileUploadEventArgs) Handles AsyncFileUpload2.UploadedComplete
        ...
        Dim sError As String = ""
        Try
            nombre = FertilizanteManager.AdjuntarImagen2(SC, AsyncFileUpload2, IdCartaDePorte, sError, DirApp(), NameOnlyFromFullPath(AsyncFileUpload2.PostedFile.FileName))
        Catch ex As Exception
            ErrHandler2.WriteError(ex)
        End Try
    ...
    Sub reloadimagen()
        Try
            ...
            Using db As New DemoProntoEntities(Auxiliares.FormatearConexParaEntityFramework(Encriptar(SC)))
                Dim oCarta = (From i In db.FertilizantesCupos Where i.IdFertilizanteCupo = IdCartaDePorte).SingleOrDefault
    ...
    Protected Sub quitarimagen1_Click(sender As Object, e As System.EventArgs) Handles quitarimagen1.Click
        ...
        linkImagen.Visible = False
        Session("NombreAdjunto") = ".."
        'If IdCartaDePorte > 0 Then FertilizanteManager.QuitarImagen1(SC, IdCartaDePorte)
    End Sub
    ...
    Protected Sub quitarimagen2_Click(sender As Object, e As System.EventArgs) Handles quitarimagen2.Click
        ...
        linkImagen_2.Visible = False
        Session("NombreAdjunto2") = ".."
        'If IdCartaDePorte > 0 Then FertilizanteManager.QuitarImagen2(SC, IdCartaDePorte)
    End Sub
            ...
            If mOk Then
                If Not mAltaItem Then
                    ...
                    If myCartaDePorte Is Nothing Then
                        myCartaDePorte = New FertilizantesCupos
                        myCartaDePorte.IdFertilizanteCupo = Me.IdEntity
                    End If
                    ...
                    Try
                        ...
                        If txtNumeroCDP.Text = "" Then 'estoy haciendo un alta
                            MsgBoxAjax(Me, "Falta el número de cupo")
                            Return
                        End If
                        ...
                        If cdp.IdFertilizanteCupo > 0 Then 'encontró algo
                            If IdEntity = -1 Then 'estoy haciendo un alta
                                MsgBoxAjax(Me, "El numero/vagon ya existe, " & cdp.NumeradorTexto & "/")
                                Return
                            Else
                    ...
                    If IdCartaDePorte = -1 And IsNothing(Request.QueryString.Get("CopiaDe")) Then
                        ...
                        If False Then
                            ...
                            AjaxControlToolkit.ToolkitScriptManager.RegisterStartupScript(Me, Me.GetType(), "StartUp", " resetForm($('form[name=aspnetForm]'));", True)
                        Else
                            Response.Redirect(String.Format("Fertilizante.aspx?Id=-1"), False)
                        End If
    ...
    Protected Sub Button10_Click(sender As Object, e As EventArgs) Handles Button10.Click
        Dim plantilla = DirApp() & "\Documentos\" & "RemitoLDC.docx"


        Dim output As String = DirApp() & "\Documentos\" & "remito_.docx"
        ...
        System.IO.File.Copy(plantilla, output)

        Dim o = FertilizanteManager.GetItem(SC, IdCartaDePorte, True)

        OpenXML_Pronto.RemitoParaLDC_XML_DOCX_Williams(output, o, HFSC.Value)
        Try
    ...
    Protected Sub Button9_Click(sender As Object, e As EventArgs) Handles Button9.Click
        Dim plantilla = DirApp() & "\Documentos\" & "OrdenDeCarga.docx"


        Dim output As String = DirApp() & "\Documentos\" & "orden_.docx"
        'tengo que copiar la plantilla en el destino, porque openxml usa el archivo que le vaya a pasar
        Dim MyFile1 = New FileInfo(output) 'busca si ya existe el archivo a generar y en ese caso lo borra
        If MyFile1.Exists Then
            MyFile1.Delete()
        ...
        OpenXML_Pronto.OrdenCarga_XML_DOCX_Williams(output, o, HFSC.Value)
        Try
            MyFile1 = New FileInfo(output) 'quizás si me fijo de nuevo, ahora verifica que el archivo existe...
            If MyFile1.Exists Then
                Response.ContentType = "application/octet-stream"
    ...
    Protected Sub chkEsPuro_CheckedChanged(sender As Object, e As EventArgs) Handles chkEsPuro.CheckedChanged
        txt_AC_Articulo1.Enabled = Not chkEsPuro.Checked
        txtPorcentaje1.Enabled = Not chkEsPuro.Checked
        txt_AC_Articulo2.Enabled = Not chkEsPuro.Checked
        txtPorcentaje2.Enabled = Not chkEsPuro.Checked
