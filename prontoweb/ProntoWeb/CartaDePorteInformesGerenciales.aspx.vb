...
Partial Class CartaDePorteInformesGerenciales
        ...
        Try
            ...
            Select Case cmbInforme.Text
                ...
                Case "Planilla de movimientos"
                    ...
                    LogicaInformesWilliams.GeneroDataTablesDeMovimientosDeStock(dtCDPs, dt2, dtMOVs, idDestinatario, idDestino, idArticulo,
                                                        fechadesde, fechahasta, HFSC.Value, cmbPuntoVenta.SelectedValue)
    ...
    Sub ResumenFacturacion()
        ...
        Try
            ...
            Dim s1 = From fac In db.linqFacturas _
            ...
            Dim s2 = From fac In db.linqFacturas _
                    From cdp In db.CartasPorteMovimientos.Where(Function(i) i.IdFacturaImputada = fac.IdFactura).DefaultIfEmpty _
                    From art In db.linqArticulos.Where(Function(i) i.IdArticulo = cdp.IdArticulo).DefaultIfEmpty _
                    From clitit In db.linqClientes.Where(Function(i) i.IdCliente = cdp.IdExportadorOrigen).DefaultIfEmpty _
                    From clidest In db.linqClientes.Where(Function(i) i.IdCliente = cdp.IdExportadorDestino).DefaultIfEmpty _
                    From clifac In db.linqClientes.Where(Function(i) i.IdCliente = fac.IdCliente).DefaultIfEmpty _
                    From emp In db.linqEmpleados.Where(Function(i) i.IdEmpleado = cdp.IdUsuarioIngreso).DefaultIfEmpty _
                    Where fac.FechaFactura >= fechadesde And fac.FechaFactura < fechahasta _
                        And (cdp.IdFacturaImputada > 0) _
                        And (idVendedor = -1 Or cdp.IdExportadorOrigen = idVendedor) _
                        And (idDestinatario = -1 Or cdp.IdExportadorDestino = idDestinatario) _
                        And (idArticulo = -1 Or cdp.IdArticulo = idArticulo) _
                        And (pv = -1 Or cdp.IdStock = pv) _
                        And (DropDownList2.Text = "Buques" Or DropDownList2.Text = "Ambos")
                    Select _
                            fac.IdFactura, _
                            fac.IdCliente, _
                            Factura = fac.TipoABC.ToString & "-" & fac.PuntoVenta.ToString & "-" & fac.NumeroFactura.ToString, _
                            CDP = CLng(cdp.NumeroCDPMovimiento), _
                            FechaFactura = fac.FechaFactura, _
                            Obs = cdp.Observaciones, _
                            KgNetos = cdp.Cantidad, _
                            Cliente = clifac.RazonSocial, _
                            VendedorDesc = clitit.RazonSocial, _
                            CuentaOrden1Desc = "BUQUE", _
                            CuentaOrden2Desc = "", _
                            CorredorDesc = "", _
                            EntregadorDesc = clidest.RazonSocial, _
                            ProcedenciaDesc = "", _
            ...
            Dim s = s1.ToList.Union(s2.ToList).ToList()
    ...
    Sub ProyeccionFacturacion()
        ...
        Dim lista As New Generic.List(Of tipoloco2)
        Dim i As tipoloco2
...
Class LogicaInformesWilliamsGerenc
