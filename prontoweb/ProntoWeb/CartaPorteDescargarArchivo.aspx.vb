...
Partial Class CartaPorteDescargarArchivo
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        ...
        Dim output = ConfigurationManager.AppSettings("AplicacionConImagenes") + "\DataBackupear\" + Request.QueryString.Get("Id")




        'verificar q el archivo esta en un comentario del usuario

        Dim u = Membership.GetUser.UserName
        Dim archivo As String = Request.QueryString.Get("Id")

        CartaDePorteManager.TienePermisosParaEsteArchivo(u, archivo, SC, ConexBDLmaster)
        ...
        '    Dim archivo As String = Request.QueryString.Get("Id")
        '    Dim coment = db.ReclamoComentarios.Where(Function(x) x.Comentario = archivo).SingleOrDefault
        '    If coment IsNot Nothing Then
        '        Dim carta = coment.Reclamo.CartasDePortes.First()

        '        TienePermisoParaLaCarta(
        ...
        If True Then
            ...
            If System.Diagnostics.Debugger.IsAttached() Then
                CartaDePorteManager.PDFcon_iTextSharp(output,
                                   HttpContext.Current.Server.MapPath(IIf(myCartaDePorte.PathImagen <> "", sDirFTP, "")) + myCartaDePorte.PathImagen,
                                 HttpContext.Current.Server.MapPath(IIf(myCartaDePorte.PathImagen2 <> "", sDirFTP, "")) + myCartaDePorte.PathImagen2
                                  )
            Else
                ...
                Try
                    CartaDePorteManager.PDFcon_iTextSharp(output,
                                      IIf(myCartaDePorte.PathImagen <> "", sDirFTP + myCartaDePorte.PathImagen, ""),
                                    IIf(myCartaDePorte.PathImagen2 <> "", sDirFTP + myCartaDePorte.PathImagen2, "")
                                      )
