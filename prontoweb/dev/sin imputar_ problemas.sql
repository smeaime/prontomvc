...
-- Select Det.IdDetalleFactura as [IdDetalleFactura], 'BUQUE' as [Tipo], Sum(MOVS.Cantidad/1000) as [Cantidad]
-- From Detallefacturas Det
-- Inner Join Facturas Fac On Fac.IdFactura=Det.IdFactura  
...
select idfactura,cantidad
,dbo.Factura_HechaEnProntoWeb(IdFactura) as hechaenpweb
,dbo.Factura_CantidadDeCartasPorteImputadasOriginalmente(IdFactura) as orig
,dbo.Factura_CantidadDeCartasPorteImputadas(IdFactura) as ahora
,dbo.Factura_TieneNotaDeCreditoImputada(IdFactura) as tieneNotaDeCreditoImputada
from 
(


Select dbo.Factura_HechaEnProntoWeb(det.IdFactura) as hechaenpweb,  dbo.Factura_CantidadDeCartasPorteImputadas(det.IdFactura) as imputaciones, det.Observaciones,det.idarticulo,art.descripcion, det.IdFactura,  IsNull(Det.Cantidad,0) as cantfac, IsNull(T.Cantidad,0) as cantcartanetofinal, 
 Det.IdDetalleFactura as [IdDetalleFactura], 'SIN IMPUTAR MAL' as [Tipo], IsNull(Det.Cantidad,0)-IsNull(T.Cantidad,0) as [Cantidad]
From Detallefacturas Det
Left Outer Join
...
--		Select Det.IdDetalleFactura as [IdDetalleFactura], 'ADMIN' End as [Tipo], 
--		Sum(CDP.NetoFinal/1000) as [Cantidad]
--		 Where  dbo.Factura_HechaEnProntoWeb(det.IdFactura)=0 
--		 Group By Det.IdDetalleFactura, CDP.Exporta

--Union

	
--		Select Det.IdDetalleFactura as [IdDetalleFactura], 'NOTACREDITO' End as [Tipo], 
--		Sum(CDP.NetoFinal/1000) as [Cantidad]
--		 Where  dbo.Factura_HechaEnProntoWeb(det.IdFactura)=0 
--		 Group By Det.IdDetalleFactura, CDP.Exporta
		 ...
		 --, MOVS.Cantidad/1000000 as [Cantidad] modificar tambien en el otro bloque
...
----where   dbo.Factura_CantidadDeCartasPorteImputadas(IdFactura)>0
--where dbo.Factura_TieneNotaDeCreditoImputada(idFactura)=0
----and dbo.Factura_HechaEnProntoWeb(IdFactura) =1

--Group By x.IdFactura
--order by 
--dbo.Factura_CantidadDeCartasPorteImputadasOriginalmente(IdFactura)-dbo.Factura_CantidadDeCartasPorteImputadas(IdFactura), sum(x.cantidad)
--dbo.Factura_CantidadDeCartasPorteImputadas(IdFactura)
