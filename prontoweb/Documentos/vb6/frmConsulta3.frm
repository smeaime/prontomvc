...
Public Property Let Id(ByVal vNewValue As Integer)
   ...
   Select Case mvarId
      ...
      Case 23
         Me.Caption = "Control de facturacion electronica"
         cmd(0).Caption = "Procesar"
         cmd(1).Visible = False
         cmd(2).Left = cmd(1).Left
         lblLabels(0).Visible = True
   ...
   Select Case Index
      Case 0
         Select Case Me.Id
            ...
            Case 23
               Set oTab = CopiarTodosLosRegistros(Aplicacion.Clientes.TraerFiltrado("_FacturacionElectronica", Array(DTFields(0).Value, DTFields(1).Value)))
               CompletarDatosFacturaElectronica oTab
               mvarSubTituloExcel = "|Desde el " & DTFields(0).Value & " al " & DTFields(1).Value
...
Public Sub CompletarDatosFacturaElectronica(ByRef oRs As ADOR.Recordset)
   ...
   End If
   If mResul Then
      mResul = FE.ObtenerTicketAcceso()
   Else
      MsgBox "No se pudo inicializar el componente de factura electronica", vbCritical
      GoTo Salida
   End If
   ...
   With oRs
      .MoveFirst
      Do While Not .EOF
         mResul = FE.bFEGetCMP(oRs.Fields("K_PuntoVenta").Value, oRs.Fields("K_TipoComprobanteAFIP").Value, oRs.Fields("K_IdIdentificacionCAE").Value)
         If mResul Then
