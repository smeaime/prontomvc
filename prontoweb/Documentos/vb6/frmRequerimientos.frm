...
Begin VB.Form frmRequerimientos 
   ...
   Begin VB.CommandButton cmdNavegar 
      ...
      Width           =   795
   End
   Begin VB.TextBox txtNumeradorEliminacionesFirmas 
      Alignment       =   1  'Right Justify
      DataField       =   "NumeradorEliminacionesFirmas"
      Enabled         =   0   'False
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   9.75
   ...
   Begin VB.CommandButton cmdAnularLiberacion 
      ...
      Width           =   1230
   End
   Begin VB.CommandButton cmdAnularFirmas 
      Caption         =   "Elim.Firmas"
      Height          =   330
      Left            =   2295
      TabIndex        =   93
      ...
      Width           =   1230
   End
   Begin VB.TextBox txtDetalleImputacion 
      DataField       =   "DetalleImputacion"
   ...
   Begin RichTextLib.RichTextBox rchObservacionesItemVisible 
      ...
      ReadOnly        =   -1  'True
      ScrollBars      =   2
      TextRTF         =   $"frmRequerimientos.frx":2C88
   End
   Begin VB.Line Line1 
   ...
   Begin VB.Label lblData 
      ...
      Width           =   1575
   End
   Begin VB.Label lblObcervacionesItem 
...
Private mvarGrabado As Boolean, mvarPorObra As Boolean, mvarModificado As Boolean, mvarImpresionHabilitada As Boolean
Private mvarAnulada As Boolean, mvarLiberada As Boolean, mFirmaActiva As Boolean, mVisualizacionParaAutorizar As Boolean
Private mPassword As String, mTipoObra As String, mCampoAprobo As String, mEstadoNavegadorComprobantes As String
...
Public Property Let VisualizacionParaAutorizar(ByVal mValor As Boolean)
   
   mVisualizacionParaAutorizar = mValor
   
End Property

Public Property Get VisualizacionParaAutorizar() As Boolean

   VisualizacionParaAutorizar = mVisualizacionParaAutorizar
...
Private Sub cmd_Click(Index As Integer)
   ...
   Select Case Index
      Case 0
         ...
         If mvarId <= 0 And DTFields(0).Value < Date And BuscarClaveINI("No permitir alta de requerimientos con fecha anterior a la del dia") = "SI" Then
            MsgBox "La fecha del requerimiento no puede ser anterior a la del dia", vbExclamation
            Exit Sub
         End If
...
'         End If
         ...
         Dim mvarImprime As Integer, mResp As Integer, i As Integer, mIdTipoCuenta As Integer
         Dim mCumplido As Boolean, mConfirmado As Boolean
         Dim mAuxS1 As String, mAuxS3 As String, mAuxS4 As String, mAuxS5 As String, mAuxErr1 As String, mAuxErr2 As String
               ...
               If .RecordCount > 0 Then
                  ...
                  Do While Not .EOF
                     If Not .Fields("Eliminado").Value Then
                        ...
                        If mAuxS1 = "SI" Then
                           ...
                           If oRsAux.RecordCount > 0 Then
                              If IsNull(oRsAux.Fields("IdUbicacionStandar").Value) Then
                                 mAuxErr1 = mAuxErr1 & vbCrLf & oRsAux.Fields("Descripcion").Value
                              End If
                           End If
                           oRsAux.Close
                        End If
                        ...
                        If mAuxS5 <> "SI" And Not IsNull(.Fields("IdArticulo").Value) Then
                           Set oRsAux = Aplicacion.Articulos.TraerFiltrado("_DatosConCuenta", .Fields("IdArticulo").Value)
                           mIdTipoCuenta = 0
                           If oRsAux.RecordCount > 0 Then mIdTipoCuenta = IIf(IsNull(oRsAux.Fields("IdTipoCuentaCompras").Value), 0, oRsAux.Fields("IdTipoCuentaCompras").Value)
                           oRsAux.Close
                           If mIdTipoCuenta = 4 Then
                              Set oRsAux = Aplicacion.Cuentas.TraerFiltrado("_PorObraCuentaMadre", Array(dcfields(0).BoundText, 0, .Fields("IdArticulo").Value, DTFields(0).Value))
                              If oRsAux.RecordCount = 0 Then
                                 MsgBox "El item " & .Fields("NumeroItem").Value & " no tiene una cuenta contable para la obra-cuenta de compras", vbExclamation
                                 oRsAux.Close
                                 Set oRsAux = Nothing
                                 Exit Sub
                              End If
         ...
         Set oRs = Nothing
         Set oRsAux = Nothing
         If Len(mAuxErr1) > 0 Then
            MsgBox "Los siguientes items no tienen ubicacion standar :" & mAuxErr1, vbExclamation
            Exit Sub
         End If
         If Len(mAuxErr2) > 0 Then MsgBox "Informacion :" & mAuxErr2, vbInformation
         ...
         With origen.Registro
            ...
            If Check2.Value = 1 Then
               .Fields("ParaTaller").Value = "SI"
            Else
               .Fields("ParaTaller").Value = Null
            End If
         End With
         ...
         If mCumplido Then
            ...
            If mResp = vbYes Then origen.Registro.Fields("Cumplido").Value = "SI"
         End If
...
Public Property Let Id(ByVal vNewValue As Long)
   ...
   If BuscarClaveINI("Habilitar tipo de compra en RM") = "SI" Then
      ...
      End With
   End If
   If BuscarClaveINI("Inhabilitar impresion de RM") = "SI" Then mvarImpresionHabilitada = False
   If BuscarClaveINI("Mostrar en requerimiento detalle de imputacion") = "SI" Then
      lblLabels(9).Visible = True
      txtDetalleImputacion.Visible = True
   Else
      rchObservaciones.Height = rchObservaciones.Height * 1.3
   End If
   ...
   If BuscarClaveINI("Requerir existencia de adjuntos en RM") = "SI" Then Frame5.Visible = True
   
   If BuscarClaveINI("Requerir destino taller en RM") = "SI" Then
      dcfields(6).Width = dcfields(6).Width - Check2.Width - 100
      With Check2
         .Top = dcfields(6).Top
         .Left = dcfields(6).Left + dcfields(6).Width + 100
         .Height = dcfields(6).Height
         .Visible = True
      End With
   End If
   ...
   Else
      With origen.Registro
         ...
         If Not IsNull(.Fields("Aprobo2").Value) Then
            ...
            Check1(0).Value = 1
         End If
         If Not IsNull(.Fields("NumeradorEliminacionesFirmas").Value) Then
            Line1.Visible = True
            txtNumeradorEliminacionesFirmas.Visible = True
         End If
         ...
         If Not IsNull(.Fields("Adjuntos").Value) Then
            ...
            End If
         End If
         If IIf(IsNull(.Fields("ParaTaller").Value), "", .Fields("ParaTaller").Value) = "SI" Then Check2.Value = 1
      End With
      ...
      If Check1(0).Value = 1 And Check1(1).Value = 1 Then
         For Each dc In dcfields
            If dc.Enabled Then dc.Enabled = False
         Next
         DTFields(0).Enabled = False
   ...
   If Me.VisualizacionParaAutorizar Then
      cmd(0).Visible = False
      cmd(2).Visible = False
      cmd(3).Visible = False
      cmdPegar.Visible = False
      cmd(1).Left = cmd(0).Left
      cmdImpre(0).Left = cmd(1).Left + cmd(1).Width + 20
      cmdImpre(1).Left = cmdImpre(0).Left + cmdImpre(0).Width + 20
      With cmdNavegar(0)
         .Top = cmd(0).Top
         .Left = cmdImpre(1).Left + cmdImpre(1).Width + 20
         .Visible = True
      End With
      With cmdNavegar(1)
         .Top = cmd(0).Top
         .Left = cmdNavegar(0).Left + cmdNavegar(0).Width + 20
         .Visible = True
      End With
      With cmdNavegar(2)
         .Top = cmd(0).Top
         .Left = cmdNavegar(1).Left + cmdNavegar(1).Width + 20
         .Visible = True
      End With
      With cmdNavegar(3)
         .Top = cmd(0).Top
         .Left = cmdNavegar(2).Left + cmdNavegar(2).Width + 20
         .Visible = True
      End With
      With lblObcervacionesItem
         .Top = cmd(0).Top - .Height
         .Left = cmdNavegar(3).Left + cmdNavegar(3).Width + 20
         .Visible = True
      End With
      With rchObservacionesItemVisible
         .Top = cmd(0).Top
         .Left = lblObcervacionesItem.Left
         .Height = cmd(0).Height
         .Width = cmd(0).Width * 7
         .Visible = True
      End With
...
Private Sub cmdAnularFirmas_Click()

   Dim oF As frmAutorizacion1
   Dim oRs As ADOR.Recordset
   Dim mError As String
   Dim mvarOK As Boolean, mvarConEntregas As Boolean
   ...
   mError = ""
   mvarConEntregas = False
   Set oRs = Aplicacion.Pedidos.TraerFiltrado("_PorItemRequerimiento", Array(-1, mvarId))
   With oRs
      If .RecordCount > 0 Then
         .MoveFirst
         Do While Not .EOF
            If .Fields("Entregado").Value > 0 Then mvarConEntregas = True
            mError = vbCrLf & mError & "Pedido : " & .Fields("Pedido").Value & " del " & .Fields("FechaPedido").Value & " Item : " & .Fields("NumeroItem").Value & " Recepcionado : " & .Fields("Entregado").Value
            .MoveNext
         Loop
      End If
      .Close
   ...
   If Len(mError) > 0 Then
      If mvarConEntregas Then mError = mError & vbCrLf & "Y existen entregas concretadas por lo que no es posible eliminar las firmas"
      Me.Refresh
      MsgBox "Los pedidos confeccionados desde esta RM son los suiguientes :" & mError, vbExclamation
      If mvarConEntregas Then Exit Sub
   End If
      
   With origen.Registro
      .Fields("Aprobo").Value = Null
      .Fields("CircuitoFirmasCompleto").Value = Null
      .Fields("IdUsuarioEliminoFirmas").Value = mIdAprobo
      .Fields("FechaEliminacionFirmas").Value = Now
      .Fields("NumeradorEliminacionesFirmas").Value = IIf(IsNull(.Fields("NumeradorEliminacionesFirmas").Value), 0, .Fields("NumeradorEliminacionesFirmas").Value) + 1
   End With
   Aplicacion.Tarea "AutorizacionesPorComprobante_EliminarFirmas", Array(EnumFormularios.RequerimientoMateriales, mvarId, -1, glbIdUsuario)
...
Private Sub cmdImpre_Click(Index As Integer)
   ...
   'If Not EmpleadoActivo(glbIdUsuario) Then
   '   MsgBox "El usuario no esta activo", vbCritical
   '   Exit Sub
   'End If
...
Private Sub cmdNavegar_Click(Index As Integer)
   ...
   Select Case Index
      Case 0
         Me.EstadoNavegadorComprobantes = "Anterior"
      Case 1
         Me.EstadoNavegadorComprobantes = "Proximo"
      Case 2
         Me.EstadoNavegadorComprobantes = "Firmar"
      Case 3
         Me.EstadoNavegadorComprobantes = "DarCumplido"
   End Select
   
   If Not actL2 Is Nothing Then
      With actL2
         .ListaEditada = Me.EstadoNavegadorComprobantes
         .Disparador = 0
      End With
   End If
...
Private Sub Lista_ItemClick(ByVal Item As MSComctlLib.IListItem)
   ...
   If rchObservacionesItemVisible.Visible Then
      rchObservacionesItemVisible.TextRTF = ""
      If Not Item Is Nothing Then
         If IsNumeric(Item.Tag) Then
            If Not IsNull(origen.DetRequerimientos.Item(Item.Tag).Registro.Fields("Observaciones").Value) Then
               rchObservacionesItemVisible.TextRTF = origen.DetRequerimientos.Item(Item.Tag).Registro.Fields("Observaciones").Value
            End If
         End If
      End If
   End If
...
'                  .SubItems(3) = "" & oRsDet.Fields("Cantidad").Value
      ...
      ElseIf InStr(1, Columnas(LBound(Columnas) + 1), "Req.") <> 0 Then
         ...
         Set oRM = oAp.Requerimientos.Item(Columnas(2))
         Set oRsRM = oRM.Registro
         Set oRsDet = oRM.DetRequerimientos.TraerFiltrado("_Todos", Columnas(2))
         ...
         End With
         rchObservaciones.TextRTF = IIf(IsNull(oRsRM.Fields("Observaciones").Value), "", oRsRM.Fields("Observaciones").Value)
...
Private Sub rchObservaciones_LostFocus()

   If Not cmd(0).Enabled Then Aplicacion.Tarea "Requerimientos_RegistrarImpresion", Array(mvarId, "", "", "", rchObservaciones.Text)
...
Private Sub txtDetalle_LostFocus()
   ...
   If Not cmd(0).Enabled Then Aplicacion.Tarea "Requerimientos_RegistrarImpresion", Array(mvarId, "", "", txtDetalle.Text)
...
Public Sub AsignaProveedor()
   ...
   Dim oL As ListItem
   Dim mDet As String, mDetId As String
   Dim mIdProv As Long, mIdLlamadoAProveedor As Long, mIdLlamadoRegistradoPor As Long
   Dim mFechaLlamadoAProveedor As Date, mFechaRegistracionLlamada As Date, mFechaEntrega As Date
...
Public Sub ActivarAnulacionLiberacion(ByVal Activar As Boolean)
   ...
   If Activar Then
      If BuscarClaveINI("Habilitar eliminacion firmas en requerimientos") = "SI" Then
...
Public Sub ActivarAnulacionFirmas(ByVal Activar As Boolean)
...
Public Property Get EstadoNavegadorComprobantes() As String

   EstadoNavegadorComprobantes = mEstadoNavegadorComprobantes

End Property

Public Property Let EstadoNavegadorComprobantes(ByVal vNewValue As String)

   mEstadoNavegadorComprobantes = vNewValue
