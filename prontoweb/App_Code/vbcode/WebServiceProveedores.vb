...
Public Class WebServiceProveedores
    ...
    Public Function GetCompletionList(ByVal prefixText As String, ByVal count As Integer, ByVal contextKey As String) As String()
        ...
        'Dim articulolist As ProveedorList = ProveedorManager.GetListParaWebService(sc, prefixText)
        'For Each proveedor As Pronto.ERP.BO.Proveedor In articulolist
        '    'items.Add(articulo.Descripcion)

        '    cod = proveedor.Id & "^" & proveedor.Cuit

        '    items.Add(AjaxControlToolkit.AutoCompleteExtender.CreateAutoCompleteItem(proveedor.RazonSocial, cod))
        'Next
        ...
        Dim proveedorlist = EntidadManager.GetStoreProcedure(sc, "wProveedores_TX_Busqueda", prefixText)
        For Each proveedor As Data.DataRow In proveedorlist.Rows
            'items.Add(articulo.Descripcion)

            cod = proveedor.Item("IdProveedor") & "^" & proveedor.Item("Cuit")
            items.Add(AjaxControlToolkit.AutoCompleteExtender.CreateAutoCompleteItem(proveedor.Item("RazonSocial"), cod))
        Next

        'http://vincexu.blogspot.com/2008/12/custom-autocomplete-3-check-if.html
        If proveedorlist.Rows.Count = 0 Then
