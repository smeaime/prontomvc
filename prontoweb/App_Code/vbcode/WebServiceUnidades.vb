...
Public Class WebServiceUnidades
    ...
    Public Function GetCompletionList(ByVal prefixText As String, ByVal count As Integer, ByVal contextKey As String) As String()
        ...
        'Dim articulolist = EntidadManager.GetStoreProcedure(sc, enumSPs.wCDPUnidades_TX_BusquedaConCUIT, prefixText)
        Dim articulolist = EntidadManager.ExecDinamico(sc, "SELECT  TOP 100  IdUnidad,Descripcion as Nombre " & _
                                                       " FROM Unidades WHERE Descripcion LIKE '" & prefixText & "%'")
        For Each proveedor As Data.DataRow In articulolist.Rows
            'items.Add(articulo.Descripcion)

            cod = proveedor.Item("IdUnidad") ' & "^" & proveedor.Item("Cuit")
