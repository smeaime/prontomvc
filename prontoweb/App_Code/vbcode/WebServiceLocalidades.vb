...
Public Class WebServiceLocalidades
    ...
    Public Function GetCompletionList(ByVal prefixText As String, ByVal count As Integer, ByVal contextKey As String) As String()
        ...
        Dim s As String = "SELECT  TOP 100  IdLocalidad,CodigoPostal,Localidades.Nombre,Partido,PROVINCIAS.Nombre as Prov FROM Localidades " & _
                                                        " LEFT JOIN PROVINCIAS ON PROVINCIAS.IdProvincia=Localidades.IdProvincia " & _
                                                        " WHERE Localidades.Nombre LIKE '" & prefixText & "%' or Localidades.CodigoPostal LIKE '" & prefixText & "%'"
        Dim articulolist = EntidadManager.ExecDinamico(sc, s)
        'Dim articulolist = EntidadManager.GetStoreProcedure(sc, "Localidades_TX_Busqueda", prefixText)
        For Each proveedor As Data.DataRow In articulolist.Rows
            'items.Add(articulo.Descripcion)

            cod = proveedor.Item("IdLocalidad") & "^" & proveedor.Item("CodigoPostal")
            items.Add(AjaxControlToolkit.AutoCompleteExtender.CreateAutoCompleteItem(proveedor.Item("CodigoPostal") & " " & proveedor.Item("Nombre") & " - " & proveedor.Item("Partido") & " " & proveedor.Item("Prov"), cod))
        Next
