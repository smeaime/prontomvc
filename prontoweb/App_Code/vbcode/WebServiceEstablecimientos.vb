...
Public Class WebServiceEstablecimientos
    ...
    Public Function GetCompletionList(ByVal prefixText As String, ByVal count As Integer, ByVal contextKey As String) As String()
        ...
        'Dim articulolist = EntidadManager.GetStoreProcedure(sc, enumSPs.wCDPEstablecimientos_TX_BusquedaConCUIT, prefixText)
        Dim articulolist = EntidadManager.ExecDinamico(sc, "SELECT  TOP 100  IdEstablecimiento,Descripcion as Nombre,Clientes.RazonSocial as Cliente, AuxiliarString1,AuxiliarString2 " & _
                                                       " FROM CDPEstablecimientos left JOIN CLIENTES on  CLIENTES.idcliente=CDPEstablecimientos.idtitular WHERE Descripcion LIKE '" & prefixText & "%'  OR  Clientes.Razonsocial LIKE '" & prefixText & "%'   OR  AuxiliarString1 LIKE '" & prefixText & "%' OR  AuxiliarString1 LIKE '" & prefixText & "%'")
        For Each proveedor As Data.DataRow In articulolist.Rows
            'items.Add(articulo.Descripcion)

            cod = proveedor.Item("IdEstablecimiento") ' & "^" & proveedor.Item("Cuit")
            items.Add(AjaxControlToolkit.AutoCompleteExtender.CreateAutoCompleteItem(proveedor.Item("Nombre") & " " & proveedor.Item("Cliente") & " " & proveedor.Item("AuxiliarString1") & " " & proveedor.Item("AuxiliarString2"), cod))
        Next
