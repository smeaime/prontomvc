...
Public Class WebServiceObras
    ...
    Public Function GetCompletionList(ByVal prefixText As String, ByVal count As Integer, ByVal contextKey As String) As String()
        ...
        'Dim articulolist = EntidadManager.GetStoreProcedure(sc, enumSPs.wCDPObras_TX_BusquedaConCUIT, prefixText)

        'Dim articulolist = EntidadManager.ExecDinamico(sc, _
        '        " Select TOP 100 IdObra ,           NumeroObra + ' - ' + Descripcion AS Nombre " & _
        '        " FROM Obras  WHERE Obras.FechaFinalizacion Is NULL  AND Obras.Jerarquia IS NOT NULL  " & _
        '        " AND ( NumeroObra LIKE '" & prefixText & "%'" & _
        '        " OR Descripcion LIKE '" & prefixText & "%'  )" & _
        '        " ORDER BY NumeroObra   ")


        Dim articulolist = EntidadManager.ExecDinamico(sc, _
                        " Select TOP 100 IdObra ,           isnull(NumeroObra,'') COLLATE Modern_Spanish_ci_as + ' - ' + isnull(Descripcion,'') COLLATE Modern_Spanish_ci_as AS Nombre " & _
                        " FROM Obras  WHERE Obras.FechaFinalizacion Is NULL   " & _
                        " AND ( NumeroObra LIKE '" & prefixText & "%'" & _
                        " OR Descripcion LIKE '" & prefixText & "%'  )" & _
                        " ORDER BY NumeroObra   ")
        ...
        For Each proveedor As Data.DataRow In articulolist.Rows
            ...
            cod = proveedor.Item("IdObra") ' & "^" & proveedor.Item("Cuit")
